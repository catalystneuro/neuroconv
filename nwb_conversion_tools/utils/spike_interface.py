    ...
    if nwbfile is None:
        ...
        with pynwb.NWBHDF5IO(str(save_path), mode=read_mode) as io:
            if read_mode == "r+":
                ...
                nwbfile_kwargs = dict(
                    session_description="Auto-generated by write_recording without a description.",
                    identifier=str(uuid.uuid4()),
                )
...
def write_sorting(
    ...
    if "NWBFile" not in metadata:
        metadata.update(NWBFile=dict())
    assert "session_start_time" in metadata["NWBFile"] or session_start_time is not None, (
        "'session_start_time' was not found auto-populated in metadata['NWBFile']! "
        "Please specify as an additional argument to run_conversion(...)"
    )
    if session_start_time is not None:
    ...
    if nwbfile is None:
        ...
        with pynwb.NWBHDF5IO(str(save_path), mode=read_mode) as io:
            if read_mode == "r+":
                ...
                nwbfile_kwargs = dict(
                    session_description="Auto-generated by write_sorting without a description.",
                    identifier=str(uuid.uuid4()),
                )
                nwbfile_kwargs.update(metadata["NWBFile"])
                nwbfile = pynwb.NWBFile(**nwbfile_kwargs)

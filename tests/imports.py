...
def test_top_level():
    """Check the import structure of the top level."""
    import neuroconv
...
def test_datainterfaces():
    """Check the import structure of the special `datainterfaces` submodule."""
    from neuroconv import datainterfaces

    current_structure = _strip_magic_module_attributes(ls=datainterfaces.__dict__)

    interface_name_list = [interface.__name__ for interface in datainterfaces.interface_list]
    expected_structure = [
        # Sub-modules
        "behavior",
        "ecephys",

...
def test_find_sub_builder_breadth_first():
    ...
    manager = get_manager()
    builder = manager.build(nwbfile)
    assert list(builder['processing'].keys()) == ['behavior1', 'behavior2'] # double check that depth-first would find 'behavior1' first
    time_series_builder = _find_sub_builder(builder, "TimeSeries")
...
def test_get_dataset_builder_not_a_dataset():
    ...
    error_message = re.escape(f"Could not find location '{location_in_file}' in builder (acquisition is not a dataset).")
    with pytest.raises(ValueError, match=error_message):
        get_dataset_builder(builder=builder, location_in_file=location_in_file)

...
def test_validator_chunk_shape_nonpositive_elements():
    ...
    expected_error = (
        "Some dimensions of the chunk_shape=(1, -2) are less than or equal to zero for dataset at location "
        "'acquisition/TestElectricalSeries/data'! (type=value_error)"
    )
    assert expected_error in str(error_info.value)
...
def test_validator_chunk_shape_exceeds_buffer_shape():
    ...
    expected_error = (
        "Some dimensions of the chunk_shape=(78126, 64) exceed the buffer_shape=(78125, 384) for dataset "
        "at location 'acquisition/TestElectricalSeries/data'! (type=value_error)"
    )
    assert expected_error in str(error_info.value)
...
def test_validator_buffer_shape_exceeds_full_shape():
    ...
    expected_error = (
        "Some dimensions of the buffer_shape=(1250000, 385) exceed the full_shape=(1800000, 384) for dataset at "
        "location 'acquisition/TestElectricalSeries/data'! (type=value_error)"
    )
    assert expected_error in str(error_info.value)

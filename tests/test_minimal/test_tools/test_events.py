...
class TestResolveDetectionPlan:
    ...
    def test_event_name_replaces_the_derived_identifier(self):
        ...
        event_type_source_ids = [name for name, _ in _resolve_detection_plan(detection_configuration)["DI/O-1"]]
    ...
    def test_derivation_is_content_based_not_positional(self):
        ...
        forward = _resolve_detection_plan({"DI/O-1": [{"detection": "rising"}, {"detection": "falling"}]})
        reversed_order = _resolve_detection_plan({"DI/O-1": [{"detection": "falling"}, {"detection": "rising"}]})

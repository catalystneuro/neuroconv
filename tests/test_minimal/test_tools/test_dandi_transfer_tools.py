...
class TestAutomaticDANDIUpload(TestCase):
    def setUp(self):
        ...
        metadata["NWBFile"].update(session_start_time=datetime.now().astimezone(), session_id="test-automatic-upload")
        metadata.update(Subject=dict(subject_id="foo", species="Mus musculus", age="P1D"))

...
class TestConditionSignal:
    ...
    def test_the_top_bit_of_a_signed_word_is_carved_correctly(self):
        """A word with its highest bit set is negative in a signed container, which must not lose it.

        SpikeGLX hands a 16-line digital word over as ``int16``, so line 15 makes the whole sample
        negative. The promotion to a wider signed type sign-extends, which is harmless here only because
        the low 16 bits survive it; a reinterpretation to unsigned would be needed to widen further.
        """
        word = np.array([0, 0b1000_0000_0000_0000, 0b1000_0000_0000_0001], dtype="uint16").view("int16")
        assert word[1] < 0  # the case being guarded

        assert_array_equal(_condition_signal(word, {"bits": [15]}), np.array([0, 1, 1]))
        assert_array_equal(_condition_signal(word, {"bits": [0]}), np.array([0, 0, 1]))
        assert_array_equal(_condition_signal(word, {"bits": [8]}), np.array([0, 0, 0]))

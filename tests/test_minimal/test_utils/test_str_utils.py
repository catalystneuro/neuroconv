...
def test_kilobytes():
    ...
    assert human_readable_size(1025) == "1.00 KiB"
    assert human_readable_size(10 * 1024) == "10.00 KiB"
    assert human_readable_size(1024 * 1024 - 1) == "1024.00 KiB"


def test_megabytes():
    """Test sizes from 1 MB to less than 1 GB"""
    assert human_readable_size(1024 * 1024) == "1.00 MiB"
    assert human_readable_size(5 * 1024 * 1024) == "5.00 MiB"
    assert human_readable_size(1024 * 1024 * 1024 - 1) == "1024.00 MiB"


def test_gigabytes():
    """Test sizes from 1 GB to less than 1 TB"""
    assert human_readable_size(1024**3) == "1.00 GiB"
    assert human_readable_size(10 * 1024**3) == "10.00 GiB"
    assert human_readable_size(1024**4 - 1) == "1024.00 GiB"
...
def test_very_large_numbers():
    ...
    assert human_readable_size(1024**6 * 3) == "3.00 EiB"

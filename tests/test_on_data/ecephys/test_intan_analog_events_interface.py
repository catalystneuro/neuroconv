...
class TestIntanAnalogEventsInterface:
    ...
    def test_configuration_is_required(self):
        with pytest.raises(ValidationError, match="detection_configuration"):
            IntanAnalogEventsInterface(file_path=FILE_PATH)
    ...
    def test_writes_configured_events_without_the_raw_trace(self):
        interface = IntanAnalogEventsInterface(
            file_path=FILE_PATH,
            detection_configuration=CONFIGURATION | OUTPUT_CONFIGURATION,
...
def test_converter_adds_adc_events_only_when_configured():
    converter = IntanConverter(
        file_path=FILE_PATH,
        detection_configuration=CONFIGURATION | OUTPUT_CONFIGURATION,
    )

    assert "AnalogADCInput" in converter.data_interface_objects
    assert "AnalogADCOutput" in converter.data_interface_objects
    assert "AnalogEvents" in converter.data_interface_objects
    assert "Digital" not in converter.data_interface_objects
    assert isinstance(converter.data_interface_objects["AnalogEvents"], IntanAnalogEventsInterface)

...
# Onset times (in the raw ComputerTimestamp time base) and their type labels from
# bl72bl82_12feb2024_stimuli.csv. Each label appears once.
EXPECTED_VALUE_TO_TIMESTAMP = {
    "whitenoise": 49926132.47,
    "pinknoise": 49956358.72,
    "AfVu": 49986359.78,
    "AmVf": 50016360.9,
    "AfVn": 50046366.14,
}
...
class TestNPMEventsInterface:
    ...
    def test_discovers_two_column_event_csv(self, interface):
        """Auto-discovery picks the two-column stimuli CSV; the multi-column raw signal CSV is
        excluded (it belongs to the fiber photometry interface)."""
        assert [path.name for path in interface._event_file_paths()] == ["bl72bl82_12feb2024_stimuli.csv"]
    ...
    def test_default_event_metadata_splits_by_label(self, interface):
        ...
        assert [event["value"] for event in events_metadata] == list(EXPECTED_VALUE_TO_TIMESTAMP)
    ...
    def test_original_timestamps(self, interface):
        ...
        assert set(original_timestamps) == set(EXPECTED_VALUE_TO_TIMESTAMP)
        for value, expected in EXPECTED_VALUE_TO_TIMESTAMP.items():
    ...
    def test_time_unit_scaling(self):
        ...
        np.testing.assert_allclose(timestamps["whitenoise"], [49926132.47 / 1e3], rtol=1e-12)

    def test_set_aligned_starting_time(self, interface):
        """Shifting by minus the first onset time places the first event at zero."""
        interface.set_aligned_starting_time(aligned_starting_time=-EXPECTED_VALUE_TO_TIMESTAMP["whitenoise"])
        timestamps = interface.get_timestamps()
        np.testing.assert_allclose(timestamps["whitenoise"], [0.0], atol=1e-6)
        np.testing.assert_allclose(
            timestamps["pinknoise"],
            [EXPECTED_VALUE_TO_TIMESTAMP["pinknoise"] - EXPECTED_VALUE_TO_TIMESTAMP["whitenoise"]],
            rtol=1e-9,
        )
    ...
    def test_add_to_nwbfile_writes_one_events_object_per_label(self, interface):
        ...
        assert set(behavior_module.data_interfaces) == set(EXPECTED_VALUE_TO_TIMESTAMP)
        for value, expected in EXPECTED_VALUE_TO_TIMESTAMP.items():
    ...
    def test_round_trip(self, interface, tmp_path):
        ...
        with NWBHDF5IO(nwbfile_path, mode="r") as io:
            ...
            read_events = read_nwbfile.processing["behavior"].data_interfaces["whitenoise"]
            assert isinstance(read_events, ndx_events.Events)
            np.testing.assert_allclose(list(read_events.timestamps[:]), [49926132.47], rtol=1e-12)

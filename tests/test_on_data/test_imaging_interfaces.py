...
class TestScanImageImagingInterfaceRecent(ImagingExtractorInterfaceTestMixin, TestCase):
    ...
    data_interface_cls = ScanImageImagingInterface
    interface_kwargs = dict(file_path=str(OPHYS_DATA_PATH / "imaging_datasets" / "Tif" / "scanimage_20220801_volume.tif"))
    save_directory = OUTPUT_PATH

    def check_extracted_metadata(self, metadata: dict):
        assert metadata["NWBFile"]["session_start_time"] == datetime(2022, 8, 8, 16, 56, 7, 329000)
        assert (
            metadata["Ophys"]["TwoPhotonSeries"][0]["description"]

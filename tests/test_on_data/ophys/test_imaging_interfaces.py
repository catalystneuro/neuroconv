...
class TestInscopixImagingInterfaceMovie128x128x100Part1(ImagingExtractorInterfaceTestMixin):
    ...
    def setup_metadata(cls, request):
        ...
        cls.device_metadata = dict(
            name=cls.device_name,
            description="Inscopix Microscope"
        )
    ...
    def check_extracted_metadata(self, metadata: dict):
        ...
        for category in ["Device", "ImagingPlane", "OnePhotonSeries"]:
            assert len(metadata["Ophys"][category]) == len(self.ophys_metadata[category]), \
        ...
        assert len(imaging_plane["optical_channel"]) == len(self.imaging_plane_metadata["optical_channel"]), \
    ...
    def check_read_nwb(self, nwbfile_path: str):
        ...
        with NWBHDF5IO(nwbfile_path, "r") as io:
            ...
            assert self.device_name in nwbfile.devices, \
                f"Device '{self.device_name}' not found in NWB file devices."
            ...
            assert one_photon_series.imaging_plane.name == self.imaging_plane_name, \

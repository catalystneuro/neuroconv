...
class TestPyPhotometryConverter:
    ...
    def test_conversion(self):
        ...
        assert set(nwbfile.acquisition) == {
            "FiberPhotometryResponseSeriesAnalog1",
            "FiberPhotometryResponseSeriesAnalog2",
        }
        ...
        assert nwbfile.acquisition["FiberPhotometryResponseSeriesAnalog1"].starting_time == 0.0
        assert nwbfile.acquisition["FiberPhotometryResponseSeriesAnalog2"].starting_time == pytest.approx(1 / 260)


class TestPyPhotometryFourColourConverter:
    """The fork's mode, where four signals come off two inputs and two lines ride beside them."""

    file_path = PYPHOTOMETRY_PATH / "mode_named_in_prose" / "four_colour_time_division.ppd"

    def test_get_available_streams(self):
        """Both colours of an input are signals of their own, and the lines still come last."""
        assert PyPhotometryConverter.get_available_streams(file_path=self.file_path) == [
            "analog_1",
            "analog_2",
            "analog_1_color_2",
    ...
    def test_conversion(self):
        """A name has to carry the colour here, since two signals share an analog input."""
        nwbfile = PyPhotometryConverter(file_path=self.file_path).create_nwbfile()

        assert set(nwbfile.acquisition) == {
            "FiberPhotometryResponseSeriesAnalog1",
            "FiberPhotometryResponseSeriesAnalog2",
            "FiberPhotometryResponseSeriesAnalog1Color2",
            "FiberPhotometryResponseSeriesAnalog2Color2",
        }
        # Four signals but still two lines: a colour is not a slot of its own on the digital side.
        assert set(nwbfile.events) == {"Digital1", "Digital2"}
        assert nwbfile.session_start_time.replace(tzinfo=None) == datetime(2021, 3, 19, 17, 43, 56)

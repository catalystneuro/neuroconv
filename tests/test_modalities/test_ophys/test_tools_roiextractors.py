...
class TestNoMetadataMutation:
    ...
    def test_add_photon_series_respects_user_dimension(self):
        """Test that user-provided dimension in metadata is respected and not overridden by extractor."""
        from roiextractors.testing import generate_dummy_imaging_extractor
        ...
        # Extractor has shape (10, 10), but user provides custom dimension
        custom_dimension = [20, 20]
                ...
                "TwoPhotonSeries": [
                    {
                        ...
                        "unit": "px",
                        "imaging_plane": "TestImagingPlane",
                        "dimension": custom_dimension,  # User-provided dimension
                    }
                ],
            }
        }
        ...
        # Verify user's custom dimension was used, not extractor's
        two_photon_series = nwbfile.acquisition["TestTwoPhotonSeries"]
        assert (
            list(two_photon_series.dimension) == custom_dimension
        ), f"Expected dimension {custom_dimension}, but got {two_photon_series.dimension}"

    def test_add_photon_series_raises_when_custom_imaging_plane_not_defined(self):
        """Test that providing custom imaging_plane name without defining it raises clear error."""
        from roiextractors.testing import generate_dummy_imaging_extractor
        ...
        # User provides photon series with custom imaging_plane name but doesn't define that plane
        metadata = {
            "Ophys": {
                "Device": [{"name": "TestMicroscope"}],
                "TwoPhotonSeries": [
                    {
                        ...
                        "description": "Test two photon series",
                        "unit": "px",
                        "imaging_plane": "MyCustomPlane",  # Custom name but no definition provided
                    }
                ],
            }
        }

        # Should raise error because MyCustomPlane is not defined in metadata
        with pytest.raises(ValueError, match="Metadata for Imaging Plane 'MyCustomPlane' not found"):
            add_photon_series_to_nwbfile(
                imaging=imaging_extractor,
                nwbfile=nwbfile,
                metadata=metadata,

...
The ``base_times`` come from two places. Initially they are the source's **native times**. A call to ``set_times``
can supply a replacement, or ``remap_times`` can compute one from the current times and synchronization pairs. The
offset starts at ``0.0``, so an object with no alignment writes its native times.

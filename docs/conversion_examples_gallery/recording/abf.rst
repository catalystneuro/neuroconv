...
several channels in a converter for a dual-patch or multi-file recording.
...
Each ``AxonIntracellularInterface`` converts a single channel: one electrode's recording in one ABF file,
written as a continuous ``PatchClampSeries`` (much as a single stream is converted in an extracellular
recording). :py:class:`~neuroconv.datainterfaces.icephys.axon.axonintracellularconverter.AxonIntracellularConverter`
combines several such channels and builds the NWB icephys hierarchy over them: the ``SimultaneousRecordings``
(channels recorded together) and ``SequentialRecordings`` (one per run, carrying the stimulus type) tables. You
give it a list of interfaces, one per channel. A single cell is the one-interface case; pass one interface per
electrode for a **dual-patch** recording (the two channels of each sweep grouped into one simultaneous
recording), or one per file for a **multi-file** experiment (each run becomes its own sequential recording, and
the runs are placed on a single timeline from each file's header start time, which requires ABF version 2). The
example below combines two channels recorded in one file.
...
.. code-block:: python
    ...
    ...     response_channel_name="_Ipatch",  # recorded in mV
    ...     mode="current_clamp",
    ... )
    >>> voltage_clamp = AxonIntracellularInterface(
    ...     file_path=f"{ECEPHY_DATA_PATH}/axon/File_axon_6.abf",
    ...     response_channel_name="IN1",      # recorded in nA
    ...     mode="voltage_clamp",
    ... )
    ...
    >>> metadata["Subject"] = dict(subject_id="subject1", species="Mus musculus", sex="M", age="P30D")
    >>>
    >>> # This should be something like: "./saved_file.nwb"
    >>> converter.run_conversion(nwbfile_path=output_folder / "axon_converter_conversion.nwb", metadata=metadata)

...
## Features
* Added `OpenEphysBinaryEventsInterface` for converting the discrete events of an Open Ephys binary recording, both the TTL lines of an event stream and the Message Center's text annotations. The `.npy` arrays are read directly rather than through neo, so each line that fired becomes its own `pynwb.event.EventsTable` holding every edge it produced, with the transition direction in an `edge` column and the latched port value in a `full_word` column that the neo path drops. [PR #1946](https://github.com/catalystneuro/neuroconv/pull/1946)

...
* Fixed the `spike2` extra installing no `sonpy`: its `python_version=='3.9'` marker can never be satisfied under `requires-python = ">=3.10"`. The extra now targets the platform/Python combinations where sonpy publishes a usable wheel (Windows, or Python 3.14+), and the availability guard and its test were updated to match. [PR #1855](https://github.com/catalystneuro/neuroconv/pull/1855)

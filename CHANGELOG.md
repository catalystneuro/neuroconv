...
* `VameInterface` now also writes, for each motif run, a curated `ndx-ethogram` product alongside the faithful `ndx-vame` series: the per-frame `MotifSeries` is run-length-encoded into an `EthogramBouts` timeline (one row per bout) with an `Ethogram` catalogue listing one row per motif id, the modal community per motif recorded in the catalogue's `category` column. The bouts link back to the producing `MotifSeries` (`source`) and the upstream `PoseEstimation` (`source_pose`). Requires the `ndx-ethogram` extension. [PR #1766](https://github.com/catalystneuro/neuroconv/pull/1766)

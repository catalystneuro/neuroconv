...
* Added `metadata_key` support to segmentation interfaces (`BaseSegmentationExtractorInterface`, `CaimanSegmentationInterface`, `CnmfeSegmentationInterface`, `ExtractSegmentationInterface`, `SimaSegmentationInterface`) and `use_new_metadata_format` parameter to `get_metadata()` for dict-based metadata pipeline. [PR #XXXX]

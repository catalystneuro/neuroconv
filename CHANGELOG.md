...
## Features
* Added `OpenEphysBinaryEventsInterface` for converting the discrete events of an Open Ephys binary recording, both the TTL lines of an event stream and the Message Center's text annotations. The `.npy` arrays are read directly rather than through neo, which pairs the edges into intervals at parse time, drops `full_words`, and skips a whole line when its rising and falling edges do not match; here each line that fired becomes its own `pynwb.event.EventsTable` holding every edge it produced, with the transition direction in an `edge` column and the latched port value in a `full_word` column, so a line left high at the end of a recording is kept and the interval reading stays available from those rows. One interface reads one event stream of one experiment, named as SpikeInterface names the matching recording stream (`Record Node 104#NI-DAQmx-103.PXIe-6341`) and selected with `block_index` where a session holds several experiments. The session start time is read from the settings file the GUI wrote for that experiment (`settings.xml` for the first, `settings_3.xml` for the third), so an events-only conversion carries the same start time as the recording it came from. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

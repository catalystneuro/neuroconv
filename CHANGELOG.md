...
## Bug Fixes
* Fixed `MiniscopeBehaviorInterface.get_metadata()` returning `session_start_time` with the `msec` field from `metaData.json` assigned directly to `microsecond`, off by a factor of 1000. The interface now converts milliseconds to microseconds correctly. [PR #1725](https://github.com/catalystneuro/neuroconv/pull/1725)
...
* Added `metadata_key` support to segmentation interfaces (`BaseSegmentationExtractorInterface`, `CaimanSegmentationInterface`, `CnmfeSegmentationInterface`, `ExtractSegmentationInterface`, `SimaSegmentationInterface`) and `use_new_metadata_format` parameter to `get_metadata()` for dict-based metadata pipeline. [PR #1708](https://github.com/catalystneuro/neuroconv/pull/1708)
...
## Improvements
* Internalized the Miniscope V4 format parsers used by `MiniscopeBehaviorInterface` into a private `_miniscope_readers` module instead of importing them from `ndx_miniscope.utils`. `ndx-miniscope` is now used only for NWB construction helpers (`Miniscope` device type, `add_miniscope_device`, `add_miniscope_image_series`). V3 Miniscope data is no longer parsed by the active interfaces; `MiniscopeImagingInterface` and `MiniscopeBehaviorInterface` now raise a clear `NotImplementedError` on V3 folders that points users to the issue tracker so V3 support can be added against real fixtures. [PR #1725](https://github.com/catalystneuro/neuroconv/pull/1725)

...
* Added `PyPhotometryEventsInterface`, which converts the digital lines of a pyPhotometry `.ppd` recording. Each line rides in the low bit of the words of the analog input it shares a slot with, so it is read as a sampled `0`/`1` signal on that input's own clock and written as one `EventsTable` per line. [PR #1916](https://github.com/catalystneuro/neuroconv/pull/1916)
* Added `PyPhotometryConverter`, which writes a pyPhotometry `.ppd` recording whole. Everything the board recorded is interleaved into one stream of words, so a file that reads as one signal to a user is several `FiberPhotometryResponseSeries` and several `EventsTable` objects in NWB, and how many of each follows from the header's acquisition mode. The converter reads the mode, builds one fiber photometry interface per fluorescence signal and one events interface for the digital lines, and names each series after the analog input its signal came off, since every single-series interface would otherwise call its series `FiberPhotometryResponseSeries` and a file cannot hold two of those. `get_available_streams` reports the signals and the lines before construction, and `detection_configuration` is forwarded to the events interface, which is also how a line is left out. [PR #1918](https://github.com/catalystneuro/neuroconv/pull/1918)
...
* Added `PyPhotometryFiberPhotometryInterface`, which converts a pyPhotometry `.ppd` recording. It dispatches on the header's `mode` field, which is the only thing that says how the interleaved signals are laid out, and writes each of them with the start time it was sampled at rather than the shared timebase every existing reader returns. [PR #1905](https://github.com/catalystneuro/neuroconv/pull/1905)

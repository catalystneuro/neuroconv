...
* `SLEAPInterface` now raises when no instance in the file carries a track, which is what a labeling project or an untracked single-animal recording looks like, instead of falling through to the deprecated write-every-track path with a warning about tracks the file does not have. [PR #1993](https://github.com/catalystneuro/neuroconv/pull/1993)

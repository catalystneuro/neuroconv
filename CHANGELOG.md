...
## Features
* Added `MiniscopeHeadOrientationInterface` for converting Miniscope head orientation data from BNO055 IMU sensor. The interface reads `headOrientation.csv` files containing quaternion orientation data (qw, qx, qy, qz) and stores them as `SpatialSeries` in the behavior processing module. Supports temporal alignment for multi-session recordings and automatically extracts device metadata (device name, device type) and session start time from Miniscope configuration files. [PR #1610](https://github.com/catalystneuro/neuroconv/pull/1610) (Closes #1535)

...
* Renamed the container-selection parameter `write_as` to `parent_container` (now keyword-only). The old `write_as` keyword still works but emits a `FutureWarning` and will be removed on or after December 2026. For the sorting functions the container value `"units"` is now `"canonical"` (the official `NWBFile.units` slot); the deprecated `write_as="units"` maps to `parent_container="canonical"`. The `write_as` parameters that select a data representation (`"raw"/"processed"/"lfp"` on the recording functions) are a different concept and are unchanged. [PR #1760](https://github.com/catalystneuro/neuroconv/pull/1760)
=======
* Renamed the container-selection parameter `write_as` to `parent_container` (now keyword-only). The old `write_as` keyword still works but emits a `FutureWarning` and will be removed on or after December 2026. The `write_as` parameters that select a data representation (`"raw"/"processed"/"lfp"` on the recording functions) are a different concept and are unchanged. [PR #1760](https://github.com/catalystneuro/neuroconv/pull/1760)

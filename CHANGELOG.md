...
* Removed the `configuration_file_path` parameter from `MiniscopeImagingInterface`, deprecated for removal on or after May 2026. The device folder is inferred from `file_paths`, so `folder_path` or `file_paths` is now the whole choice, and roiextractors can drop the parameter it has been accepting and ignoring for the same reason.
* `MiniscopeImagingInterface.__init__` and `.add_to_nwbfile` no longer accept arguments positionally past the first, deprecated for removal on or after August 2026.

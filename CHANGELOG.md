...
## Features
* `GuppyInterface` is now public, which is also how a session GuPPy processed out of an existing NWB file is converted — GuPPy reads NWB as an input format, so for those sessions the acquisition is already converted and there is nothing for a converter to read in. Hand the file to `add_to_nwbfile` and the interface adds GuPPy's outputs to it, pointing the two GuPPy registries at the tables already there: GuPPy's `storesList.csv` ids were derived from that file's contents (`<series>_<column>` names a response series column, `<table>_<value>` names an `EventsTable`'s rows), so each recording site links to the `FiberPhotometryTable` rows its fibers occupy and each event to its occurrence rows. Nothing is copied and everything else the file holds is untouched; writing to a new path exports the source with the outputs added. Stores the file does not hold — GuPPy's custom event CSVs, for instance — leave that registry link-free with a warning naming them, and the two registries resolve independently. An NWB file holding no fiber photometry still gets the minimal link-free registries, unchanged. [PR #1880](https://github.com/catalystneuro/neuroconv/pull/1880)

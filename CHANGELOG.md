...
## Features
* Added `GuppyConverter` to bundle a GuPPy session's raw acquisition, raw events, and GuPPy-derived outputs into a single conversion. The recording format is selected with `acquisition_format` (`"tdt"`, `"csv"`, `"doric"` or `"npm"`, covering every format GuPPy itself reads). Install with `neuroconv[guppy]`, which now carries every supported acquisition format. Acquisition series are grouped by excitation wavelength rather than written one per store, so a session's `signal` and `control` channels each become one `FiberPhotometryResponseSeries` column-stacked across recording sites. The `GuppyEventsTable` registry references only the occurrences GuPPy actually analyzed: GuPPy discards an onset closer to the recording's start than `abs(baselineCorrectionStart)`, and the later of any pair closer together than `timeInterval`, so a session's acquisition table legitimately holds occurrences that no peri-event product covers. `_GuppyInterface` reads the surviving onsets from `<event>_<recording_site>.hdf5` and exposes them as `analyzed_event_onsets`; `generate_mock_guppy_output_folder` gained an `event_onsets` argument that fills those files and labels every peri-event product's trial columns from one list. [PR #1783](https://github.com/catalystneuro/neuroconv/pull/1783)

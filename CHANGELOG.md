...
## Features
* Added `CSVFiberPhotometryInterface` for converting raw fiber photometry recordings from CSV file(s), parsed into the `ndx-fiber-photometry` format. It inherits from `BaseFiberPhotometryInterface` and is single-series: point it at one or more `file_paths` and name the `timestamps_column` and `data_columns` (by header name or positional index), and it writes one `FiberPhotometryResponseSeries` whose channels are the data columns read across the files. Install with `pip install "neuroconv[csv_fp]"`. [PR #1754](https://github.com/catalystneuro/neuroconv/pull/1754)

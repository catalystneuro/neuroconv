...
## Features
* `KiloSortSortingInterface` now writes the templates in `templates.npy` as the `waveform_mean` column of the units table, together with the electrodes each unit came from and its peak electrode as `max_electrode`. Each template is unwhitened and restricted to its footprint, the channels Kilosort fit it on, and the electrodes table is built from the sorter folder when the file does not already have one, so a Kilosort folder with no raw data still gets a full electrodes table. Kilosort records no gain of its own and `waveform_mean` is fixed to volts by the schema, so the waveforms are written only when a recording is registered or the interface is constructed with `gain_to_uV`, and skipped with a warning otherwise. The electrodes table it writes goes through the dict-based metadata helpers only, so the device and the electrode groups are customized through `metadata["Devices"]` and `metadata["Ecephys"]["ElectrodeGroups"]`. Because the electrode information is now always written, `write_ecephys_metadata` no longer has any effect on this interface; it is still accepted so that a converter passing it does not fail. [PR #1871](https://github.com/catalystneuro/neuroconv/pull/1871)

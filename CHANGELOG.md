...
* Added `MedPCCodedEventsInterface`, for the other layout an MSN program can write and one nothing in NeuroConv could read before: every event is a single `TIME.EVENTCODE` value in one array, the integer part being the time in clock ticks and the fractional digits the event type's code, packed by a program line like `Set A(Y) = BTIME-U + code/1000`. Which layout a file is in is decided by the program rather than by MedPC, which is why it is a separate interface rather than a mode: the two share only how a session is selected and how its header is read. Two things have to be stated because the file does not carry them. `clock_ticks_per_second` (500 for a 2 ms clock, 200 for a 5 ms one), since the file stores its times in ticks and the wrong rate silently scales every timestamp. And `event_configuration`, the legend naming the codes, which cannot be derived even when the MSN program ships beside the data, as that copy is often a later version whose numbering disagrees with the file. The legend is optional and is a legend rather than a declaration: every code the array holds becomes an event type whether or not it is named, and one left out takes its digits as its name (`code_029`), while a code named but never fired is written as an empty table. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

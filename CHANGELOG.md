...
* `dict_deep_update` now forwards `compare_key` and `list_dict_deep_update` into the nested mappings and list entries it recurses into, where both had fallen back to their defaults, so a `compare_key` other than `name` no longer raises `KeyError` below the top level and `list_dict_deep_update=False` replaces nested list entries as asked. [PR #2009](https://github.com/catalystneuro/neuroconv/pull/2009)
* The `neuroconv` command line entry point no longer fails with `AttributeError: 'str' object has no attribute 'parent'` when `--output-folder-path` is not given, and a session's `nwbfile_name` keeps its leading and trailing `n`, `w` and `b` characters: the `.nwb` suffix was removed with `str.strip`, which strips characters rather than a suffix, so `brain_waves.nwb` was written as `rain_waves.nwb`. [PR #2011](https://github.com/catalystneuro/neuroconv/pull/2011)
* `add_segmentation_to_nwbfile` called without metadata no longer refuses an extractor that holds ROIs but no traces. The placeholder metadata it fills in carries a `RoiResponses` entry, which the trace writer took for metadata the caller had written and rejected for lacking the traces to match, so a masks-only segmentation converted only when the caller wrote out metadata that named no traces. [PR #2015](https://github.com/catalystneuro/neuroconv/pull/2015)
...
* A GuPPy NPM session's channel count is now read from the run's own `.npm_params.json`, beside the timestamp column and time unit it already recorded there, falling back to the `GuPPyParamtersUsed.json` snapshot for runs written before GuPPy recorded it there. [PR #2046](https://github.com/catalystneuro/neuroconv/pull/2046)
...
## Improvements
* Declared ecephys electrode registries now require a saved `channel_to_electrode` mapping. Source geometry, contact identity, and other properties follow that association into annotated rows, so attaching a probe after preparing annotations no longer disconnects them. Missing mappings, undeclared targets, and conflicting contact identities raise instead of silently creating or reassigning electrodes. Templates generate the mapping with the rows; conversions without a declared registry remain automatic. [PR #2001](https://github.com/catalystneuro/neuroconv/pull/2001)
...
* The events interfaces now list their event types with `get_event_type_source_ids` from their header or configuration and build the `event_types` metadata from that list instead of the other way round, so listing the types reads no events for TDT and per-array MedPC files, whose configuration errors now surface on the first read instead of in `get_metadata`. [PR #2038](https://github.com/catalystneuro/neuroconv/pull/2038)
* Added `EventsInterfaceTestMixin` to `neuroconv.tools.testing.data_interface_mixins`, a `DataInterfaceTestMixin` whose round trip checks that every event type's written rows carry the times `get_event_times` reports, and moved the events test classes onto it. `DataInterfaceTestMixin` also gained an `edit_metadata` hook to shape the metadata the round trip writes, so a test class can write under the names a user would give the event types. [PR #2040](https://github.com/catalystneuro/neuroconv/pull/2040)
* Changelog entries are now written one file per entry under `changelog_entries/`, named `<PR number>.<type>.md`, and assembled into `CHANGELOG.md` by `towncrier build` at release time, so two pull requests no longer edit the same lines. [PR #2042](https://github.com/catalystneuro/neuroconv/pull/2042)
* The series inside a `PoseEstimation` container now link to a single timestamps dataset instead of each writing its own copy, which takes a ten-keypoint container over five thousand irregularly sampled frames from 1.02 MB to 0.65 MB and leaves regularly sampled data, written as a rate, untouched. [PR #2044](https://github.com/catalystneuro/neuroconv/pull/2044)
...
* Deprecated `MedPCInterface`, to be removed in v0.12.0, in favor of `MedPCArrayEventsInterface`, which reads the same files and writes their events as native `pynwb.event.EventsTable` objects into `nwbfile.events` rather than as `ndx-events` objects and `IntervalSeries` into the behavior processing module; `ndx-events` moved with it to a new `medpc_legacy` extra, leaving `medpc` for the events interfaces. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
...
* Writing every track and every recording of a `.slp` at once is deprecated, to be removed in v0.12.0, since an NWB file holds one subject and a `PoseEstimation` holds one camera view. Name them with `SLEAPInterface`'s new `track_name` and `video_name` arguments and use one interface per pair; `get_available_tracks` and `get_available_videos` list them. [PR #1965](https://github.com/catalystneuro/neuroconv/pull/1965)
...
* Fixed `make_nwbfile_from_metadata` converting `metadata["Subject"]["date_of_birth"]` from its ISO 8601 string to a `datetime` inside the caller's own dictionary. The `NWBFile` block was deep-copied and the subject block was not, so a conversion stating a date of birth came back with the metadata it passed in rewritten. [PR #1962](https://github.com/catalystneuro/neuroconv/pull/1962)
...
## Features
* Added `BORISInterface`, which converts one observation of a BORIS project, writing its events as a `pynwb.event.EventsTable` and the coding scheme beside them as an `ndx-ethogram` `Ethogram` catalogue with the closed state bouts in an `EthogramBouts` table. [PR #1996](https://github.com/catalystneuro/neuroconv/pull/1996)
* Added `MoseqKeyPointsInterface`, which converts one recording of a keypoint-MoSeq `results.h5`, writing the syllable sequence as `ndx-ethogram` bouts and the centroid, heading and latent-state trajectory as core NWB objects. [PR #1995](https://github.com/catalystneuro/neuroconv/pull/1995)
* Added `MedPCArrayEventsInterface` and `MedPCPackedEventsInterface`, which write the events of a MedPC output file as native `pynwb.event.EventsTable` objects into `nwbfile.events`, one for files holding one lettered variable per event type and one for files packing the event code into the decimals of the time value. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* Added `ExternalImageInterface`, which writes a collection of images into an `Images` container as paths pointing at the source files instead of embedding their pixels, so the originals keep their format and their metadata and are not duplicated across the NWB files that reference them. [PR #1987](https://github.com/catalystneuro/neuroconv/pull/1987)
...
* Added `PyPhotometryConverter`, which writes a pyPhotometry `.ppd` recording whole, reading the header's acquisition mode to build one fiber photometry interface per fluorescence signal, named after the stream it came off, plus one events interface for the digital lines. [PR #1918](https://github.com/catalystneuro/neuroconv/pull/1918)
* `GuppyInterface` now reads GuPPy's spontaneous mode, where each recording site's own detected transients stand in for external TTLs, writing the PSTHs and peak/AUC summaries built around them. Such an event is registered once per recording site, since the sites do not share a transient train. [PR #1976](https://github.com/catalystneuro/neuroconv/pull/1976)
* `GuppyInterface` now writes GuPPy's whole-session time-binned metrics, the behavioral covariates binned onto those bins, and the correlations between them as `ndx-guppy` objects, with each covariate's scored values written as a `TimeSeries` the two covariate products reference. Both GuPPy steps are optional, so the objects appear only for a session that ran them. [PR #1975](https://github.com/catalystneuro/neuroconv/pull/1975)
* `DatasetIOConfiguration` now exposes `full_size_in_bytes`, `maximum_ram_usage_per_iteration_in_bytes` and `disk_space_usage_per_chunk_in_bytes`, the three quantities its printout reports, which until now were computed inside `__str__` and could only be read by parsing the printed text. [PR #1980](https://github.com/catalystneuro/neuroconv/pull/1980)
* `GuppyInterface` now writes the outputs of GuPPy's tonic analysis as an `ndx-guppy` `GuppyTonicEpochs` object: the per-recording-site epoch windows and the mean of each normalized trace over each window, one row per (recording_site, epoch, trace_type). It is written only for a session that ran that optional GuPPy step. [PR #1974](https://github.com/catalystneuro/neuroconv/pull/1974)
* A pose estimation container can link the `ImageSeries` its keypoints were tracked from, through a `source_video_metadata_key` (and `labeled_video_metadata_key`) addressing an entry in `metadata["Behavior"]["ExternalVideos"]`, the way `device_metadata_key` addresses `metadata["Devices"]`. `LightningPoseConverter` now writes that link instead of naming the `ImageSeries` in the `original_videos` path field. [PR #1964](https://github.com/catalystneuro/neuroconv/pull/1964)
* `set_probe` on the recording interfaces now takes `channel_id_to_contact_id`, which states which contact each channel recorded as `{channel_id: contact_id}` and applies the wiring to the probe for you. A probe from a catalogue describes a part rather than a wiring, so it arrives with no channel assignment and could not be attached until the caller called probeinterface's `set_device_channel_indices` themselves. That call takes channel *indices* positional to the probe's own contact order, with `-1` for a contact nothing recorded, which is three conventions to hold at once and none of them is what a wiring table says; a list off by a permutation has the right length, raises nothing, and attributes every channel to the wrong contact. The mapping is by id on both sides, a contact absent from its values is unrecorded by that interface, and a contact or channel that does not exist, or two channels claiming one contact within the same recording, is refused. The caller's probe is not modified.
* The electrodes table can now be stated in `metadata["Ecephys"]["ElectrodesTable"]`, as `rows`, one entry per electrode pointing at its group with `electrode_group_metadata_key`, and `columns`, which renames a column, describes it, pins its dtype or gives an encoded one a vocabulary written as a `MeaningsTable`. A row wins over the recording for the fields it states and inherits the rest, `get_metadata_template()` on the recording interfaces returns the block prefilled, and the electrode keys derive from contact identity, so a probe's AP and LF bands share rows and the joined `AP0,LF0` channel name the SpikeGLX interface used to fabricate is removed. [PR #2001](https://github.com/catalystneuro/neuroconv/pull/2001)
* Added `add_subject_to_nwbfile`, which writes `metadata["Subject"]` onto an NWBFile that already exists. The subject was reachable only through `make_nwbfile_from_metadata`, so there was no way to add one to a file in hand and no single place that owned turning the metadata block into a `Subject`. Adding a subject to a file that already holds one raises, since an NWBFile describes one subject. [PR #1962](https://github.com/catalystneuro/neuroconv/pull/1962)
...
* The events writer now writes a payload field holding several values per event, a trial tagged with two conditions or a behavior scored with two qualifiers, as a ragged column instead of flattening them into a single string, and reads which columns take that shape off the payload instead of from a metadata flag. [PR #1997](https://github.com/catalystneuro/neuroconv/pull/1997)
...
* `DeepLabCutInterface` reads the skeleton's edges from the project config, which states them as pairs of bodypart names, instead of only from the `_meta.pickle` beside the output file. DeepLabCut does not always write that pickle, and when it is absent the skeleton was written with nodes and no connections. The pickle is still read when the config names no skeleton. [PR #1969](https://github.com/catalystneuro/neuroconv/pull/1969)
...
* A local `pytest` run now performs the same `EncodingWarning` check CI performs. `PYTHONWARNDEFAULTENCODING` is set by `pytest-env`, which runs after the interpreter has already read it, so the flag only ever reached the xdist workers and a `read_text()` or `write_text()` without an explicit `encoding=` passed locally while failing on every CI platform. The `--numprocesses=logical --dist=loadscope` that each workflow spelled out on its command line moved into `addopts`, so any invocation starts those workers; the runs meant to stay serial, the doctests, the per-format gallery pages and the four `tests/imports.py` calls that each need a virgin interpreter, now say `--numprocesses=0` explicitly. [PR #1973](https://github.com/catalystneuro/neuroconv/pull/1973)

...
* Writing every track and every recording of a `.slp` at once is deprecated, to be removed on or after August 2027, since an NWB file holds one subject and a `PoseEstimation` holds one camera view. Name them with `SLEAPInterface`'s new `track_name` and `video_name` arguments and use one interface per pair; `get_available_tracks` and `get_available_videos` list them. [PR #YYYY](https://github.com/catalystneuro/neuroconv/pull/YYYY)

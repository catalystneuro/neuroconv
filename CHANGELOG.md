...
## Bug Fixes
* Fixed `OpenEphysBinaryAnalogInterface` discarding the caller's `TimeSeries` metadata. Its entry was built inside `add_to_nwbfile` and assigned unconditionally, after the metadata argument had already been accepted, so any edit to that entry's name or description was silently overwritten by the interface's own defaults. The entry now comes out of `get_metadata` like every other analog interface, so it can be edited before the write rather than being replaced by it. [PR #1869](https://github.com/catalystneuro/neuroconv/pull/1869)

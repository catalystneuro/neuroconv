...
## Bug Fixes
* Fixed the Inscopix segmentation gallery page not showing its install command. The `.. code-block:: bash` directive was missing the blank line before its content, so docutils dropped the block and the page went straight from "Install NeuroConv with the additional dependencies" to the conversion example, with `pip install "neuroconv[inscopix]"` appearing nowhere on the rendered page.

...
* `interface.alignment` on the fiber photometry interfaces is now the full alignment surface: it names the interface's time-bearing objects, so `alignment.keys()` enumerates them and `alignment[key]` reaches one, and it carries `set_times` for per-sample times you already trust and `remap_times(stream_sync_times=, reference_sync_times=)` for re-expressing a drifting clock against a reference one through shared synchronization pulses. `shift_times` and `remap_times` need no key, as one correction applies to every object, while `set_times` is per-object literal values and is only keyless where the interface writes a single series. [PR #1922](https://github.com/catalystneuro/neuroconv/pull/1922)

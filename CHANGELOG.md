...
## Features
* `ExternalVideoInterface` now holds the `alignment` surface, one object per video file keyed by the file stem, and the surface gained `alignment[key].start_at(starting_time)`, which places one object by its first sample without building its times. The `set_aligned_*` methods and `get_timestamps` still work under a `FutureWarning` and will be removed in v0.12.0. [PR #2037](https://github.com/catalystneuro/neuroconv/pull/2037)
...
## Improvements
* Added `docs/how_to/align_external_video.rst`, the how-to for aligning behavior video to another modality. [PR #2037](https://github.com/catalystneuro/neuroconv/pull/2037)

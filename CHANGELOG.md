...
## Bug Fixes
* `SLEAPInterface` now writes the corrections a person made when proofreading a `.slp`, which were silently dropped. The frames the timestamps were drawn from were selected from every instance in the file while the rows were built from the network's instances alone, so a frame a person had labeled where the network found nothing contributed a time and no row: the samples after it were written against earlier frames' times, or the write failed on a length mismatch, depending on whether the labeled frames happened to be evenly spaced. Both now come from one selection, and where a human instance and a predicted one sit on the same track the human's is written, since that is what correcting the network means. A human-placed point has no score, so it is written with a confidence of `1.0` and a point the annotator marked not visible with `NaN`, with `confidence_definition` saying so. A file the network alone wrote is unaffected and says nothing about confidence. [PR #1990](https://github.com/catalystneuro/neuroconv/pull/1990)

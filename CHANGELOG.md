...
## Bug Fixes
* Fixed `RuntimeWarning: divide by zero` in `calculate_regular_series_rate` when the first two timestamps are identical (zero time-step). The function now returns `None` early in this case instead of performing an invalid division. [PR #1700](https://github.com/catalystneuro/neuroconv/pull/1700)

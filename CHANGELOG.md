...
* A `timestamps` dataset is now written with the shuffle filter ahead of its compression method. Timestamps are close to a monotonic ramp, so shuffle turns their near-constant high bytes into long runs the compressor collapses: on two million irregular float64 timestamps that is 11.75 MB against 7.51 MB on HDF5 and 11.62 MB against 7.62 MB on Zarr, at the same chunk shape and compression level. The filter is lossless and needs no plugin, and `data` is untouched. [PR #1983](https://github.com/catalystneuro/neuroconv/pull/1983)
...
## Bug Fixes
* Fixed the Zarr shuffle codec being instantiated with `numcodecs`' default `elementsize` of 4 regardless of the dataset's dtype, which on float64 grouped the wrong bytes and recovered almost nothing. It is now taken from the dtype unless the caller states one. [PR #1983](https://github.com/catalystneuro/neuroconv/pull/1983)

...
## Features
* `ExternalVideoInterface` now carries the `alignment` surface the fiber photometry and events interfaces hold, and each of its video files is separately addressable under the stem of its path, since a camera triggered per trial places each file independently. `alignment.shift_times(delta)` moves the camera as a whole, `alignment[stem].set_times(times)` gives one file the times a per-frame pulse train measured, and `alignment[stem].is_fine_aligned` reports whether one has been given times of its own, which a rigid shift is not, since a shift is gross alignment and moves them all. The four `set_aligned_*` methods and `get_timestamps` still work under a `FutureWarning` and will be removed in v0.12.0; `set_aligned_segment_starting_times` is the one whose replacement is not a rename, because the new setter is absolute and so does not shift twice when called twice. Several files with no times of their own are still written as one recording split in place, unchanged, but now under a warning naming them, since a recorder rotating its output every few minutes and a camera triggered per trial produce the same list of files and nothing in them tells the two apart. Giving each file its times silences it; a rigid shift does not, since it moves the whole interface and says nothing about any one file. Where the files really are one recording split in place, their starting times are the cumulative durations, `get_frame_counts()` over the new `get_header_frame_rates()`. Files that overlap once set raise instead of writing an `ImageSeries` whose timeline runs backwards. `alignment[key].is_set` and `alignment.is_fully_set` report whether an object's times were given rather than taken from its source, which is what the warning is decided on. [PR #1713](https://github.com/catalystneuro/neuroconv/pull/1713)
* Added `docs/how_to/align_external_video.rst`, walking from a digital line to an aligned video across the four timing shapes a camera produces: one file or one per trial, crossed with a trigger at each onset or a pulse per frame.
* Added `MockExternalVideoInterface`, which writes the `ImageSeries` an `ExternalVideoInterface` writes without a video file on disk. The frame count and the timestamps are constructor arguments rather than reads, so a test or a documentation example can compose a video of any length into a conversion without encoding one; the paths it writes into `external_file` deliberately do not resolve, so the file it produces cannot be mistaken for a publishable one. The frame count the write path needs now comes from `get_frame_counts` on `ExternalVideoInterface`, which is public because a conversion measuring its frame times from a pulse train wants to check the pulse count against it, and is the seam the mock overrides.

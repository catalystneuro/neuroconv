...
## Improvements
* CI now runs pytest with `-n logical` instead of `-n auto`, and only measures coverage on the single job that uploads it. pytest-xdist resolves `auto` to physical cores when psutil is installed, which is two on a four-vCPU GitHub runner, so the Linux and Windows jobs were running half the workers they had cores for; the macOS runners already reported their full count and are unaffected. Coverage instrumentation ran on all eight matrix jobs even though the Codecov upload has always been gated to ubuntu on Python 3.10. [PR #1861](https://github.com/catalystneuro/neuroconv/pull/1861)

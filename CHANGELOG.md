...
## Features
* Added `NPMFiberPhotometryInterface` for converting raw Neurophotometrics (NPM) fiber photometry data. The interface demultiplexes the interleaved channels of a raw NPM CSV in memory (by the `Flags`/`LedState` column for newer files, or row-cycling for older files) into per-channel `FiberPhotometryResponseSeries` via `ndx-fiber-photometry`. [PR #1756](https://github.com/catalystneuro/neuroconv/pull/1756)

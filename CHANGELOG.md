...
## Features
* `EDFRecordingInterface` now takes a `stream_name`, and `EDFRecordingInterface.get_stream_names` lists the ones a file offers. EDF groups its channels into one stream per sampling rate, so a recording that samples some of its signals at a different rate than the rest carries more than one, and until now those files could not be opened at all: the extractor raised asking for an argument the interface had no way to accept. `get_available_channel_ids` takes the stream name as well, and `channels_to_skip` applies within the selected stream. [PR #1892](https://github.com/catalystneuro/neuroconv/pull/1892)

...
## Features
* Added `NPMFiberPhotometryInterface` and `NPMLegacyFiberPhotometryInterface` for converting raw Neurophotometrics (NPM) fiber photometry data. Both demultiplex the interleaved channels of a raw NPM CSV in memory into per-channel `FiberPhotometryResponseSeries` via `ndx-fiber-photometry`: the modern interface reads header-bearing files (positively identified by their `Flags`/`LedState` column), and the legacy interface reads the older header-less, row-cycling format. [PR #1756](https://github.com/catalystneuro/neuroconv/pull/1756)

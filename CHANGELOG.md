...
## Bug Fixes
* Fixed `InscopixImagingInterface` falsely rejecting single-plane recordings as multiplane. The previous byte-grep check fired on any file whose JSON metadata footer contained a `"multiplane"` configuration block, which every modern Inscopix file carries regardless of plane count. Detection now reads per-frame `efocus` values via the `isx_movie_get_frame_header` C entry point and looks for a cycle, falling back to the `microscope.multiplane.enabled` JSON flag when per-frame headers are unavailable. [PR #1735](https://github.com/catalystneuro/neuroconv/pull/1735)
...
* Added `metadata_key` support to imaging interfaces (`BaseImagingExtractorInterface`, `TiffImagingInterface`, `Hdf5ImagingInterface`, `SbxImagingInterface`, `MicroManagerTiffImagingInterface`, `InscopixImagingInterface`, `ThorImagingInterface`, `FemtonicsImagingInterface`, `ScanImageImagingInterface`, `ScanImageLegacyImagingInterface`) and `use_new_metadata_format` parameter to `get_metadata()` for dict-based metadata pipeline. [PR #1694](https://github.com/catalystneuro/neuroconv/pull/1694), [PR #1702](https://github.com/catalystneuro/neuroconv/pull/1702), [PR #1703](https://github.com/catalystneuro/neuroconv/pull/1703), [PR #1704](https://github.com/catalystneuro/neuroconv/pull/1704), [PR #1705](https://github.com/catalystneuro/neuroconv/pull/1705), [PR #1721](https://github.com/catalystneuro/neuroconv/pull/1721), [PR #1723](https://github.com/catalystneuro/neuroconv/pull/1723)

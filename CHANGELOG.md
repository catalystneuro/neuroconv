...
## Features
* Added `BaseMNEContinuousDataInterface`, a base for interfaces that read continuous data through MNE-Python, covering the EEG/MEG format family (EEGLAB, BrainVision, EGI, FIF) that neither SpikeInterface nor neo reads. A subclass builds an `mne.io.BaseRaw` in `_read_raw()` and one interface writes one MNE channel type, so `BaseMNEElectricalSeriesInterface` writes the neural electrode kinds as an `ElectricalSeries`, `BaseMNETimeSeriesInterface` writes any other kind as a `TimeSeries`, and composing them is how a whole file is written. Data streams through the new `MNERawDataChunkIterator`, `mne` is an optional extra, and a `Raw` whose channels did not share a native sampling rate is refused instead of written with the samples MNE interpolates. [PR #1898](https://github.com/catalystneuro/neuroconv/pull/1898)

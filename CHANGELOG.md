...
## Features
* `KiloSortSortingInterface` now writes the templates in `templates.npy` as the `waveform_mean` column of the units table. Each template is unwhitened with `whitening_mat_inv.npy` and then zeroed again outside the channels Kilosort fit it on, because unwhitening with the dense inverse fills every channel with the inverse-whitening of a zero, which states the noise covariance rather than the cell. Kilosort records no scaling of its own and the schema fixes `waveform_mean` to volts, so the interface must be constructed with `gain_to_uV`; without it the waveforms are skipped with a warning. The layout follows the linkage: on its own the interface writes no electrodes table and no `electrodes` column, since a phy folder states which channels a unit came from only as indices into the recording Kilosort was run on, so the channel axis spans every sorted channel in `channel_map.npy` order and means the same thing for every unit. What the interface writes for a folder it converts on its own is chosen with `waveform_representation`, one of `"none"`, `"dense"` (the default, described above) and `"sparse_with_electrodes_table"`, which writes an electrodes table from `channel_positions.npy` and `channel_shanks.npy` and narrows each unit's waveform to the electrodes its template was fit on. That last one is not the default because a phy folder states no acquisition device, so the Device and the ElectrodeGroup have to be invented, and `"none"` is how a conversion says spike times only rather than saying it by withholding the gain. `KiloSortSortingInterface.get_unit_ids_to_channel_ids` derives the mapping `SortedRecordingConverter` needs straight from the templates, checking the recording against the channel count and sampling rate in `params.py`; when the electrodes come from a converter that way the layout follows them, and `waveform_representation` only decides whether the waveforms are written at all. [PR #1871](https://github.com/catalystneuro/neuroconv/pull/1871)

...
* Added HERD general-anatomy annotation. `ndx-pose` `Skeleton` node names (pose-estimation keypoints, e.g. `"Snout"`, `"Shoulder"`) are annotated with machine-readable UBERON references stored in-file under `/general/external_resources`, for a small curated set of skeleton parts and muscles (`ANATOMY_TERMS`). [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)
* Added HERD strain annotation. `Subject.strain` is annotated with a machine-readable RRID (Research Resource Identifier) reference stored in-file under `/general/external_resources`. [PR #1884](https://github.com/catalystneuro/neuroconv/pull/1884)

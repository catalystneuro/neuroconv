...
## Bug Fixes
* Fixed the device registry silently merging two metadata keys that resolve to the same NWB `name`. The writers are idempotent on the name, so the second key's fields were dropped without warning; metadata validation now raises instead. Two interfaces that genuinely share a device should reference one metadata key rather than declaring it twice. [PR #1829](https://github.com/catalystneuro/neuroconv/pull/1829)

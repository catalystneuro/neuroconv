...
## Features
* Added `BORISEventsInterface`, which converts one observation of a BORIS project. A behavior's point or state kind is declared in the coding scheme rather than marked on the rows, so state bouts are paired on subject plus code and written as one row carrying the bout's length, and the whole scheme reaches the file including behaviors nothing was scored against. Writes an `ndx-ethogram` `Ethogram` catalogue and an `EthogramBouts` table beside the events where that extension is installed. [PR #1996](https://github.com/catalystneuro/neuroconv/pull/1996)

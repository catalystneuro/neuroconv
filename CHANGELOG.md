...
## Features
* `AxonIntracellularInterface` and `BrukerVoltageRecordingInterface` now support gross temporal alignment via `interface.alignment.shift_times(delta)`, which moves everything the interface writes by `delta` seconds to place it on the shared session clock. Both now inherit a new `BaseIntracellularInterface` that owns the offset, and `AxonIntracellularConverter` and `BrukerVoltageRecordingConverter` place their files and electrodes on one timeline through that same method instead of assigning a private attribute on each interface, so a shift applied afterwards moves the placed set as a block. [PR #1921](https://github.com/catalystneuro/neuroconv/pull/1921)

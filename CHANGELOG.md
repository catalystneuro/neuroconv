...
## Bug Fixes
* Fixed `configure_and_write_nwbfile` failing with `ValueError: Could not find location '...' in builder` on an NWB file read from disk whose extension the reading process never imported. The builder used to size each dataset was built by the global manager from `pynwb.get_manager()`, which only knows the extensions this process imported, so a container from an unimported extension was built without any of its datasets. A read file is now built by the manager that read it, whose type map carries the namespaces cached in the file. [PR #1904](https://github.com/catalystneuro/neuroconv/pull/1904)

...
## Improvements
* Each interface now writes two NWB files in its test class instead of six, and both of them are read back and checked. `run_conversion(backend=...)` resolves the same default backend configuration internally that `run_conversion(backend_configuration=...)` receives, so the two spellings were writing byte-identical files per interface; that equivalence is now asserted once on a mock interface, and the per-interface tests keep only the hdf5 and zarr writes that `check_read_nwb` validates. The zarr file used to be written twice and never checked against the source data, so this raises per-format coverage while cutting test time. Every read-mode `NWBHDF5IO`/`NWBZarrIO` context manager in the test suite is now `pynwb.read_nwb` with an explicit `nwbfile.read_io.close()`, which is what lets one `check_read_nwb` body serve both backends. [PR #1862](https://github.com/catalystneuro/neuroconv/pull/1862)

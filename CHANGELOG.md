...
* `calculate_regular_series_rate` now accepts a series only if replacing it with `starting_time` and `rate` displaces no sample by more than a microsecond, instead of testing whether the consecutive differences all round to the same microsecond, which let a slowly drifting series through; the rate it returns is measured across the whole series rather than off the first difference, so it also changes slightly on series that were already accepted. The `tolerance_decimals` parameter is removed with no deprecation path. [PR #1925](https://github.com/catalystneuro/neuroconv/pull/1925)

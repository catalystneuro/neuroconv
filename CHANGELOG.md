...
* Added `DANNCEConverter`, which combines `DANNCEInterface` with one `ExternalVideoInterface` per camera (keyed by camera name via a `video_file_paths` dict). It writes each camera's video first and then links it as that camera's `source_video` in the DANNCE `MultiCameraPoseEstimation` container. [PR #1798](https://github.com/catalystneuro/neuroconv/pull/1798)

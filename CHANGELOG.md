...
* Added `BaseEventsInterface`, a shared base for events interfaces that writes native `pynwb` 4.0 `EventsTable` objects into `nwbfile.events`. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)
* Added `MockEventsInterface` for exercising the events writer across payload shapes (timestamp-only, single categorical value, multi-field struct) without acquisition data. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

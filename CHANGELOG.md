...
* Added ontology annotation tools in `neuroconv.tools.ontology`, split into two steps. The `infer_species_ontology_metadata` / `infer_strain_ontology_metadata` / `infer_brain_region_ontology_metadata` functions resolve a subject's species and strain and the file's anatomical `location` fields (the electrodes table `location` column, `ElectrodeGroup.location`, `ImagingPlane.location`, and the `FiberPhotometryTable` `location` column) to ontology terms and write them into the metadata next to the value they describe (`metadata["Subject"]["ontology"]["species"]`/`["strain"]`, `metadata["<modality>"]["ontology"]["brain_regions"]`). The `add_species_external_resource` / `add_strain_external_resource` / `add_brain_region_external_resources` functions then write those stated terms into the file as machine-readable HERD references under `/general/external_resources` (species -> NCBITaxon, strain -> RRID, brain regions -> the Allen Mouse/Human Brain Atlas or UBERON); a conversion runs them automatically, so they are a no-op unless the metadata carries an `ontology` block. [PR #1776](https://github.com/catalystneuro/neuroconv/pull/1776) [PR #1884](https://github.com/catalystneuro/neuroconv/pull/1884)

...
* `SLEAPInterface` writes the container itself through the shared pose writer instead of handing the file to `sleap_io.io.nwb_predictions.append_nwb_data`, so it lands in the `behavior` processing module like every other pose interface rather than in a per-video `SLEAP_VIDEO_000_*` one, and it emits pose metadata where it emitted none. A multi-animal `.slp` now takes one interface per track, named with the new `track_name` argument (`get_available_tracks` lists them), since an NWB file holds one subject. [PR #1952](https://github.com/catalystneuro/neuroconv/pull/1952)

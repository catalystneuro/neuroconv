...
* When a recording carries a timestamps vector that is regular, the three `spikeinterface` writers now write the rate measured from those timestamps instead of `recording.get_sampling_frequency()`, matching what the ophys writers already did. A recording whose timestamps disagree with its nominal rate, which is what an alignment or a calibrated clock produces, no longer has that disagreement discarded. [PR #1926](https://github.com/catalystneuro/neuroconv/pull/1926)

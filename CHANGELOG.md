...
* Fixed `EDFRecordingInterface.get_metadata` silently discarding all subject metadata. The metadata dict is a `DeepDict` (a `defaultdict`), and `dict.get` does not go through `__missing__`, so `metadata.get("Subject", dict()).update(...)` updated a throwaway dict and the `subject_id` and `date_of_birth` read from the EDF header never reached the NWB file. The subject metadata is now assigned rather than updated in place, and the schema-required `species`/`sex` fields are defaulted so that reading a patient code out of a file cannot turn valid metadata into invalid metadata. The EDF header's birthdate is also parsed into a `datetime` (and dropped when unparseable) rather than being forwarded to pynwb as a string, which `Subject.date_of_birth` does not accept.

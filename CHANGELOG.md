...
* Migrated `DoricFiberPhotometryInterface` to a single-series interface built on `BaseFiberPhotometryInterface`, selecting input streams via the new `stream_names` argument. Constructing without `stream_names` is deprecated and will be removed on or after January 2027.

...
## Features
* Added `BORISEventsInterface`, which converts one observation of a BORIS project. A behavior's point or state kind is declared in the coding scheme rather than marked on the rows, so state bouts are paired on subject plus code and written as one row carrying the bout's length, and the whole scheme reaches the file including behaviors nothing was scored against. Each modifier slot a behavior declares is written as its own column rather than as the `|`-joined string BORIS records. The scheme is written beside the events as an `ndx-ethogram` `Ethogram` catalogue, and the closed state bouts as an `EthogramBouts` table. [PR #1996](https://github.com/catalystneuro/neuroconv/pull/1996)

...
* The electrodes table can now be stated in metadata, over the table the recording derives. `metadata["Ecephys"]["ElectrodesTable"]` holds `rows`, one entry per electrode carrying its column values and pointing at its group with `electrode_group_metadata_key`, and `columns`, which can rename a column, describe it, pin its dtype and give an encoded column a vocabulary written as a `MeaningsTable`. A row wins over the recording for the fields it states and inherits the rest, so a dictionary annotating one column of one electrode says only that. The block takes the shape `FiberPhotometryTable` already uses for keyed rows and the one the events tables already use for column annotation, so a table stated in metadata reads the same across the three modalities. `get_metadata_template()` on the recording interfaces returns the whole thing prefilled from the recording, so a user edits rather than authors, and a channel reaches a row through `channel_to_electrode` on the series entry or through the same key derivation the template used. Two consequences are the point of it: an electrode's group is stated by its row, so a conversion no longer has to call `recording.set_property("group", ...)` before writing, which is what a YAML-driven specification could not do; and the electrode keys derive from physical contact identity, so the AP and LF bands of one probe land on one set of rows without the joined `AP0,LF0` channel name the SpikeGLX interface used to fabricate, which is removed here. `Ecephys.Electrodes` is untouched and keeps its own meaning as the list of column descriptions, so no existing metadata changes shape.

...
* Deprecated `es_key` parameter in `SpikeGLXNIDQInterface` and will be removed on or after June 2026. This parameter has no effect as the interface writes analog data as TimeSeries and digital data as LabeledEvents, not ElectricalSeries. [PR #1601](https://github.com/catalystneuro/neuroconv/pull/1601)
...
* Enhanced `SpikeGLXNIDQInterface` to support custom metadata for analog channels, enabling users to split NIDQ analog channels into multiple TimeSeries objects with custom groupings via the new `analog_channel_groups` parameter. Users can now organize analog channels semantically (e.g., audio, accelerometer, temperature sensors). The default behavior (single TimeSeries for all analog channels) remains unchanged. [PR #1601](https://github.com/catalystneuro/neuroconv/pull/1601)

...
* Added the shared machinery for the signal-encoded events interfaces, which derive events from a sampled signal, together with `MockSignalEncodedEventsInterface` in `neuroconv.tools.testing` to exercise it. `DoricEventsInterface` and `DoricCSVEventsInterface` are the first users. [PR #1825](https://github.com/catalystneuro/neuroconv/pull/1825)

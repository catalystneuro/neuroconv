...
## Features
* Added `BaseMNEContinuousDataInterface`, a base for interfaces that read continuous data through MNE-Python, covering the EEG/MEG format family (EEGLAB, BrainVision, EGI, FIF) that neither SpikeInterface nor neo reads. A subclass builds an `mne.io.BaseRaw` in `_read_raw()`, and one interface writes one MNE channel type to one neurodata object: `BaseMNEElectricalSeriesInterface` writes the neural electrode kinds as an `ElectricalSeries` backed by an electrodes table carrying `channel_name`, `group` and `location`, while `BaseMNETimeSeriesInterface` writes any other kind as a `TimeSeries` with the unit MNE assigned that type. A `Raw` holds several kinds on one shared timebase, so composing interfaces is how a whole file is written, and `get_channel_types()` reports which ones a file needs. The destination is the class rather than a routing table inside a shared writer, which keeps `eog`/`ecg`/`emg` (electrode voltages, but of eye, heart and muscle), `stim` lines (not measurements at all) and `mag`/`grad` (teslas and teslas per meter) out of the neural series without the base deciding on the user's behalf. Scoping is by channel type and never by unit, because MNE labels trigger lines volts just as it labels EEG channels volts. Sampling rate is the other scoping axis: MNE reports one `sfreq` per `Raw` and resamples slower channels up to the fastest, so a source that stores channels at several rates is refused with an error naming the rates it found, rather than written with interpolated samples that were never recorded. Such a format is covered by one interface per rate, using the `exclude` argument its MNE reader offers. Data streams through the new `MNERawDataChunkIterator`, so a `Raw` opened with `preload=False` is written without being held in memory, and a preloaded one (which some formats force when the samples sit inline in the container) goes through the same path from memory. MNE applies the calibration on read and always returns float64, so this writes the `physical_units` representation with `conversion=1.0`. This first version writes no electrode coordinates and is not temporally alignable. `mne` is an optional extra, and the bases themselves import it nowhere. [PR #1898](https://github.com/catalystneuro/neuroconv/pull/1898)

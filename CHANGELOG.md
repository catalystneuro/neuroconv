...
* Added `metadata_key` parameter to `BaseRecordingExtractorInterface` (inherited by `BaseLFPExtractorInterface` and all recording interfaces) and a `use_new_metadata_format` flag on `get_metadata()` for the dict-based metadata pipeline. When enabled, `get_metadata()` emits the dict-based `Ecephys.ElectricalSeries` keyed by `metadata_key`; the default device and electrode groups are left to the pipeline (a default device, and one group per channel-group synthesized from the recording's `group` properties), mirroring how the ophys interfaces leave Device/ImagingPlane defaults to the placeholder factory. The default remains the old list-based format; `metadata_key` defaults to the value of `es_key`. [PR #1747](https://github.com/catalystneuro/neuroconv/pull/1747)

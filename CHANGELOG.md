...
## Bug Fixes
* Fixed adding units without electrode indices to a units table that already has an `electrodes` column, which failed with `TypeError: 'NoneType' object is not subscriptable` from hdmf. This happens with multiple probes when the electrodes of a later one cannot be resolved; those units are now written with an empty electrode region. [PR #1832](https://github.com/catalystneuro/neuroconv/pull/1832)

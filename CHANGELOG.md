...
## Features
* Every interface now answers `get_metadata_template()`, the counterpart to `get_metadata()`: it returns the same source-derived values wrapped in the full structure the writer expects, with the `*_metadata_key` cross-references already resolved and every field only the experimenter can supply set to `None`. `get_metadata()` reports only what the source recorded, which is faithful but leaves a user no indication of what else the file needs, and reconstructing the nesting and the cross-references from the JSON schema by hand is real work. The blanks are the checklist: what comes back `None` is exactly what the source could not tell us. `BaseFiberPhotometryInterface` overrides it with the whole provenance chain, sized to the traces the interface writes at one `FiberPhotometryTable` row per trace; an interface with no chain to scaffold returns its metadata unchanged. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)
* A `FiberPhotometryTable` row's `notes` now reaches the file. The column has always been in ndx-fiber-photometry and the writer never passed it through, so anything stated there was silently dropped. In the same pass, a row's optional `*_metadata_key` set to `None` counts as unfilled rather than as a reference to a device named `None`, which is what lets a template be written after only its required blanks are filled. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

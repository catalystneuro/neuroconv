...
* Added `BaseFiberPhotometryInterface`, a single-stream base class that owns the shared NWB-writing and metadata logic: idempotent device/indicator/table construction, name-based `FiberPhotometryTable` region resolution, a standard scalar temporal-alignment API, and a runnable default metadata scaffold that warns about unset placeholder fields. Child interfaces implement only a small format-reading seam. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

## Removals, Deprecations and Changes
* Bumped minimum `pynwb` version to `>=4.0.0`. [PR #1769](https://github.com/catalystneuro/neuroconv/pull/1769)
* Migrated `TDTFiberPhotometryInterface` to a single-stream interface built on `BaseFiberPhotometryInterface`: each interface now writes one `FiberPhotometryResponseSeries` selected via the new `stream_name` constructor argument (with `metadata_key` for combining several interfaces over one shared `FiberPhotometryTable`), and `get_available_streams()` lists a tank's stores. Constructing without `stream_name` routes to the previous multi-stream behavior via a deprecating router, emits a `DeprecationWarning`, and will be removed on or after August 2026. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

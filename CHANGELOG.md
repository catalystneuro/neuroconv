...
## Features
* Added `neuroconv.tools.fiber_photometry_hardware_catalogue`, a reference catalogue of published fiber photometry hardware specifications, with one function per model type (`get_reference_optical_fiber_model`, `get_reference_excitation_source_model`, `get_reference_photodetector_model`, `get_reference_band_optical_filter_model`, `get_reference_edge_optical_filter_model`, `get_reference_dichroic_mirror_model`) to look a part up and return it as a ready `metadata["DeviceModels"]` entry. No fiber photometry format records the numerical aperture of a fiber, the wavelength range of an excitation source or the gain of a photodetector: a Doric file names the console rather than the modules plugged into it, and TDT, Neurophotometrics and comma-separated-value exports carry nothing at all, so those fields could previously only be filled by inventing them. The vendors do publish them, so the catalogue ships 68 parts from Doric Lenses, Thorlabs, RWD Life Science, Tucker-Davis Technologies, Newport, Semrock and Neurophotometrics, covering all six `ndx-ophys-devices` model classes, each carrying the page its values came from, addressed by model type, manufacturer and part number, or by product name where the vendor publishes no number. All three are needed because nothing makes a part number globally unique (short ones like `2151` or `PS1` are exactly the kind that collide) and one product commonly yields several models: a Doric fluorescence mini-cube is filters and dichroic mirrors at once, and a Neurophotometrics FP3002 is excitation sources and a detector. A test constructs every row as its real `ndx-ophys-devices` class, which checks the data against the extension version this repository pins rather than against one recorded elsewhere. `list_reference_device_models()` returns the rows for browsing, the user-guide page is generated from the same file so the table and the fill cannot disagree, and the fiber photometry gallery entries now fill their device models through it. What a vendor does not publish is stated rather than guessed: the Neurophotometrics camera's sensor identity and gain are left unset, and its patch cords are listed as unrepresentable because their numerical aperture is published as a range where the NWB model requires a single number. [PR #1899](https://github.com/catalystneuro/neuroconv/pull/1899)

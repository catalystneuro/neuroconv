...
## Features
* Added `set_segment_starting_times` method to `ExternalVideoInterface` for constructing timestamps from per-segment starting times. The frame rate is read from video metadata by default, or can be overridden explicitly. Also auto-computes `starting_frames` from video metadata when not explicitly provided. [PR #1713](https://github.com/catalystneuro/neuroconv/pull/1713)

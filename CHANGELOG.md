...
## Features
* Added `TDTEventsInterface` for converting discrete events (epocs) from a TDT tank folder. Each selected epoc is written as an `ndx_events.Events` object (onset timestamps) in a `behavior` ProcessingModule, with the epocs to store selectable via the `event_names` parameter. The shared TDT-tank reading logic (`load`, `get_events`) is factored into a `TDTLoadMixin` reused by `TDTFiberPhotometryInterface`. The interface is also bundled into `TDTFiberPhotometryGuppyConverter`, which now captures all three parts of a GuPPy session (raw acquisition, raw events, and GuPPy-derived outputs). The converter restricts the raw events to the behavioral event stores GuPPy listed in `storesList.csv` (exposed by `GuppyInterface.event_store_to_event_name`) and names each with the human-readable label from that file (e.g. the `PrtN` store becomes the `port_entries` Events object).

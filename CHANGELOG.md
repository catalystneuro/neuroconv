...
## Bug Fixes
* Fixed `VameInterface.get_metadata_schema` closing its `Behavior` node with `additionalProperties: False`, which rejected sibling registries written by other interfaces (e.g. a pose interface's `Behavior/Pose`) when VAME was combined in a converter. The node is now left open while VAME's own `Vame` and `Ethograms` sub-schemas stay strictly validated. [PR #1791](https://github.com/catalystneuro/neuroconv/pull/1791)
...
* Fixed `BrukerTiffImagingInterface` writing the z-plane *spacing* (the swing between the first and last plane) into the volumetric `ImagingPlane.origin_coords` where a position is expected. `origin_coords` z is now the first plane's focal depth (read from the active `zDevice`), while the inter-plane step and total depth extent go to `grid_spacing` and `field_of_view`. [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)
...
* `VameInterface` now also writes, for each motif run, a curated `ndx-ethogram` product alongside the faithful `ndx-vame` series: the per-frame `MotifSeries` is run-length-encoded into an `EthogramBouts` timeline (one row per bout) with an `Ethogram` catalogue listing one row per motif id, the modal community per motif recorded in the catalogue's `category` column. The bouts link back to the producing `MotifSeries` (`source`), the upstream `PoseEstimation` (`source_pose`), and, when a project sets `video_metadata_key`, the video `ImageSeries` (`source_video`). Requires the `ndx-ethogram` extension. [PR #1770](https://github.com/catalystneuro/neuroconv/pull/1770)
...
* Added `BrukerTiffImagingInterface`, a unified interface for Bruker Prairie View OME-TIFF imaging data backed by the new unified `BrukerTiffImagingExtractor` from roiextractors. Single-plane, volumetric, and multi-channel cases are handled through one class, with channels selected by name via `channel_name` (e.g. `"Ch1"`, `"Ch2"`). The interface emits metadata in the new dict-based format only (`Devices`, `Ophys.ImagingPlanes`, `Ophys.MicroscopySeries`). [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)

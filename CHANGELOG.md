...
* Added documentation for the new ecephys metadata structure: a how-to guide (`docs/how_to/annotate_ecephys_metadata.rst`) covering single probe, channels from multiple probes (e.g. Intan with several headstages), and multi-shank probes; and a developer guide describing the dict-based schema for `Devices`, `ElectrodeGroups`, and `ElectricalSeries` (`docs/developer_guide/ecephys_metadata_structure.rst`). The pipeline implementation will follow in subsequent PRs. [PR #1729](https://github.com/catalystneuro/neuroconv/pull/1729)

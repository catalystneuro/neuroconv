...
* Fixed the electrodes and units tables getting a different column order in every Python process, so the same recording or sorting wrote a structurally different NWB file on each run. `_add_electrodes_to_nwbfile` and `_add_units_table_to_nwbfile` build `data_to_add` as an insertion-ordered dict in a deliberate order, but then converted it to a `set` and iterated that set at the point where the columns are created. Set iteration order for strings follows their hashes, and Python randomizes string hashing per process, so six consecutive runs of a recording carrying eight custom properties produced six different electrode table layouts while the recording's own property order stayed constant. The sets are still used for membership tests, but the column-adding loops now iterate `data_to_add`, so custom properties become columns in the order the extractor reports them. The row-adding paths and the null-value lookups are ordered by the existing table's `colnames` for the same reason. Nothing is added to or dropped from either table, only the column order changes. [PR #1878](https://github.com/catalystneuro/neuroconv/pull/1878)

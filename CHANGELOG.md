...
## Features
* Added `CSVFiberPhotometryInterface` for converting raw fiber photometry recordings from a CSV file, parsed into the `ndx-fiber-photometry` format. It inherits from `BaseFiberPhotometryInterface` and is single-series: point it at one `file_path` and name the `timestamps_column` and `data_columns` (by header name or positional index), and it writes one `FiberPhotometryResponseSeries`. Install with `pip install "neuroconv[csv_fp]"`. [PR #1754](https://github.com/catalystneuro/neuroconv/pull/1754)

...
## Features
* Added `PyPhotometryFiberPhotometryInterface`, which converts a pyPhotometry `.ppd` recording. The format is binary and interleaves every signal the board recorded into one stream, and only the header's `mode` field says how to separate them, so the interface maps the four spellings that field has had onto one layout table and refuses a mode it does not know instead of falling back to the two-signal default, which reads a laboratory fork's recording as interleaved colours without raising. One interface reads one signal, since the board samples its analog inputs one after another: each signal is written with the start time its slot implies, where every existing reader reports them all as starting at zero. A recording written by header version 1.1 stores an LED-on sample and the LED-off baseline it is corrected against, and the conversion writes their difference plus both measurements, warning that no file of that version was available to test against. [PR #1905](https://github.com/catalystneuro/neuroconv/pull/1905)

...
* Added `IntanConverter` for one-call conversion of multi-stream Intan recordings. Parses the `.rhd`/`.rhs` file header via neo, auto-discovers which streams are present, and routes each to `IntanRecordingInterface`, `IntanAnalogInterface`, or `IntanStimInterface` as appropriate. Exposes a `streams` filter to opt out of specific streams and forwards `saved_files_are_split` to every sub-interface.

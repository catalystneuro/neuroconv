...
## Features
* Added `BaseMNEContinuousDataInterface`, a base for interfaces that read continuous data through MNE-Python, covering the EEG/MEG format family (EEGLAB, BrainVision, EGI, FIF) that neither SpikeInterface nor neo reads. A subclass builds an `mne.io.BaseRaw` in `_read_raw()` and the base writes it as an `ElectricalSeries` plus an electrodes table carrying `channel_name`, `group` and `location`. Data streams through the new `MNERawDataChunkIterator`, so a `Raw` opened with `preload=False` is written without being held in memory, and a preloaded one (which some formats force when the samples sit inline in the container) goes through the same path from memory. MNE applies the calibration on read and always returns float64, so this writes the `physical_units` representation with `conversion=1.0`. This first version handles only channels destined for an `ElectricalSeries`: it writes no electrode coordinates and does not yet partition a `Raw` by channel type, and it is not temporally alignable. `mne` is an optional extra, and the base itself imports it nowhere. [PR #1898](https://github.com/catalystneuro/neuroconv/pull/1898)

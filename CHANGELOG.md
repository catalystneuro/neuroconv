...
* `SpikeGLXRecordingInterface` keys its probe device by the probe's serial number rather than by its position, so `metadata["Devices"]["neuropixels_imec0"]` is now `metadata["Devices"]["neuropixels_18194809281"]`. The key has to name the physical probe, since the AP and LF interfaces hold the same one and their metadata has to merge into a single entry, and the serial number is the only field that identifies a unit rather than a slot. A script editing the old key must be updated; there is no deprecation path, because the new key is derived from the data. The written device name is unchanged, but its description is: the JSON blob of probe annotations is gone, replaced by the `DeviceModel` link. `SpikeGLXSyncChannelInterface` stops emitting a device entirely, since it writes a plain `TimeSeries`, which has no device link, so the device it created was never referenced by anything, and the probe's provenance now comes from the recording interface. [PR #1894](https://github.com/catalystneuro/neuroconv/pull/1894)
...
## Features
* `SpikeGLXRecordingInterface` and `OpenEphysBinaryRecordingInterface` now write the attached Neuropixels probe's identity in a form the geometry can be rebuilt from: a `DeviceModel` carrying the catalogue `manufacturer` and `model_number` (`imec`, `NP1110`), and a `Device` carrying the unit's `serial_number` and linking to that model. `probeinterface.get_probe(manufacturer, model_number)` reconstructs the probe from those two strings alone. SpikeGLX previously flattened the same facts into a JSON blob appended to the device description, which nothing could read back; Open Ephys claimed no device at all and fell through to the placeholder. Streams that carry no probe are unaffected. [PR #1894](https://github.com/catalystneuro/neuroconv/pull/1894)
...
* Added a `data_representation` write option (`"digital_counts"`, the default, or `"physical_units"`) to `add_recording_to_nwbfile` and `BaseRecordingExtractorInterface.add_to_nwbfile`. `"physical_units"` folds each channel's gain and offset into float data, so channels with heterogeneous per-channel offsets can be written to a single `ElectricalSeries`. [PR #1814](https://github.com/catalystneuro/neuroconv/pull/1814)

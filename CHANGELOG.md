...
* `get_timestamps`, `set_aligned_timestamps`, `set_aligned_starting_time` and `align_by_interpolation` are deprecated on the fiber photometry interfaces and will be removed on or after August 2027. Each now has a successor under `interface.alignment` (`alignment[key].get_times`, `alignment[key].set_times`, `shift_times` and `remap_times`) and routes into it behind a `FutureWarning`, so the behaviour is unchanged and there is no longer a second alignment mechanism to reconcile with the first. [PR #1922](https://github.com/catalystneuro/neuroconv/pull/1922)
...
## Features
* `BaseFiberPhotometryInterface` now supports gross temporal alignment via `interface.alignment.shift_times(delta)`, which moves every time-bearing object the interface writes by `delta` seconds to place it on the shared session clock, so every interface built on it gains it at once. The shift is rigid and reaches both the `FiberPhotometryResponseSeries` and any `CommandedVoltageSeries` beside it, leaving the sampling rate and the source times untouched, though `get_timestamps()` reports the shifted values. [PR #1919](https://github.com/catalystneuro/neuroconv/pull/1919)
* `interface.alignment` on the fiber photometry interfaces is now the full alignment surface: it names the interface's time-bearing objects, so `alignment.keys()` enumerates them and `alignment[key]` reaches one, and it carries `set_times` for per-sample times you already trust and `remap_times(local_sync_times=, reference_sync_times=)` for re-expressing a drifting clock against a reference one through shared synchronization pulses. `shift_times` positions the whole interface and `remap_times` is one clock's correction, so neither takes a key, while literal times belong to one object and are given through it, `alignment[key].set_times(times)`. [PR #1922](https://github.com/catalystneuro/neuroconv/pull/1922)

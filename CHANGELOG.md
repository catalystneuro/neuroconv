...
## Features
* The imaging and segmentation interfaces gained `get_metadata_template()`, the counterpart to `get_metadata()` that fiber photometry already offered: it returns the same source-derived values wrapped in the full structure the writer expects, with the `device_metadata_key` and `imaging_plane_metadata_key` cross-references already resolved and every field only the experimenter can supply set to `None`. `get_metadata()` reports only what the source recorded, which is faithful but leaves a user no indication of what else the file needs. The blanks are the checklist: what comes back `None` is exactly what the source could not tell us, which for ophys is the brain region imaged, the indicator, the two wavelengths and the name every object needs. The segmentation template is sized to the pipeline's output, offering one entry per trace and per summary image the file actually holds, since naming one it does not hold writes nothing. The `Metadata Templates` page in the user guide carries both structures as a YAML or JSON file to fill in by hand, and `How to Annotate Optical Physiology Data` gained a section on filling one in. The ophys and fiber photometry conversion gallery pages now point at the template and the how-to rather than at the developer guide.
* Added `BaseEventsInterface.get_event_times(event_type_source_id)`, returning one event type's onset times so a line used as a clock (a camera's frame-out pulse, an alignment pulse) can be read back to align another stream against, plus `get_event_type_source_ids` listing the handles it takes. It answers on the interface's current clock, the source's own times plus `alignment.offset`, and returns the same array the writer writes.

...
## Features
* Added `TDTEventsInterface` for converting discrete events (epocs) from a TDT tank folder, depending on the `ndx-events` extension. Each selected epoc is written as an `ndx_events.Events` object (onset timestamps) under `metadata["Events"][metadata_key]`; epocs that carry real offset (STROFF) durations or meaningful strobe values are detected and raise `NotImplementedError` pointing to a feature request. [PR #1751](https://github.com/catalystneuro/neuroconv/pull/1751)

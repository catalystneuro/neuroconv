...
## Bug Fixes
* Stopped the metadata schemas requiring `subject_id`, `sex` and `species` whenever a `Subject` key is present. NWB requires none of the three (`pynwb.Subject()` constructs with no arguments at all), and the requirement was not self-consistent either: `base_metadata_schema.json` demanded all three while `metadata_schema.json`, which the YAML conversion route validates against, demanded only `subject_id`. Interfaces that read partial subject information out of a file had to invent the rest to pass validation, which is what `metadata_principles.rst` tells them not to do, so the Inscopix interfaces no longer fill `subject_id` with `"Unknown"` or `species` with `"Unknown species"`. A file that does not state a species now produces no species, and NWB Inspector reports the omission instead of a placeholder silently passing its check. `sex` still falls back to `"U"`, which is NWB's own term for unknown rather than an invented value. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

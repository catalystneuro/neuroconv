...
* Added `SpikeGLXSyncChannelInterface` for converting Neuropixel synchronization channels from SpikeGLX recordings. Sync channels contain a 16-bit status word where bit 6 carries a 1 Hz square wave used for sub-millisecond timing alignment across acquisition devices. The `SpikeGLXConverterPipe` now includes sync channels by default (one per probe, preferring AP over LF), which can be disabled with `include_sync_channels=False`. [PR #1600](https://github.com/catalystneuro/neuroconv/pull/1600)

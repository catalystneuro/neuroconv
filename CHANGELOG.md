...
## Bug Fixes
* Fixed `ImageInterface` writing all images as `float64` regardless of source dtype. `SingleImageIterator.dtype` now reports the dtype derived from the PIL image mode (uint8 for `L`/`RGB`/`RGBA`/`LA`, uint16 for `I;16`), so on-disk datasets match the source. Also fixed `SingleImageIterator.maxshape` returning `(None, None)`, which broke the chunk-shape estimator for single images; it now returns the concrete image shape. [PR #1744](https://github.com/catalystneuro/neuroconv/pull/1744)

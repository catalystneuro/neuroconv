...
## Features
* Added `PyPhotometryFiberPhotometryInterface`, which converts a pyPhotometry `.ppd` recording. The format is binary and holds every signal the board recorded interleaved word by word, so the generic CSV path cannot reach it, and nothing in a file says how to separate the words except the header's `mode` field, whose spelling changed twice: the same acquisition is `GCaMP/RFP` under header version 0.1, `2 colour continuous` under 0.2 and 0.3, and `2EX_2EM_continuous` from 1.0 onward, and an even older generation predates the JSON header entirely. The interface maps all four onto one layout table and refuses a mode it does not know rather than falling back to the two-signal default the format documents, because a laboratory fork exists whose file is indistinguishable from an ordinary two-signal recording except by that string and which the default reads as interleaved colours without raising. One interface reads one signal, the way the Neurophotometrics one takes one excitation, because the board has no simultaneous analog-to-digital conversion: in the pulsed modes the sampling timer advances one analog input per tick, so each signal is written with the start time its slot implies rather than with the shared time vector every existing reader returns. Recordings made with header version 1.1 store an LED-on sample and the LED-off baseline it is corrected against; the response series carries their difference, which is what earlier firmware computed on the board, and both measurements are written beside it. No recording of that version exists in any public deposit, so that path is written from pyPhotometry's own reader source rather than from a file and warns when it reads one ([issue #1907](https://github.com/catalystneuro/neuroconv/issues/1907)). [PR #1905](https://github.com/catalystneuro/neuroconv/pull/1905)

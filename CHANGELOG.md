...
## Features
* `GuppyConverter` gained `acquisition_format="nwb"`, covering the fifth and last input format GuPPy reads: a session GuPPy processed out of an existing NWB file. Both raw folder paths point at the folder holding that one `.nwb` file, and the converter reads its `FiberPhotometryResponseSeries` columns and its events — core `EventsTable` objects as well as the ndx-events v0.2 types — into a new file alongside the GuPPy outputs. GuPPy derives its `storesList.csv` ids from the file's own contents (`<series>_<column>` for a multi-channel series, `<table>_<value>` for a merged events table), so the ids resolve against the source directly. The source also already states the fiber photometry provenance, so unlike the other four formats `get_metadata` returns the devices, indicators, `FiberPhotometryTable` and per-series regions read back out of it rather than requiring the user to restate them. Only the series and the events cross over; anything else the source file holds is not carried into the converted file. [PR #1880](https://github.com/catalystneuro/neuroconv/pull/1880)

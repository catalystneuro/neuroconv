...
## Bug Fixes
* Fixed `make_nwbfile_from_metadata` converting `metadata["Subject"]["date_of_birth"]` from its ISO 8601 string to a `datetime` inside the caller's own dictionary. The `NWBFile` block was deep-copied and the subject block was not, so a conversion stating a date of birth came back with the metadata it passed in rewritten.
...
## Features
* Added `add_subject_to_nwbfile`, which writes `metadata["Subject"]` onto an NWBFile that already exists. The subject was reachable only through `make_nwbfile_from_metadata`, so there was no way to add one to a file in hand and no single place that owned turning the metadata block into a `Subject`. Adding a subject to a file that already holds one raises, since an NWBFile describes one subject.

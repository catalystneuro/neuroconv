...
## Bug Fixes
* `DeepLabCutInterface.add_to_nwbfile` now raises a `UserWarning` when no timestamps have been set and the DataFrame integer index is used as a fallback, which incorrectly implies a 1 Hz sampling rate. The warning instructs users to call `set_aligned_timestamps` to provide correct timestamps. [PR #1728](https://github.com/catalystneuro/neuroconv/pull/1728)

...
* Enhanced `SpikeGLXNIDQInterface` to support custom metadata for analog channels, enabling users to split NIDQ analog channels into multiple TimeSeries objects with custom groupings. Users can now organize analog channels semantically (e.g., audio, accelerometer, temperature sensors) using a flexible dictionary structure. Maintains backward compatibility with the old single-TimeSeries format (deprecated, will be removed on or after May 2026). [PR #1601](https://github.com/catalystneuro/neuroconv/pull/1601)

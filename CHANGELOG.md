...
* Deprecated `compression_method` and `compression_options` on both dataset IO configuration models, to be removed in v0.12.0. Use `compressors` and `compressor_options` instead. Both spellings work until then, and specifying both raises. One behavior change comes with this: reading back an HDF5 dataset that was written without compression now leaves the options `None` instead of reporting `{'compression_opts': None}` for a compression method that is not there.
...
## Features
* `HDF5DatasetIOConfiguration` and `ZarrDatasetIOConfiguration` now describe a dataset's codec pipeline with an ordered `compressors` list and a matching `compressor_options`, replacing the single-valued `compression_method` and `compression_options`. This is the vocabulary Zarr v3 uses, and it makes the HDF5 `shuffle` and `fletcher32` filters reachable for the first time: they compose with a compression method rather than replacing one, so `compressors=["shuffle", "gzip"]` says what a single slot could not. HDF5 fixes where each filter sits in its pipeline, so the list is validated against that order rather than honoured as written.

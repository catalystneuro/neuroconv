...
* A `FiberPhotometryTable` row's `notes` now reaches the file. The column has always been in ndx-fiber-photometry and the writer never passed it through, so anything stated there was silently dropped. In the same pass, a row's optional `*_metadata_key` set to `None` counts as unfilled rather than as a reference to a device named `None`, which is what lets a template be written after only its required blanks are filled. [PR #1911](https://github.com/catalystneuro/neuroconv/pull/1911)

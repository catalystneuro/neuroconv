...
## Bug Fixes
* `SLEAPInterface.get_available_tracks` now offers only the tracks that carry an instance, and naming one the file declares but never populated says so instead of failing inside numpy with `ValueError: need at least one array to stack`. [PR #1993](https://github.com/catalystneuro/neuroconv/pull/1993)
* `SLEAPInterface` now warns naming the instances that carry no track and are therefore not written, since a point with no track belongs to no individual and one frame can hold several of them. [PR #1993](https://github.com/catalystneuro/neuroconv/pull/1993)
* `SLEAPInterface` now writes a file where no instance carries a track, a labeling project or an untracked single-animal recording, as one individual taking no `track_name`, matching what SLEAP's own analysis export does, and raises on a frame of such a file holding several untracked instances instead of writing them into one slot where the last one wins. [PR #1993](https://github.com/catalystneuro/neuroconv/pull/1993)

...
* Deprecated `MedPCInterface`, to be removed on or after August 2027. `MedPCEventsInterface` reads the same files and is the replacement; it writes their events as native `pynwb.event.EventsTable` objects into `nwbfile.events` rather than as `ndx-events` `Events` objects and `IntervalSeries` inside a `BehavioralEpochs` in the behavior processing module, so a conversion that moves over writes a differently shaped file on purpose. The rewrite is mechanical. Which arrays hold events is declared on the interface instead of in the metadata: `medpc_name_to_info_dict` is keyed by the MedPC variable itself (`"A"`, not `"left_nose_poke_times"`), so the mapping that had to be given twice, once to `__init__` as `metadata_medpc_name_to_info_dict` and again as `metadata["MedPC"]["medpc_name_to_info_dict"]`, is given once. An old `metadata["MedPC"]["IntervalSeries"]` entry becomes a `durations_name` on the onset array's entry (`"G": {"name": "port_entries", "durations_name": "E"}`), written as one durative event type carrying each event's duration rather than as the `+1`/`-1` transition points of an `IntervalSeries`, which also retires the `ValueError` a second interval entry used to raise by constructing `BehavioralEpochs` twice. An old `metadata["MedPC"]["Events"]` entry's `description` becomes `metadata["Events"]["medpc"]["event_types"][<variable>]["event_description"]`, and passing `metadata["MedPC"]` to the new interface raises rather than being silently ignored. The single-line variables `get_metadata` used to copy into `metadata["MedPC"]` are not read at all, since that put data in the metadata dictionary; read them with `medpc_helpers.get_medpc_variables` and write them where they belong. Two written results also change: an array holding no events becomes a zero-row table rather than being skipped, and a durative type whose durations array is shorter than its onsets, which is what a session cut short mid-event leaves behind, records the missing offsets as `NaN` rather than dropping those onsets. MedPC is the last interface writing `ndx-events`, so the dependency leaves NeuroConv when this interface is removed. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
...
* Passing metadata in the old list-based format now raises a `FutureWarning`, once per process, naming August 2027 as the earliest it is removed. The metadata is still converted on the way in, so the file written is unchanged. [PR #1935](https://github.com/catalystneuro/neuroconv/pull/1935)
* `get_metadata()` now returns the dict-based format instead of the old list-based one, as `use_new_metadata_format` defaults to `True` on the 47 public `get_metadata` and `get_metadata_schema` surfaces. `use_new_metadata_format=False` still returns the old shape. [PR #1934](https://github.com/catalystneuro/neuroconv/pull/1934)
* The fiber photometry installation extras are grouped as their own modality instead of as part of ophys, the way ecephys and ophys are already grouped and the way `src/neuroconv/datainterfaces/fiber_photometry/` already sits beside them. `csv_fp`, `doric_fp`, `npm_fp`, `tdt_fp` and `guppy` resolve through a new `fiber_photometry_minimal` extra holding `ndx-fiber-photometry`, and `neuroconv[fiber_photometry]`, which used to be that bare dependency, is now the aggregate over all five. Nothing breaks in this release, as `neuroconv[ophys]` still installs the photometry extras and `neuroconv[full]` now names fiber photometry directly. `neuroconv[ophys]` will stop installing them on or after August 2027, so a photometry conversion should ask for `neuroconv[fiber_photometry]`, or for the single format extra it uses, instead of relying on the ophys aggregate. [PR #1933](https://github.com/catalystneuro/neuroconv/pull/1933)
* Deprecated `AbfInterface`, and with it the `BaseIcephysInterface` it is the only subclass of, to be removed on or after August 2027. `AxonIntracellularInterface` reads the same Axon Binary Format files and is the replacement. `AbfInterface` writes a separate series per sweep, so a file of many sweeps becomes many objects in the file, and it infers the clamp mode from ABF metadata that is not reliable, falling back to izero when it finds no command data, so the series class can be wrong. The new interface takes the clamp mode explicitly and writes one continuous series per electrode, with each sweep addressed as a range in the intracellular recordings table. Where `AbfInterface` took a list of files, use one `AxonIntracellularInterface` per electrode per file and combine them with `AxonIntracellularConverter`. The removal will also drop the `ndx-dandi-icephys` dependency, of whose twenty-five fields NeuroConv only ever wrote four (`cell_id`, `slice_id`, `targeted_layer`, `inferred_layer`), none of them read from the file. [PR #1932](https://github.com/catalystneuro/neuroconv/pull/1932)
...
* Bumped minimum `hdmf` version to `>=6.1.0`. [PR #1924](https://github.com/catalystneuro/neuroconv/pull/1924)
* `run_conversion` no longer accepts a `backend_configuration` together with an in-memory `nwbfile` or with `append_on_disk_nwbfile=True`; both raise `ValueError` naming what to call instead. Both paths add the conversion's own data to the file after the point where a caller could have derived a configuration, so the configuration never describes the file being written, and since [PR #1910](https://github.com/catalystneuro/neuroconv/pull/1910) made a configuration have to describe that file, the combination could only fail at write time. To customize the configuration, build the file with `create_nwbfile`, derive the configuration from it, and write it with `configure_and_write_nwbfile`; to append with defaults, pass `backend` instead. `run_conversion(nwbfile_path=..., backend_configuration=...)` is unchanged. [PR #1910](https://github.com/catalystneuro/neuroconv/pull/1910)
* `SpikeGLXRecordingInterface` keys its probe device by the probe's serial number instead of its position, so a script editing `metadata["Devices"]["neuropixels_imec0"]` must now edit `metadata["Devices"]["neuropixels_18194809281"]`. There is no deprecation path, as the new key is derived from the data. The device name is unchanged and its description loses the JSON blob of probe annotations, replaced by the `DeviceModel` link. `SpikeGLXSyncChannelInterface` stops emitting a device, as it writes a plain `TimeSeries` which has no device link, so nothing ever referenced the one it created. [PR #1895](https://github.com/catalystneuro/neuroconv/pull/1895)
* `IntanRecordingInterface` names its electrode groups after the headstage port the channels were recorded on (`A`, `B`, `C`) instead of the port's position in a sorted list (`0`, `1`, `2`), and writes the port per channel as a `port` column on the electrodes table. Intan names amplifier channels `Port-Number` (`A-001`) and SpikeInterface recovers the letter into a `group_names` property that nothing downstream reads, since the write pipeline looks for `group_name`, so until now the port reached the file only as a stray `group_names` column sitting beside `group_name` and differing from it by one character. That column is gone, replaced by `port`. The port is kept as a channel property as well as a group name because attaching a probe regroups the channels and overwrites the group name, while the port stays true of every channel afterwards. A script addressing a group by name, `nwbfile.electrode_groups["0"]`, needs updating. [PR #1897](https://github.com/catalystneuro/neuroconv/pull/1897)
* A `device_metadata_key` or `device_model_metadata_key` that names nothing in its registry now raises one curated `ValueError` naming the field, the value written, the registry it resolves into, and the keys that registry actually holds. What a typo produced used to depend on which modality you were editing: ecephys, ophys and icephys raised a bare `KeyError: 'typo'`, the video interfaces raised a curated `KeyError` from their own hand-rolled check, and `device_model_metadata_key` raised a differently worded `ValueError`. The check now lives in `_add_device_to_nwbfile`, which every modality resolves through, so the two hand-rolled ones are gone. The only change for a caller is that the video interfaces raise `ValueError` where they raised `KeyError`. [PR #1893](https://github.com/catalystneuro/neuroconv/pull/1893)
* `AxonIntracellularInterface` reports a device only where the telegraph header names the amplifier. An ABF v1 file has no telegraph block, and a file whose instrument was set manually carries nothing that identifies one either, so those recordings may not be on Axon hardware at all and `get_metadata` no longer answers with a generic `AxonAmplifier` device on the file's behalf. They now write the icephys placeholder device, named `PlaceholderIntracellularDevice`, which `tools/icephys.py` supplies at write time the way the ecephys and ophys paths already did for their own modalities. The name states no instrument class on purpose: across published icephys files on DANDI that link holds a digitizer about half the time, an amplifier a fifth of the time, and a rig, the acquisition software or the pipette itself for the rest, so a default cannot pick one honestly. A file whose telegraph does name the model is unchanged. [PR #1893](https://github.com/catalystneuro/neuroconv/pull/1893)
* The device a write path invents when nothing describes the hardware is now built where the object that requires it is created, in the ecephys, ophys and icephys paths alike, instead of being injected into a rebuilt copy of the caller's `Devices` registry. It is also down to what NWB requires of a `Device`, which is a name and nothing else, so the ecephys one no longer carries `"description": "Ecephys probe. Automatically generated."`. The names are now `PlaceholderElectrodeDevice`, `PlaceholderMicroscope` and `PlaceholderIntracellularDevice`, replacing `Device` and `Microscope`. The prefix says the value was defaulted rather than stated, which the name has to carry because `Device` requires nothing else and NWB Inspector only inspects descriptions, and it stops the placeholder colliding with a name a user is likely to choose: `Microscope` is the commonest device name in published ophys files on DANDI, and `Amplifier` and `Device` are both in use as stated names. Two behaviors change with it. A `metadata["Devices"]["default_metadata_key"]` entry is no longer picked up by an electrode group or imaging plane that names no device of its own, so state that link with the entry's own `device_metadata_key` instead. And a device the user named `Device` or `Microscope` no longer raises a duplicate-name error against the placeholder it collided with. [PR #1893](https://github.com/catalystneuro/neuroconv/pull/1893)
...
* NeuroConv now writes the dict-based metadata format for itself. When no metadata is passed, `create_nwbfile`, `run_conversion` and `add_to_nwbfile` fill it with the dict-based shape on both interfaces and converters, and `NWBConverter.get_metadata` gained `use_new_metadata_format` so a converter mixing dict-only and dual-mode interfaces returns one consistent dictionary. `get_metadata()` still returns the old list-based format, so a script that reads it, edits it and passes it back is unaffected. Files written without explicit metadata change where the two paths disagree: `MaxOneRecordingInterface` writes a named `MaxOne` device instead of rewriting the placeholder device's description; `NeuroScopeRecordingInterface`, `AxonaRecordingInterface` and `MEArecRecordingInterface` stop writing invented filler (`location=""`, templated electrode-group descriptions, the MEArec device description); imaging interfaces that state no series name write `MicroscopySeries` rather than `TwoPhotonSeries` or `OnePhotonSeries`; `ThorImagingInterface`'s optical channel takes the placeholder description rather than an empty string; and `MicroManagerTiffImagingInterface`'s imaging plane carries no description, which NWB does not require and the source does not state. [PR #1853](https://github.com/catalystneuro/neuroconv/pull/1853)
* `SpikeGLXNIDQInterface` now addresses a channel by the board's own name (`"XA0"`, `"XD0"`), which is what `~snsChanMap`, the SpikeGLX user interface and CatGT all show. `get_channel_names()` returns those names rather than neo's stream-qualified ids, and `analog_channel_groups`, `detection_configuration` and `get_event_times_from_ttl` all take them. neo prefixes its channel ids with the stream (`"nidq#XA0"`) so they stay unique across a run's imec and NI streams, but inside this interface the prefix is a constant, since it only ever reads the nidq stream. The prefixed spelling is still accepted everywhere a channel is named, behind a `FutureWarning`, and will be removed on or after August 2027. [PR #1842](https://github.com/catalystneuro/neuroconv/pull/1842)
* Renamed the default `metadata_key` on the analog and sync interfaces to snake_case, so a key is a dict handle everywhere and CamelCase is reserved for the `name` of a neurodata type: `IntanStimInterface` `"TimeSeriesIntanStim"` -> `"intan_stim"`, `IntanAnalogInterface` `"TimeSeriesAnalogIntan"` -> `"intan_analog"`, `SpikeGLXSyncChannelInterface` `"SpikeGLXSync"` -> `"spikeglx_sync"`, `SpikeGLXNIDQInterface` `"SpikeGLXNIDQ"` -> `"spikeglx_nidq"`, `EDFAnalogInterface` `"analog_edf_metadata_key"` -> `"edf_analog"`. `IntanConverter`'s routing keys drop their object-type prefix (`"time_series_intan_dc"` -> `"intan_dc"`, and likewise for the auxiliary, ADC input, ADC output and stim streams), and `SpikeGLXConverterPipe` now keys each probe's sync interface as `f"spikeglx_sync_{probe}"` rather than `f"SpikeGLXSync{stream_id}"`. This is a breaking change with no deprecation path: a script that edits `metadata["TimeSeries"]["SpikeGLXNIDQ"]` must be updated to the new key. The written objects' names are unchanged. [PR #1870](https://github.com/catalystneuro/neuroconv/pull/1870)
...
* `DataInterfaceTestMixin` no longer provides `test_run_conversion_with_backend_configuration`; its single parametrized `test_all_conversion_checks` writes the interface out once per backend with `run_conversion(backend=...)` and reads each file back through `check_read_nwb`. `run_conversion` derives the default backend configuration itself, so passing one explicitly wrote the same file at the cost of an extra `create_nwbfile` per interface per backend, and that equivalence is asserted once on a mock interface instead. `test_all_conversion_checks` no longer builds an `NWBConverter` either: with a single interface that only exercised `dict_deep_update` against the base metadata schema, and the multi-interface behavior worth testing is already covered by the real converters in `tests/test_on_data`. [PR #1866](https://github.com/catalystneuro/neuroconv/pull/1866)
* `DataInterfaceTestMixin` no longer provides `test_run_conversion_with_backend`, `test_configure_backend_for_equivalent_nwbfiles`, or the `check_run_conversion_in_nwbconverter_*` helpers, and `check_read_nwb` is now called for the zarr file as well as the hdf5 one. A downstream `check_read_nwb` that opens the file with `NWBHDF5IO` instead of `pynwb.read_nwb` will fail on the zarr pass; set `check_read_nwb_backends = ("hdf5",)` on the test class to opt that backend out, as `ImagingExtractorInterfaceTestMixin` does for roiextractors' HDF5-only `NwbImagingExtractor`. `test_all_conversion_checks` keeps its name but no longer writes a file; it builds the interface through an `NWBConverter` and stops there. [PR #1862](https://github.com/catalystneuro/neuroconv/pull/1862)
* The `stimulus_type` column on the intracellular-recordings table is now optional. NWB requires a stimulus type on every `SequentialRecordings` entry, so `_build_icephys_hierarchical_tables` supplies `"not described"` there when the source states none, but an interface whose format carries no protocol section no longer writes a recordings column holding nothing but that placeholder on every row. `MockIcephysInterface` accepts `stimulus_type=None` to write files the same way. `AxonIntracellularInterface` is unchanged. [PR #1852](https://github.com/catalystneuro/neuroconv/pull/1852)
...
* Added `*args` positional argument deprecation to `add_imaging_to_nwbfile` to enforce keyword-only arguments. Will be enforced on or after February 2027. [PR #1680](https://github.com/catalystneuro/neuroconv/pull/1680)
* Added `*args` positional argument deprecation to `add_segmentation_to_nwbfile` to enforce keyword-only arguments. Will be enforced on or after February 2027. [PR #1687](https://github.com/catalystneuro/neuroconv/pull/1687)
...
* Deprecated `add_electrodes_to_nwbfile`; the implementation is now private (`_add_electrodes_to_nwbfile`). Calling it standalone can produce missing or spurious device/electrode-group metadata, so the public wrapper emits a `FutureWarning`. Use `add_recording_metadata_to_nwbfile` to add devices, electrode groups, and electrodes together. Will be removed on or after February 2027. [PR #1719](https://github.com/catalystneuro/neuroconv/pull/1719)
...
* Updated the URL for EMBER with `sandbox=True` for the `automatic_dandi_upload` helper function. Also updated the testing suite to use a new sandbox Dandiset. [PR #1777](https://github.com/catalystneuro/neuroconv/pull/1777)

## Bug Fixes
* Fixed `DeepLabCutInterface` writing the row index into the pose series as if it were seconds. The rows are video frames, so a conversion that stated no timestamps produced a file timed at 1 Hz. Neither the `.h5`/`.csv` nor the project config records a frame rate, so it has to come from the caller: the new `sampling_frequency` argument for a constant rate, or `set_aligned_timestamps` for explicit times, and writing with neither now raises. Files already written this way carry wrong timing rather than missing timing and need re-converting. [PR #1937](https://github.com/catalystneuro/neuroconv/pull/1937)
...
* Fixed every LFP interface writing its series as `ElectricalSeries` rather than `ElectricalSeriesLFP`. The name lived only in `BaseLFPExtractorInterface`'s `es_key` default and the dict-based format names a series from its own entry, so it was lost on the path a conversion that passes no metadata has taken since [PR #1853](https://github.com/catalystneuro/neuroconv/pull/1853). [PR #1928](https://github.com/catalystneuro/neuroconv/pull/1928)
* Fixed `get_metadata(use_new_metadata_format=False)` not being honored: twenty-three `get_metadata` overrides and `NWBConverter._get_interface_metadata` ended their old-format branch in a bare call that let the parent's default answer instead of the format they had been asked for. Nothing changes at the current default, where a bare call already means the old format. [PR #1928](https://github.com/catalystneuro/neuroconv/pull/1928)
* The shared test mixins now name the dict-based format `test_metadata` and `check_extracted_metadata` in every mixin, and the old-format pair `test_metadata_old_list_format` and `check_extracted_metadata_old_list_format`. A downstream test class overriding `test_metadata` to assert the old shape has to rename it. [PR #1928](https://github.com/catalystneuro/neuroconv/pull/1928)
...
* Fixed every dataset added to an NWB file after `nwbfile.objects` was first read being written with no compression and no chunking. `objects` is a cache pynwb builds on first access and never invalidates, and NeuroConv both read it mid-write, inside the GuPPy and VAME interfaces, and read it again to decide what to configure, so anything a later interface added was invisible to the configuration. All six call sites now walk the file with `all_children()` instead. One call order changes with it: building a backend configuration, adding a container, then calling `configure_backend` now raises `ValueError: The number of default configurations (N) does not match the number of specified configurations (M)!` where it used to silently leave the added dataset unconfigured. [PR #1910](https://github.com/catalystneuro/neuroconv/pull/1910)
* Fixed `ScanImageImagingInterface.get_available_channels` and `get_available_planes` raising `AttributeError`. Both called methods roiextractors had renamed before the `roiextractors>=0.9.0` we require, so neither had worked for any file since that pin, and nothing in the test suite called either one. They now read through `get_available_channel_names` and `get_available_num_planes`, and `get_available_planes` keeps returning the list of plane names it documents rather than the count the extractor now reports. [PR #1908](https://github.com/catalystneuro/neuroconv/pull/1908)
* Fixed `ScanImageImagingInterface.get_metadata` raising `AttributeError: 'NoneType' object has no attribute 'replace'` on a single-channel file opened without naming its channel, which is the documented way to open one. Only the old list-based branch interpolated the channel unguarded. The dict-based branch already handled it, and the two now agree that an unnamed channel leaves the objects with the plain names (`ImagingPlane`, `TwoPhotonSeries`). The photon series description no longer reads `for None` either. [PR #1908](https://github.com/catalystneuro/neuroconv/pull/1908)
...
* Fixed an ecephys electrode group being unable to reach its device's model. `_add_electrode_groups_to_nwbfile` handed the device writer a private dictionary holding only the `Devices` registry, so a device naming its model with `device_model_metadata_key` failed with `device_model_metadata_key '...' is not present in metadata['DeviceModels'] (available: [])` no matter what the caller had written. Nothing on the ecephys path had ever linked a model, which is why this went unnoticed since the registry was introduced. The caller's metadata now reaches the device writer whole. [PR #1893](https://github.com/catalystneuro/neuroconv/pull/1893)
* Fixed `add_fiber_photometry_devices` writing every entry of the converter-wide `metadata["Devices"]` and `metadata["DeviceModels"]` registries instead of only fiber photometry's own. An entry naming no `type` came out as a plain `Device`, which the interface that owned it then reused rather than building its intended subtype. Only the devices a `FiberPhotometryTable` row references are written now, so an entry no row references is no longer written here. [PR #1883](https://github.com/catalystneuro/neuroconv/pull/1883)
...
* Fixed the dict-based ophys write path on the interfaces that had never been driven with it. Six interfaces registered a `metadata["Devices"]` entry with no `name`, which `run_conversion` now rejects with `'name' is a required property` and which crashed `add_to_nwbfile` with `KeyError: 'name'` wherever an imaging plane linked it; `SbxImagingInterface`, `FemtonicsImagingInterface` and the two Inscopix interfaces also registered a device no imaging plane referenced, so the writer quietly substituted its placeholder and the description the source recorded was lost. Both are fixed by naming the entry and linking it from an imaging plane. An optical channel entry stating only what the source knows (Thor knows its channel names but not their emission wavelengths) is now completed from the placeholder template rather than rejected by `OpticalChannel`, `ScanImageImagingInterface` and `ThorImagingInterface` name their imaging plane and series after the channel and plane again so two channels in one conversion no longer overwrite each other, and `MicroManagerTiffImagingInterface` keeps the `unit` and `format` the source states. The registries are also keyed by the microscope now rather than by the interface (`scan_image_microscope`, `thor_microscope`, `scanbox_microscope`, `femtonics_microscope`, and `inscopix_{serial}` where the file records one), the way Bruker and Miniscope already were: an interface is per channel, plane or session while a device is per instrument, so a two-channel ScanImage conversion registered one microscope twice and, once the entries carry a name, the duplicate-name check rejected it. Two written-file changes ride along: Thor's optical channel description becomes the placeholder sentence rather than an empty string, and Inscopix's acquisition settings move from series fields that no NWB class accepts into the imaging plane description, where the old format carried them. [PR #1872](https://github.com/catalystneuro/neuroconv/pull/1872)
* Fixed `SpikeGLXNIDQInterface` writing its digital events on a different clock than its analog channels. Event times came from the reader's event extractor, which counts from zero at the start of the file and ignores the stream's `firstSample`, while the analog `TimeSeries` from the same board starts at `firstSample / niSampRate`. One physical instant was therefore recorded at two times in the same file, 11.9 s apart on the `DigitalChannelTest_g0` fixture, and misaligned against the IMEC probes in a `SpikeGLXConverter` run as well since each stream carries its own `firstSample`. The events are now read against the recording's own clock. [PR #1842](https://github.com/catalystneuro/neuroconv/pull/1842)
* Fixed a recording interface in a converter reading the metadata format off another interface's block. A converter builds one metadata dictionary and hands the same one to every interface, so an interface that emits only the dict-based format contributes a dict-shaped top-level `Devices` while a recording interface contributes a list-based `Ecephys`, and the whole-dictionary detection then routed the recording to the dict path, where it raised `ValueError: metadata['Ecephys']['ElectricalSeries'] does not contain key 'ElectricalSeries'` looking for an entry its own `get_metadata` never wrote. An ordinary camera-plus-ephys conversion was enough to reach it. The three decision points on the recording write path now each ask about the block they are about to write rather than about the dictionary's overall shape, so the electrode groups also stop resolving to nothing and writing a placeholder `Device` over the one the metadata names. [PR #1867](https://github.com/catalystneuro/neuroconv/pull/1867)
* Fixed the dict-based ophys write path raising `KeyError: 'name'` on a `metadata["Ophys"]["PlaneSegmentations"]` entry that does not name its own segmentation, which is every segmentation interface except `Suite2pSegmentationInterface`. The name now defaults to `PlaneSegmentation` where the object is built, the way the photon series already defaults its own. Reusing an existing segmentation by name is how two interfaces deliberately share one, so that stays allowed for a name the caller stated, but a defaulted name cannot express that intent and a second entry that collides on one now raises instead of silently dropping its ROIs. [PR #1865](https://github.com/catalystneuro/neuroconv/pull/1865)
* Fixed the dict-based ophys write path writing every ROI property column with an empty description. The three quality metrics that every segmenter names identically (`snr`, `r_values`, `cnn_preds`) now carry the same descriptions the old list-based path gives them. [PR #1865](https://github.com/catalystneuro/neuroconv/pull/1865)
* `BaseImagingExtractorInterface.get_metadata(use_new_metadata_format=True)` now returns a `metadata["Ophys"]["MicroscopySeries"]` entry naming the series it writes, mirroring what `BaseRecordingExtractorInterface` does for the `ElectricalSeries`. The name had until now materialised inside the write path from the placeholder template, so it was invisible to a caller inspecting the metadata before the write and could not be edited there. `MicroscopySeries` is the generic used wherever the source does not say what was imaged; interfaces that know their modality keep overriding it, and the written files are unchanged. [PR #1865](https://github.com/catalystneuro/neuroconv/pull/1865)
* Fixed `MiniscopeConverter` silently dropping the behavior video whenever a User Config file is passed. The behavior camera folders are now discovered from `devices[cameras]` in that config, the same way the imaging device folders already were, instead of globbing the lab-specific `BehavCam*` name at a fixed depth of two below the top folder, where config-driven layouts never sit. Each camera folder is written as an `ExternalVideoInterface` with the timestamps of its own `timeStamps.csv` and a `Device` of its own, aligned to the same session start time as the imaging interfaces. The failure was unconditionally silent, since the `AssertionError` it raised was swallowed and the notice explaining it was guarded by a `verbose` flag that `super().__init__` had already reset. The legacy (no config file) discovery mode is unchanged. [PR #1851](https://github.com/catalystneuro/neuroconv/pull/1851)
...
* Fixed `BrukerTiffImagingInterface` describing every microscope as the same anonymous device. The key and the name were both constants, so two folders acquired on two different Bruker systems merged into one `metadata["Devices"]` entry and the second silently overwrote the first, and no file said which microscope it came from. PrairieView records the system in the `.xml` (`<SystemIDs SystemID="..."><SystemID SystemID="4842" Description="DUKE"/>`), so the device entry is now keyed `bruker_device_<system number>`, carries the machine-unique identifier as its `serial_number`, and names the site in its description. The device's NWB name is unchanged, so two systems in one file now raise the duplicate-name error instead of merging silently. Files predating `SystemIDs` keep the old key and carry no serial number. [PR #1860](https://github.com/catalystneuro/neuroconv/pull/1860)
* Fixed `_build_icephys_hierarchical_tables` deciding which recordings rows were simultaneous from their `(start_index, count)` range instead of the sweep's resolved time. An index is only a proxy for time while every series shares one clock origin: two electrodes of a dual patch both read index 0 and genuinely are simultaneous, but so do the rows of an interface that writes one series per sweep, which are not. Those collapsed into a single `SimultaneousRecordings` entry claiming one electrode recorded several things at once, contradicting the `sweeps` table in the same file, which has always resolved through the series. No shipped interface could reach it, since `AbfInterface` and `AxonIntracellularInterface` both concatenate into one series per electrode; `MockIcephysInterface` could. [PR #1852](https://github.com/catalystneuro/neuroconv/pull/1852)
* Migrated `MiniscopeConverter`'s User Config mode to the dict-based metadata format: one `metadata["Devices"]` entry and one `metadata["Ophys"]["ImagingPlanes"]` entry per Miniscope, shared by all of its recordings, and one `metadata["Ophys"]["MicroscopySeries"]` entry per recording. Several Miniscopes of one hardware design now also share a single `DeviceModel`. The deprecated folder-discovery mode keeps the list-based format until its December 2026 removal. [PR #1849](https://github.com/catalystneuro/neuroconv/pull/1849)
* Fixed `MiniscopeConverter` attributing one recording's acquisition settings to every recording of the same Miniscope. The ndx-miniscope schema declares `gain`, `led0`, `ewl` and the rest as attributes of the device, but the DAQ writes them per recording, so refocusing the lens or changing the LED power between two recordings left the shared device carrying whichever recording was merged last. A device now keeps only the settings all of its recordings agree on, and the ones that vary are reported on each `MicroscopySeries`, which is the recording they are true of. Folder discovery is also sorted, so the conversion no longer depends on the order the filesystem returns paths in. [PR #1849](https://github.com/catalystneuro/neuroconv/pull/1849)
* Fixed `ExternalVideoInterface` and `InternalVideoInterface` dropping `metadata["DeviceModels"]` on the way to the device registry, so a camera `Device` naming its model by `device_model_metadata_key` raised `ValueError` reporting the registry the caller had just populated as empty. Both interfaces now declare `DeviceModels` in their metadata schema, where a model entry may omit the NWB-required `manufacturer`. [PR #1849](https://github.com/catalystneuro/neuroconv/pull/1849)
* Fixed `MiniscopeImagingInterface` losing the device configuration and crashing on real values. `Miniscope` is now a registered device type built by the shared registry from a single config-to-device mapping, so `gain`, `led0`, `frameRate`, `framesPerFile`, `compression` and `ROI` reach the device instead of only its name, values are coerced to the dtype the ndx-miniscope schema declares (`gain: 3.5` and `frameRate: 50` are real values that raised `TypeError`), and settings the schema has no field for (`ewl`, and the sensor offsets of the `ROI`) are named in the device description rather than dropped. The hardware design the DAQ reports as `deviceType` is now written as the `DeviceModel` that pynwb 4 asks for, in place of the `model_name` it deprecates, so several Miniscopes of one design share a single model object. The device is keyed in `metadata["Devices"]` by the Miniscope it describes rather than by the interface's `metadata_key`, since one Miniscope recorded over several sessions is one device but one series per session. Only the dict-based metadata branch changes. [PR #1847](https://github.com/catalystneuro/neuroconv/pull/1847)
* Fixed `metadata["DeviceModels"]` being dropped on the way to the device registry when writing an imaging plane, so a `Devices` entry naming its model by `device_model_metadata_key` raised `ValueError` reporting the registry the caller had just populated as empty. A `DeviceModel` entry may now also omit the NWB-required `manufacturer`, which an acquisition file rarely records; it is filled where the model is built. [PR #1847](https://github.com/catalystneuro/neuroconv/pull/1847)
* Made the `load-data` action's cache fallback plainly unconditional. The expression meant to arm `restore-keys` only when the AWS credentials are missing could never evaluate to empty, because an empty string is falsy in GitHub expressions, so the fallback was already active on every run. It is safe there: each download clears its directory before fetching, which is what keeps a prefix-restored copy from carrying files that S3 no longer has. Behavior is unchanged; the expression and the comments around it are now honest about it. [PR #1828](https://github.com/catalystneuro/neuroconv/pull/1828)
* Fixed `BrukerTiffImagingInterface` registering its microscope under the per-interface `metadata_key`. The microscope belongs to the folder, not to a channel or a depth plane, so writing more than one of them from the same folder (through `BrukerTiffConverter`, or by combining per-channel interfaces in a `ConverterPipe`) registered one device name under several `metadata["Devices"]` keys, which the duplicate-name check added in [PR #1824](https://github.com/catalystneuro/neuroconv/pull/1824) rejects. Every interface over a folder now shares one device key. [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)
* Fixed `_GuppyInterface.get_metadata()` reporting a session start time of `1970-01-01` for GuPPy sessions whose acquisition timestamps are session-relative (CSV, NPM, and most Doric inputs). The interface derived `NWBFile/session_start_time` from the `timeRecStart` dataset, treating the key's absence as "no absolute start time" — but GuPPy always writes that key, as the raw store's first timestamp, which is `0.0` for a session-relative timebase and an absolute epoch only for TDT. `timeRecStart` is a clock origin rather than a recording start, so the interface no longer populates `session_start_time` at all; the acquisition interface or user metadata supplies it. `generate_mock_guppy_output_folder` now derives `timeRecStart` and `correctionIndex` from its raw timebase the way GuPPy does instead of taking a `session_start_time` argument, so the fixture can no longer express a value GuPPy would not write. [PR #1840](https://github.com/catalystneuro/neuroconv/pull/1840)
...
* Fixed the daily dev workflows. [PR #1828](https://github.com/catalystneuro/neuroconv/pull/1828)
...
* Fixed `DeepDict.copy()` raising `TypeError: dict expected at most 1 argument, got 2`. `DeepDict` subclasses `defaultdict`, whose copy protocol reconstructs via `type(self)(default_factory, self)`, but `DeepDict.__init__` hardcodes its own factory and forwards the extra arguments. This surfaced when running a new dict-based-metadata imaging interface through a `ConverterPipe` (e.g. `BrukerTiffConverter` on multi-channel data): the metadata merged by `dict_deep_update` contains `DeepDict` leaves, which the ophys add path copies. [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)
...
## Features
* Added `MedPCEventsInterface`, which writes the events of a MedPC output file as native `pynwb.event.EventsTable` objects into `nwbfile.events`. It replaces `MedPCInterface`, now deprecated, and reads both layouts a MSN program can write, since where an event type's identity lives is decided by the program rather than by the format. In the per-array layout each lettered array is one event type, declared through `medpc_name_to_info_dict` keyed by the MedPC variable, and an entry naming a `durations_name` is durative. New with this interface, an entry naming `value_names` carries a per-event value from another array as a column of the same table (`"S": {"name": "cs_presentations", "value_names": {"K": "cs_type"}}`), which is how a program that records the type of each trial beside its time is written; the column is seeded bare and its codes are relabelled and explained in the metadata through `column_categories`. Also new, the packed-code layout is read through `packed_code_configuration`, where every event is one `TIME.EVENTCODE` value in a single array, the integer part being the time in clock ticks and the fractional digits the event type's code. Two things have to be stated there because the file does not carry them: `clock_ticks_per_second` (500 for a 2 ms clock, 200 for a 5 ms one), since the wrong rate silently scales every timestamp, and `code_to_info_dict`, which names the codes and cannot be derived even when the MSN program ships beside the data, as the program is often a later version whose numbering disagrees with the file. The legend is optional: every code in the array becomes an event type, and one it does not name takes its digits as its name (`code_029`). Alignment is `alignment.shift_times` for a rigid offset, plus `set_aligned_timestamps` to substitute times recovered from another device per event type. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* The imaging and segmentation interfaces gained `get_metadata_template()`, which returns the source-derived values wrapped in the full structure the writer expects, with the cross-references resolved and every field only the experimenter can supply left `None`; the segmentation one is sized to the pipeline's output, offering one entry per trace and per summary image the file actually holds. Both structures are published as YAML and JSON on the `Metadata Templates` page, `How to Annotate Optical Physiology Data` gained a section on filling one in, and the ophys and fiber photometry gallery pages now point at those two. [PR #1939](https://github.com/catalystneuro/neuroconv/pull/1939)
* Added `BaseEventsInterface.get_event_times(event_type_source_id)`, returning one event type's onset times so a line used as a clock (a camera's frame-out pulse, an alignment pulse) can be read back to align another stream against, plus `get_event_type_source_ids` listing the handles it takes. It answers on the interface's current clock, the source's own times plus `alignment.offset`, and returns the same array the writer writes. [PR #1931](https://github.com/catalystneuro/neuroconv/pull/1931)
* Added `metadata_key` (default `"lightning_pose"`) and `use_new_metadata_format` to `LightningPoseDataInterface` and `LightningPoseConverter`, emitting the dict-based shape `DeepLabCutInterface` already uses: top-level `metadata["Devices"]` and `metadata["Pose"]` holding `Skeletons` and `PoseEstimations`, linked via `device_metadata_key`/`skeleton_metadata_key`. `add_to_nwbfile` writes either shape, auto-detected from the metadata, so default behavior is unchanged. [PR #1927](https://github.com/catalystneuro/neuroconv/pull/1927)
...
* Added `ScanImageConverter` and `ThorConverter`, which write every channel of an acquisition in one call. `ScanImageImagingInterface` and `ThorImagingInterface` each read one channel at a time, so a dual-colour session meant instantiating one interface per channel by hand after querying which channels the file holds. Each converter builds that set from the file itself, one `ImagingPlane` and one `TwoPhotonSeries` per channel over a single shared device, the way `BrukerTiffConverter` already does. A ScanImage acquisition spread over several files is followed from its first file, and a volumetric one is written as a 4D series per channel. [PR #1913](https://github.com/catalystneuro/neuroconv/pull/1913)
* `ThorConverter.get_available_channels` reads the channel names from the acquisition's `Experiment.xml`. `ThorTiffImagingExtractor.get_available_channel_names` reads the OME-XML instead, which carries no channel names for this format, so it falls back to `["0", "1"]` and the extractor then rejects those same strings as a `channel_name`. The `Wavelengths` block of `Experiment.xml` is what `ThorImagingInterface` documents and what the extractor accepts. [PR #1913](https://github.com/catalystneuro/neuroconv/pull/1913)
* The `brukertiff`, `micromanagertiff`, `scanimage`, `thor` and `tiff` extras now install `tifffile[codecs]`. Without it a compressed TIFF raises `<COMPRESSION.LZW: 5> requires the 'imagecodecs' package` from inside tifffile, mid-read, reading as though the file were corrupt, and ThorImageLS writes LZW-compressed TIFFs whenever the operator leaves compression on. Compression is a property of the file a user was handed rather than a choice they made, so they cannot know to install it in advance. This is meant to be temporary, and the comment in `pyproject.toml` states the condition for taking it back out. [PR #1913](https://github.com/catalystneuro/neuroconv/pull/1913)
* `BaseEventsInterface` now supports gross temporal alignment via `interface.alignment.shift_times(delta)`, which offsets every event time by `delta` seconds to position an events interface on the shared session clock. The offset is applied at write, leaving durations unchanged, and the source times are never mutated. Implemented by composition: the interface exposes a small alignment component as `interface.alignment` rather than inheriting `BaseTemporalAlignmentInterface`, whose single-timestamp-array contract does not fit events. [PR #1795](https://github.com/catalystneuro/neuroconv/pull/1795)
* The fiber photometry interfaces gained `get_metadata_template()`, the counterpart to `get_metadata()`: it returns the same source-derived values wrapped in the full structure the writer expects, sized to the traces the interface writes at one `FiberPhotometryTable` row per trace, with the `*_metadata_key` cross-references already resolved and every field only the experimenter can supply set to `None`. `get_metadata()` reports only what the source recorded, which is faithful but leaves a user no indication of what else the file needs, and reconstructing the nesting and the cross-references from the JSON schema by hand is real work. The blanks are the checklist: what comes back `None` is exactly what the source could not tell us. The new `Metadata Templates` page in the user guide carries the same structure as a YAML or JSON file to fill in by hand. [PR #1911](https://github.com/catalystneuro/neuroconv/pull/1911)
* A `FiberPhotometryTable` row's `notes` now reaches the file. The column has always been in ndx-fiber-photometry and the writer never passed it through, so anything stated there was silently dropped. [PR #1911](https://github.com/catalystneuro/neuroconv/pull/1911)
* Added `Suite2pConverter`, which writes every plane and channel of a Suite2p output folder in one call. `Suite2pSegmentationInterface` reads one plane and one channel at a time, so a session segmented over three planes in two colours meant instantiating six interfaces by hand after querying `get_available_planes` and `get_available_channels` to know what to ask for. The converter builds that set itself, one `PlaneSegmentation`, `ImagingPlane` and set of traces per pair. Channels are discovered per plane rather than for the folder as a whole, so a session whose red channel was segmented for only some of its planes writes the planes that have it and skips the ones that do not. The `combined` folder Suite2p writes for a multi-plane session is skipped, since its ROIs are the per-plane ones concatenated. [PR #1902](https://github.com/catalystneuro/neuroconv/pull/1902)
* `SpikeGLXRecordingInterface` and `OpenEphysBinaryRecordingInterface` now write the attached Neuropixels probe's identity as a `DeviceModel` carrying the catalogue `manufacturer` and `model_number` (`imec`, `NP1110`) and a `Device` carrying the unit's `serial_number`, so `probeinterface.get_probe(manufacturer, model_number)` rebuilds the probe from what is in the file. Streams that carry no probe are unaffected. [PR #1895](https://github.com/catalystneuro/neuroconv/pull/1895)
* `add_recording_to_nwbfile` reads the same identity off the recording's probe, so an electrode group that names no device of its own links to the probe instead of the placeholder and a caller passing no metadata gets the provenance too. This only fires for a single-probe recording whose probe names a `model_name`, which is what makes it reconstructable, so a probe carrying only a manufacturer (Biocam, Maxwell) keeps `PlaceholderElectrodeDevice` as before. A group naming its own `device_metadata_key` still wins and no probe-derived device is created alongside it. [PR #1895](https://github.com/catalystneuro/neuroconv/pull/1895)
...
* Added `saved_files_are_split` to `IntanDigitalInterface`, the last Intan interface without it, so a session saved with RHX's "create a new save file every N minutes" option converts its digital lines too. The rotated files are read as one continuous recording before edge detection, which is what a digital line needs rather than a longer trace: a pulse that rises in one file and falls in the next reads as the single pulse it is, and every pulse carries the time it has on the session's clock rather than an offset into the chunk that happens to hold it. `IntanConverter` no longer skips the digital stream for a rotated session. [PR #1891](https://github.com/catalystneuro/neuroconv/pull/1891)
...
* `GuppyConverter` now reads a session whose events did not all come from the acquisition system. GuPPy's custom-event import writes the events it imported back out as one-column `timestamps` CSVs, which then sit in the session folder beside whatever the rig recorded, so a session's `storesList.csv` can list stores from two sources at once. `events_folder_path` is scanned for those CSVs, and an event store with one of its own is read from it while every other store falls to `acquisition_format` — no new argument. A store the acquisition source also carries is read from its CSV, which is the source GuPPy itself prefers for it. Traces still come from a single `acquisition_format`. [PR #1879](https://github.com/catalystneuro/neuroconv/pull/1879)
* `GuppyInterface` is now public, which is how a session GuPPy processed out of an existing NWB file is converted: hand the file to `add_to_nwbfile` and GuPPy's outputs are added to it, with each recording site linked to the `FiberPhotometryTable` rows its fibers occupy in the file already there. Writing to a new path exports the source with the outputs added. The events registry references an `EventsTable` of GuPPy's own analyzed onsets, written into `nwbfile.events` under an editable name — the same table whether the interface runs standalone or inside `GuppyConverter`, so every peri-event product reaches the occurrences it was built from either way. The raw events a `GuppyConverter` run writes are consequently no longer merged into one `BehavioralEvents` table: each event type gets its own `EventsTable`, exactly as adding those events interfaces directly would write it. [PR #1880](https://github.com/catalystneuro/neuroconv/pull/1880)
* Added `GuppyConverter` to bundle a GuPPy session's raw acquisition, raw events, and GuPPy-derived outputs into a single conversion. The recording format is selected with `acquisition_format` (`"tdt"`, `"csv"`, `"doric"` or `"npm"`), and `neuroconv[guppy]` now installs all four. Acquisition series are grouped by excitation wavelength rather than written one per store, so a session's `signal` and `control` channels each become one `FiberPhotometryResponseSeries` column-stacked across recording sites. [PR #1783](https://github.com/catalystneuro/neuroconv/pull/1783)
* The modality metadata blocks are now described by `get_metadata_schema()` in the dict-based format: `Ecephys.ElectrodeGroups` and `Ecephys.ElectricalSeries` on the recording base, `Electrodes` and `UnitProperties` on the sorting base, `Ophys.ImagingPlanes` and `MicroscopySeries` on the imaging base, plus `PlaneSegmentations`, `RoiResponses` and `SegmentationImages` on the segmentation base, each an object keyed by `metadata_key` rather than an array, with the `device_metadata_key` and `imaging_plane_metadata_key` cross-references no hdmf class knows about. Entries stay permissive, since each one is handed to a pynwb constructor, but their shape is now checked, so an edit written against the old format that lands in a block whose name exists in both (`metadata['Ecephys']['ElectricalSeries']['name'] = ...`) is rejected at validation instead of being silently ignored at write time. The old list-based format is validated exactly as before, against `_get_metadata_schema_for_old_list_format`. With the shape declared centrally, the local `Devices` and `DeviceModels` declarations in `ExternalVideoInterface`, `InternalVideoInterface` and `BaseFiberPhotometryInterface` are gone. [PR #1873](https://github.com/catalystneuro/neuroconv/pull/1873)
* Added `IntanDigitalInterface` for converting Intan digital TTL lines into discrete events. Lines are selected and edge-detected via a `detection_configuration` argument keyed by the header's own name for each line, so one interface covers both digital words, writing native `pynwb.event.EventsTable` objects. Coded multi-bit words are deferred. [PR #1812](https://github.com/catalystneuro/neuroconv/pull/1812)
...
* `SpikeGLXNIDQInterface` now writes its digital events as native `pynwb.event.EventsTable` objects into `nwbfile.events`, rather than as `ndx-events` `LabeledEvents` objects into `acquisition`. Which lines to read and how is set through a new `detection_configuration` argument on the shared signal-encoded grammar: SpikeGLX packs its digital lines into one integer word per saved channel, so a line is addressed as word plus bit (`{"XD0": [{"signal_conditioning": {"bits": [0]}, "detection": "high_period"}]}`), matching what `~snsChanMap` and CatGT use, and the analog channels are inventoried alongside the words so they can be cut into events with `thresholds`. By default every line the header declares is read as a `high_period`, which is lossless: each rising edge is an onset and its duration runs to the falling edge. The `digital_channel_groups` argument is deprecated and will be removed on or after August 2027; it is now translated onto the new grammar rather than writing through a separate path, so a group still yields one object holding every edge, with `labels_map` naming the two edges through the table's `event_type` column instead of through an index into a `labels` list. `ndx-events` is no longer a dependency of `neuroconv[spikeglx]`. [PR #1842](https://github.com/catalystneuro/neuroconv/pull/1842)
* Dict-based metadata can now be passed to `run_conversion`. `validate_metadata` on both `BaseDataInterface` and `NWBConverter` picks the schema that matches the metadata it is given, so dict-based metadata is checked against the base schema instead of being rejected by a modality schema that only describes the old list-based shape (`'Device' is a required property`); list-based metadata is validated exactly as before. The base schema now also declares the top-level `Devices` and `DeviceModels` registries, which no schema described until now, so every interface that emits them stops having to declare its own and `MiniscopeConverter` no longer needs its local workaround for the gap. One consequence worth knowing: an interface that emits the legacy list-valued `metadata["Devices"]` has that list validated for the first time, against a required `name` with no slash in it. Every interface that emits one already satisfies this. [PR #1868](https://github.com/catalystneuro/neuroconv/pull/1868)
* Added the dict-based metadata format to `NeuroScopeRecordingInterface` and `NeuroScopeLFPInterface` via `get_metadata(use_new_metadata_format=True)`, together with the `metadata_key` argument (defaults `"neuroscope_recording"` and `"neuroscope_lfp"`). The shared `get_ecephys_metadata` helper now returns the electrode groups as `Ecephys.ElectrodeGroups` keyed by group name rather than as a list. The `.xml` describes the shank structure and no acquisition device, so no device is claimed and the old format's `location=""` and placeholder device link are not carried over. The old list-based format stays the default. [PR #1859](https://github.com/catalystneuro/neuroconv/pull/1859)
* Added the dict-based metadata format to `AxonRecordingInterface`, `MaxOneRecordingInterface` and `MEArecRecordingInterface` via `get_metadata(use_new_metadata_format=True)`, together with the `metadata_key` argument (defaults `"axon_recording"`, `"maxone_recording"` and `"mearec_recording"`). Each registers its device in the top-level `metadata["Devices"]` registry and links every channel group to it through `device_metadata_key`: Axon keeps its `Axon Instruments` device and its `ElectricalSeriesRaw` naming, MaxOne is now a named `MaxOne` device carrying the Maxwell software version instead of only rewriting the placeholder device's description, and MEArec keeps the electrode template name the file records while dropping the boilerplate device description, which no field supports. The old list-based format stays the default. [PR #1858](https://github.com/catalystneuro/neuroconv/pull/1858)
* Added the dict-based metadata format to `NeuralynxRecordingInterface` and `AxonaRecordingInterface` via `get_metadata(use_new_metadata_format=True)`, together with the `metadata_key` argument (defaults `"neuralynx_recording"` and `"axona_recording"`). Each registers its acquisition system in the top-level `metadata["Devices"]` registry (Neuralynx reads it from the header's `AcquisitionSystem`, described by the Cheetah application and version; Axona reports the DacqUSB software version from the `.set` header) and links every channel group to it through `device_metadata_key`. The old list-based format stays the default. [PR #1856](https://github.com/catalystneuro/neuroconv/pull/1856)
* The ecephys converters now route the dict-based metadata format. `OpenEphysBinaryConverter` gives each neural stream its own `metadata_key`, derived from the stream name, and names each stream's `ElectricalSeries` in `get_metadata(use_new_metadata_format=True)`, so a multi-stream session can be written from dict-based metadata (the interfaces cannot do this themselves: each one only knows it is "the" Open Ephys recording, so they all key and name alike). `IntanConverter` passes its routing table's key through as `metadata_key` rather than translating it into `es_key`. [PR #1850](https://github.com/catalystneuro/neuroconv/pull/1850)
...
* `Subject` metadata no longer requires `subject_id`, `sex` and `species`. NWB requires none of them, so an interface that reads only part of a file's subject information can now report just that instead of inventing the rest. The Inscopix interfaces no longer fill `subject_id` with `"Unknown"` or `species` with `"Unknown species"`; NWB Inspector reports the omission instead. `sex` still falls back to `"U"`, NWB's own term for unknown. [PR #1845](https://github.com/catalystneuro/neuroconv/pull/1845)
...
* Added the `metadata_key` argument to `AlphaOmegaRecordingInterface`, `BlackrockRecordingInterface`, `EDFRecordingInterface`, `PlexonRecordingInterface`, `PlexonLFPInterface`, `Plexon2RecordingInterface`, `CellExplorerRecordingInterface`, and `CellExplorerLFPInterface`, and made their `get_metadata` accept `use_new_metadata_format=True`. None of them reads a device or electrode groups out of the source, so the argument and its snake_case default (`alpha_omega_recording`, `blackrock_recording`, `edf_recording`, `plexon_recording`, `plexon_lfp`, `plexon2_recording`, `cell_explorer_recording`, `cell_explorer_lfp`) are the whole of their dict-based metadata support. The two LFP interfaces name their own series (`ElectricalSeriesLF` and `ElectricalSeriesLFP`) rather than taking the base default. The old list-based format stays the default. [PR #1848](https://github.com/catalystneuro/neuroconv/pull/1848)
* Added the `metadata_key` argument to `SpikeGadgetsRecordingInterface`, `BiocamRecordingInterface`, `MCSRawRecordingInterface`, `Spike2RecordingInterface`, `TdtRecordingInterface`, and `WhiteMatterRecordingInterface`, which previously raised `TypeError` when it was passed. None of them claims a device or electrode groups, so the argument and its snake_case default (`spikegadgets_recording`, `biocam_recording`, `mcs_raw_recording`, `spike2_recording`, `tdt_recording`, `white_matter_recording`) are the whole of their dict-based metadata support; the old list-based format stays the default. [PR #1844](https://github.com/catalystneuro/neuroconv/pull/1844)
* Added the dict-based metadata format to `OpenEphysBinaryRecordingInterface` and `OpenEphysLegacyRecordingInterface` via `get_metadata(use_new_metadata_format=True)`, with the `ElectricalSeries` keyed by the new keyword-only `metadata_key` argument (default `"open_ephys_recording"`, also exposed on the `OpenEphysRecordingInterface` dispatcher). Neither interface has ever claimed a device or electrode groups, so this is a shape change only, and the old list-based format stays the default. [PR #1843](https://github.com/catalystneuro/neuroconv/pull/1843)
* Added the dict-based metadata format to `SpikeGLXRecordingInterface` via `get_metadata(use_new_metadata_format=True)`: the Neuropixels probe is registered in the top-level `Devices` registry (keyed per probe, so a probe's AP and LF streams share one device) and each per-shank electrode group links to it via `device_metadata_key`. The old list-based format stays the default. [PR #1816](https://github.com/catalystneuro/neuroconv/pull/1816)
...
* Added `CSVFiberPhotometryInterface` for converting a raw fiber photometry recording from a CSV file, parsed into the `ndx-fiber-photometry` format; it can demultiplex an interleaved recording (excitation channels multiplexed frame-by-frame down the rows) via a `demux_configuration`, reading one channel per interface. [PR #1754](https://github.com/catalystneuro/neuroconv/pull/1754) [PR #1808](https://github.com/catalystneuro/neuroconv/pull/1808)
...
* Added a `plane_index` parameter to `BrukerTiffImagingInterface` and a `plane_separation_type` option to `BrukerTiffConverter`, so volumetric Bruker data can be written either as one 4D `TwoPhotonSeries` per channel (`"contiguous"`) or one 2D `TwoPhotonSeries` per depth plane (`"disjoint"`), with each imaging plane carrying its own focal depth read from the active `zDevice`. This uses roiextractors' `select_plane` and replaces the deprecated `BrukerTiffMultiPlaneConverter`. [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)
...
* Added `BrukerTiffConverter`, a convenience converter that auto-enumerates channels in a Bruker folder and builds one `BrukerTiffImagingInterface` per channel. Replaces the deprecated `BrukerTiffSinglePlaneConverter`. [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)
...
* Added `metadata_key` and a `use_new_metadata_format` parameter to `DeepLabCutInterface.get_metadata()`/`get_metadata_schema()` for the dict-based metadata pipeline: top-level `metadata["Devices"]` and `metadata["Pose"]` holding `Skeletons` and `PoseEstimations` registries linked via `device_metadata_key`/`skeleton_metadata_key`. `add_to_nwbfile` writes either shape (auto-detected from the metadata), so default behavior is unchanged. The `pose_estimation_metadata_key` argument is deprecated in favor of `metadata_key` and will be removed on or after February 2027. [PR #1750](https://github.com/catalystneuro/neuroconv/pull/1750)
...
* Added `DoricFiberPhotometryInterface` for converting fiber photometry data from Doric Neuroscience Studio `.doric` HDF5 files. [PR #1727](https://github.com/catalystneuro/neuroconv/pull/1727)
* Added `BrukerVoltageRecordingInterface` and `BrukerVoltageRecordingConverter` for the intracellular recordings Bruker PrairieView writes alongside two-photon imaging, closing [issue #379](https://github.com/catalystneuro/neuroconv/issues/379). PrairieView writes one CSV/XML pair per cycle, a cycle being one sweep, and the interface takes an explicit list of them for one electrode and concatenates them into a single `PatchClampSeries` placed on one timeline by each cycle's `DateTime`. The clamp mode is read off the unit of the amplifier's `Primary` output (`mV` for current clamp, `pA` for voltage clamp) rather than being required, and the `Multiplier` / `Divisor` scaling is carried in the series' `conversion` so the samples stay exactly what PrairieView wrote. Column identity comes from the XML's `Enabled` flags positionally rather than from the CSV header, because the header's names are shifted on real published recordings (a whole recording day of the Zhai et al. 2025 deposit labels its columns `Secondary, LED` on files whose enabled signals are `Primary` and `Secondary`), and believing them writes the membrane potential as a current. [PR #1854](https://github.com/catalystneuro/neuroconv/pull/1854)

## Improvements
* Added `docs/how_to/extract_events_from_signals.rst`, documenting the `detection_configuration` argument the signal-encoded events interfaces take. The grammar had shipped user-facing and undocumented: it was taught from scratch on the SpikeGLX and Intan recording gallery pages and never mentioned on the Doric events page, although both Doric interfaces take it. The page covers which signals are read, how each one becomes a line with `bits` or `binarize`, and which of its transitions become events, along with the failure a parameter name cannot state, that a configuration is a selection and drops the signals it does not name. The SpikeGLX, Intan and Doric gallery pages now point at it and keep only what is specific to their own hardware, so the grammar is explained once instead of twice and omitted once. `docs/how_to/annotate_events_metadata.rst` gained a pointer as well, since that page owns what the events are called and how they are laid out while this one owns which events exist. [PR #1942](https://github.com/catalystneuro/neuroconv/pull/1942)
* The SpikeGLX converters' dict-based metadata is now pinned by tests, one per probe count, and every test that opts into the old list-based format carries an `_old_list_format` suffix so that block can be deleted in one cut when the format goes. [PR #1930](https://github.com/catalystneuro/neuroconv/pull/1930)
* Writing an `EventsTable`, a `PlaneSegmentation`, an ethogram bouts table, a GuPPy events table or an icephys sweeps table is now linear in the number of rows rather than quadratic. `DynamicTable.add_row` calls hdmf's `is_ragged` on the *entire* column after appending each value, so row *n* costs O(n) and filling a table costs O(n^2). None of these tables can hold a ragged cell, since every value they write is a scalar, so the check was walking the whole column on every row to reach the same answer it reached the row before. They now pass `check_ragged=False`, which hdmf documents for exactly this case. A twelve-minute fiber photometry recording whose digital line pulses at 30 Hz writes its 21,330 events in 0.9 s instead of 201 s, and a segmentation with 8,000 image-mask ROIs writes in 0.5 s instead of 22 s. The written files are unchanged. [PR #1920](https://github.com/catalystneuro/neuroconv/pull/1920)
* The link checker no longer follows `ffmpeg.org` or `nwb-users.slack.com`, the two hosts behind every recent failure of the "Testing External Links" job. Since the first one on 2026-07-03 there have been 18 ffmpeg.org failures, and the host is not blocking us: its HTTPS listener drops connections under load, measured as 3 failures and a 2.2 second median across 25 connects on port 443 against 25 clean ones at 0.2 seconds on port 80 of the same address, so the checker times out on a link that is fine. Slack answers the checker with a plain 403. Raising `linkcheck_timeout` was the alternative and it only narrows the window, since the latency tail is unbounded (a 133 second connect was measured), and `linkcheck_retries` does not apply here at all because sphinx stops retrying on a timeout. The three ffmpeg.org links are the site root and its two landing pages. [PR #1912](https://github.com/catalystneuro/neuroconv/pull/1912)
* The YAML specifications printed in `docs/user_guide/docker_demo.rst` and `docs/user_guide/aws_demo.rst` run again. Both still passed `file_path` to `SpikeGLXRecordingInterface`, which has taken `folder_path` and `stream_id` since v0.9.0, so both failed on their first session, and the AWS one asked for a DANDI upload without giving its sessions the `session_id` that requires. Neither was caught because the Docker CI test runs the specification maintained under `tests/`, not the one the page prints, and those two pages are the only documentation the specification format has. The Docker page also no longer promises console output that the command has not printed since the converter stopped being constructed with `verbose=True`. [PR #1906](https://github.com/catalystneuro/neuroconv/pull/1906)
* Added `docs/user_guide/converting_multiple_sessions.rst`, covering the conversion of many sessions and what to do with the files afterwards. It walks the loop over `LocalPathExpander` results that converts one session at a time, and then the two things you do with the resulting folder, uploading it to a Dandiset with `automatic_dandi_upload` and reorganizing it into a BIDS layout with `nwb2bids`. Both are dataset-wide steps over finished NWB files, which is why they live here rather than on the pages about converting a single session, and neither layout is NeuroConv's own: the DANDI one comes from the `dandi` client's organize step and the BIDS one from `nwb2bids`. [PR #1906](https://github.com/catalystneuro/neuroconv/pull/1906)
* A recording whose `group_name` property disagrees with its `group` property now says which property to fix. The two go out of step whenever channels are re-grouped after a name was set, which is easy to do without noticing because several interfaces set a name of their own at construction, and the old messages, "The number of group names must match the number of groups" and "Inconsistent mapping between group numbers and group names", named neither the properties involved nor the remedy. They now report the conflicting values and say to restate `group_name` or delete it. [PR #1897](https://github.com/catalystneuro/neuroconv/pull/1897)
...
* The conversion gallery now states which installation extra each page needs, in `docs/conversion_examples_gallery/extras_by_gallery_entry.json`, rather than the formatwise installation workflow assuming the extra is named after the page's file. The two names are independent and an undefined extra is a silent no-op for both pip and uv, so a page whose name did not happen to match one installed nothing and failed much later with an import error that read like a missing dependency; `guppy_fp.rst` against the `guppy` extra cost six days of red dailies. The table is the source of truth for which page is tested and with which extra (`null` where the page converts on a base install), and `skip` carries the reason for the eight that are not tested. The gallery directory is still walked, but only to hold the table to it, so a page nobody registers fails collection instead of going quietly untested. [PR #1887](https://github.com/catalystneuro/neuroconv/pull/1887)
* A green CI run reported 2154 pytest warnings, and 956 of them came from two categories no caller can act on. `EncodingWarning` is now ignored by default and re-raised as an error for the `neuroconv` and `tests` modules, so the PEP 597 policy [PR #1660](https://github.com/catalystneuro/neuroconv/pull/1660) introduced still fails CI for our own `open()` calls while the ~700 reported by dependencies stay quiet; the four remaining sites under `tests/` that [PR #1857](https://github.com/catalystneuro/neuroconv/pull/1857) could not reach now declare `encoding="utf-8"`. The `num_samples` `DtypeConversionWarning` is also ignored, since `TimeSeries.num_samples` is a computed pynwb property returning `len(data)` and no caller can influence the dtype that reaches hdmf. The 17 class-scoped fixtures written as instance methods are now `@classmethod`, which is what pytest 10 requires and what they already meant, since each reached `request.cls` rather than touching `self`; and the image interface tests name the raw layout through `Image.frombuffer` for the modes Pillow 13 stops inferring from `Image.fromarray(..., mode=)`. Both write the same files as before. [PR #1869](https://github.com/catalystneuro/neuroconv/pull/1869)
* `DataInterfaceTestMixin.test_metadata` now validates `get_metadata()` against the interface's own `get_metadata_schema()`, which nothing in the pytest-style mixin did before: the schema was checked for well-formedness and the metadata was checked for existence, but never one against the other. Required fields a source cannot know are filled at write time, so the metadata is validated as the writer would receive it rather than as raw provenance. [PR #1866](https://github.com/catalystneuro/neuroconv/pull/1866)
* CI installs with `uv` instead of `pip` everywhere in `testing.yml`, including the six isolated-extras environments, each of which creates a virtual environment and resolves a group from scratch. The cross-run cache is deliberately left off, since those steps exist to prove each extras group installs on a machine that has nothing. The Python versions CI tests against move from `all_python_versions.txt` to `min_python_version.txt` and `max_python_version.txt`, which the workflows compose the full list from, so the minimum has a name instead of being a literal repeated in three ternaries. The live-service tests, which exercise DANDI, EMBER and Globus network behavior rather than anything platform-specific, now run on every operating system but only the newest Python for a pull request, with the full matrix still running nightly in the dev dailies. Finally, the YAML DANDI upload test polls the sandbox until it reports the assets this run uploaded instead of sleeping a flat 60 seconds, which was 40 percent of that job. [PR #1864](https://github.com/catalystneuro/neuroconv/pull/1864)
* Each interface now writes two NWB files in its test class instead of six, and both of them are read back and checked. `run_conversion(backend=...)` resolves the same default backend configuration internally that `run_conversion(backend_configuration=...)` receives, so the two spellings were writing byte-identical files per interface; that equivalence is now asserted once on a mock interface, and the per-interface tests keep only the hdf5 and zarr writes that `check_read_nwb` validates. The zarr file used to be written twice and never checked against the source data, so this raises per-format coverage while cutting test time. Each `check_read_nwb` now reads through `pynwb.read_nwb`, which sniffs the backend, so one body serves both formats. [PR #1862](https://github.com/catalystneuro/neuroconv/pull/1862)
...
* Declared an explicit `encoding="utf-8"` at the 33 text-mode `open()`, `read_text()` and `write_text()` call sites under `tests/` and `docs/` that were still relying on the platform default. [PR #1660](https://github.com/catalystneuro/neuroconv/pull/1660) already makes this a hard error, but its warning filter is scoped to the `neuroconv` module, so these sites emitted an `EncodingWarning` rather than failing and had gone unnoticed. Running `tests/test_minimal` and `tests/test_modalities` now reports 43 warnings instead of 87, and every remaining one comes from hdmf or numpy. [PR #1857](https://github.com/catalystneuro/neuroconv/pull/1857)
* The Bruker TIFF conversion gallery now leads with `BrukerTiffConverter`, which covers every channel in a folder and both volumetric layouts, and presents `BrukerTiffImagingInterface` afterwards as the way to convert a single channel or a single depth plane. [PR #1773](https://github.com/catalystneuro/neuroconv/pull/1773)

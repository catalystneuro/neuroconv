...
* Added `BaseEventsInterface`, a shared base for events interfaces that writes native `pynwb` 4.0 `EventsTable` objects into `nwbfile.events`. [PR #1774](https://github.com/catalystneuro/neuroconv/pull/1774) [PR #1787](https://github.com/catalystneuro/neuroconv/pull/1787)
...
* Added `DANNCEInterface` for converting DANNCE and social-DANNCE (sDANNCE) 3D pose estimation `.mat` output files to NWB format using the ndx-pose extension. A single interface handles both single-animal DANNCE output and multi-animal sDANNCE output. 3D landmark data is written as an `ndx_pose.MultiCameraPoseEstimation` container, with per-camera `Device`/source-video linking via `camera_names`/`source_videos`. Camera calibration is auto-detected and applied via the `calibration_path` constructor argument (or the standalone `DANNCEInterface.get_camera_calibrations()` method), supporting three real-world DANNCE calibration layouts (a directory of per-camera `hires_camN_params.mat` files, a single `calibration.json`, or a Label3D-style `*_dannce.mat` with `camnames`/`params`) -- camera names and `ndx_pose.CalibratedCamera` parameters are both populated automatically with no manual parsing required. [PR #1710](https://github.com/catalystneuro/neuroconv/pull/1710)

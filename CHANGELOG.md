...
* Deprecated `BrukerTiffSinglePlaneImagingInterface`, `BrukerTiffMultiPlaneImagingInterface`, `BrukerTiffSinglePlaneConverter`, and `BrukerTiffMultiPlaneConverter`. Use the unified `BrukerTiffImagingInterface` (or `BrukerTiffConverter` for multi-channel and multi-plane folders) instead. Will be removed on or after December 2026. [PR #1732](https://github.com/catalystneuro/neuroconv/pull/1732)
...
* Fixed `BrukerTiffImagingInterface` writing the z-plane *spacing* (the swing between the first and last plane) into the volumetric `ImagingPlane.origin_coords` where a position is expected. `origin_coords` z is now the first plane's focal depth (read from the active `zDevice`), while the inter-plane step and total depth extent go to `grid_spacing` and `field_of_view`. [PR #1732](https://github.com/catalystneuro/neuroconv/pull/1732)
* Fixed `DeepDict.copy()` raising `TypeError: dict expected at most 1 argument, got 2`. `DeepDict` subclasses `defaultdict`, whose copy protocol reconstructs via `type(self)(default_factory, self)`, but `DeepDict.__init__` hardcodes its own factory and forwards the extra arguments. This surfaced when running a new dict-based-metadata imaging interface through a `ConverterPipe` (e.g. `BrukerTiffConverter` on multi-channel data): the metadata merged by `dict_deep_update` contains `DeepDict` leaves, which the ophys add path copies. [PR #1732](https://github.com/catalystneuro/neuroconv/pull/1732)
...
* Added `BrukerTiffImagingInterface`, a unified interface for Bruker Prairie View OME-TIFF imaging data backed by the new unified `BrukerTiffImagingExtractor` from roiextractors. Single-plane, volumetric, and multi-channel cases are handled through one class, with channels selected via zero-indexed `channel_name` strings. The interface emits metadata in the new dict-based format only (`Devices`, `Ophys.ImagingPlanes`, `Ophys.MicroscopySeries`). [PR #1732](https://github.com/catalystneuro/neuroconv/pull/1732)
* Added `BrukerTiffConverter`, a convenience converter that auto-enumerates channels in a Bruker folder and builds one `BrukerTiffImagingInterface` per channel. Replaces the deprecated `BrukerTiffSinglePlaneConverter`. [PR #1732](https://github.com/catalystneuro/neuroconv/pull/1732)

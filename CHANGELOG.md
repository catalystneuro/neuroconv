...
## Improvements
* Added `docs/user_guide/converting_multiple_sessions.rst`, covering the conversion of many sessions and what to do with the files afterwards. It gives the two routes for the conversion itself, the Python loop over `LocalPathExpander` results and the YAML specification, and `run_conversion_from_yaml` had no documentation at all before this: the `experiments` / `sessions` structure it reads was shown only inside the Docker and AWS demos, and `upload_to_dandiset` only inside the AWS one. It then covers the two things you do with the resulting folder, uploading it to a Dandiset with `automatic_dandi_upload` and reorganizing it into a BIDS layout with `nwb2bids`. Both are dataset-wide steps over finished NWB files, which is why they live here rather than on the pages about converting a single session, and NeuroConv writes neither layout itself. [PR #1906](https://github.com/catalystneuro/neuroconv/pull/1906)

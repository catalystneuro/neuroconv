...
## Features
* Added `GuppyConverter` to bundle a GuPPy session's raw acquisition, raw events, and GuPPy-derived outputs into a single conversion. The recording format is selected with `acquisition_format` (`"tdt"`, `"csv"`, or `"doric"` — the last covering all three Doric layouts; GuPPy also reads Neurophotometrics sessions, which are not supported yet). Install with `neuroconv[guppy]`, which now carries every supported acquisition format. Acquisition series are grouped by excitation wavelength rather than written one per store, so a session's `signal` and `control` channels each become one `FiberPhotometryResponseSeries` column-stacked across recording sites. [PR #1783](https://github.com/catalystneuro/neuroconv/pull/1783)

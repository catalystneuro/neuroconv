...
## Features
* `BaseFiberPhotometryInterface` now supports gross temporal alignment via `interface.alignment.shift_times(delta)`, which moves every time-bearing object the interface writes by `delta` seconds to place it on the shared session clock, so every interface built on it (CSV, multi-file CSV, NPM, Doric and the single-series TDT one) gains it at once. The shift is rigid and reaches both the `FiberPhotometryResponseSeries` and any `CommandedVoltageSeries` beside it, leaving the sampling rate alone and never mutating the source times, which `get_original_timestamps()` still reports unshifted. Unlike the events interfaces, which hold the same alignment component but expose no timestamps getter, `get_timestamps()` here reflects the shift, so an interface never reports times the file will disagree with; `set_aligned_timestamps` stays authoritative, so the times handed to it are returned verbatim and a pending shift is discarded rather than added to them. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

...
## Improvements
* Added a draft of the reworked temporal alignment user guide (`docs/user_guide/temporal_alignment.rst`) describing a proposed per-clock origin model: coarse alignment via `shift_times`, fine alignment via `set_times`, and multiple clocks per interface via `timebases` and `set_timebase_origin`, illustrated by figures generated from a committed script. The page is marked as a draft; the API it documents is proposed and not yet implemented. [PR #1793](https://github.com/catalystneuro/neuroconv/pull/1793)

...
* Both MedPC events interfaces now state what a stored value is worth, since no MedPC file records it: `time_unit`, which may give one unit per event type, `clock_ticks_per_second`, and `relative_mode` for a program that stored the interval since the previous event rather than the elapsed time. Every wrong choice decodes without error, so the onsets read are checked for running backwards and against the session length the header states. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

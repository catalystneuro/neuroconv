...
## Features
* Added `MedPCArrayEventsInterface` and `MedPCPackedEventsInterface`, which write the events of a MedPC output file as native `pynwb.event.EventsTable` objects into `nwbfile.events`, differing in where an event type's identity comes from: one lettered variable per type, or a code packed into the decimals of the time value. No MedPC file records what a stored value is worth, so both take `time_unit` and `relative_mode`, and every wrong choice decodes without error, which is why the onsets read are checked for running backwards and against the session length the header states. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

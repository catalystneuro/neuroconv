...
## Improvements
* `BaseFiberPhotometryInterface.get_metadata()` no longer emits a placeholder device/indicator/table scaffold (`"PLACEHOLDER"` strings and `NaN` wavelengths); it now returns only the response-series entry, so an interface writes a bare `FiberPhotometryResponseSeries` unless the user supplies the full provenance chain. Added `get_example_metadata()`, which returns a complete, realistic template (signal + isosbestic-control rows) to fill in. [PR #1800](https://github.com/catalystneuro/neuroconv/pull/1800)

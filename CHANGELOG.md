...
* Added `BaseFiberPhotometryInterface`, a single-series base class (one `FiberPhotometryResponseSeries` per interface, assembled from one or more input streams) that owns the shared NWB-writing and metadata logic: idempotent device/indicator/table construction, `FiberPhotometryTable` region resolution, a standard scalar temporal-alignment API, and a runnable default metadata scaffold that warns about unset placeholder fields. Child interfaces implement only a small format-reading seam. Fiber photometry now has its own `datainterfaces.fiber_photometry` category (split out from `ophys`, mirroring the ecephys/icephys split), and its metadata follows the dict-based `metadata_key` system (consistent with the ophys metadata pattern): shared containers are dicts keyed by `metadata_key` with `_metadata_key` references, lifted to the top-level `metadata["FiberPhotometry"]` key rather than nested under `metadata["Ophys"]`. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

## Removals, Deprecations and Changes
* Bumped minimum `pynwb` version to `>=4.0.0`. [PR #1769](https://github.com/catalystneuro/neuroconv/pull/1769)
* Migrated `TDTFiberPhotometryInterface` to a single-series interface built on `BaseFiberPhotometryInterface`: each interface now writes one `FiberPhotometryResponseSeries` assembled from the input stream(s) named by the new `stream_names` constructor argument (with `metadata_key` for combining several interfaces over one shared `FiberPhotometryTable`), and `get_available_streams()` lists a tank's stores. Constructing without `stream_names` routes to the previous multi-series behavior via a deprecating router, emits a `DeprecationWarning`, and will be removed on or after August 2026. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

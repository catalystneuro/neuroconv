...
## Features
* `ExternalVideoInterface` now carries the `alignment` surface the fiber photometry and events interfaces hold, with each video file separately addressable under the stem of its path, so `alignment.shift_times(delta)` moves the camera as a whole while `alignment[stem].start_at(starting_time)` places one file where its trigger fired without building its times, `alignment[stem].set_times(times)` gives one file the times a pulse train measured and `alignment.remap_times(...)` re-expresses a camera that keeps its own clock on the session's; the four `set_aligned_*` methods and `get_timestamps` still work under a `FutureWarning` and will be removed in v0.12.0.

...
* Fixed `EDFRecordingInterface.get_metadata` silently discarding all subject metadata. The metadata dict is a `DeepDict` (a `defaultdict`), and `dict.get` does not go through `__missing__`, so `metadata.get("Subject", dict()).update(...)` updated a throwaway dict and the `subject_id` and `date_of_birth` read from the EDF header never reached the NWB file. The subject metadata is now assigned rather than updated in place. The subject's sex is also read from the header (mapping the `Female`/`Male` pyedflib reports, and the older `gender` key, onto NWB's `F`/`M`) instead of always being reported as unknown, and the schema-required `species`/`sex` fields are defaulted only as a fallback, so that reading a patient code out of a file cannot turn valid metadata into invalid metadata. Note that a de-identified file retaining only the sex subfield now gains a `Subject` with a placeholder `subject_id` of `"Unknown"`, where previously it had none. The EDF header's birthdate is also parsed into a `datetime` (and dropped when unparsable) rather than being forwarded to pynwb as a string, which `Subject.date_of_birth` does not accept. A subject field that is present but cannot be interpreted now warns, while the conventional spellings of "unknown" stay silent. [PR #1841](https://github.com/catalystneuro/neuroconv/pull/1841)

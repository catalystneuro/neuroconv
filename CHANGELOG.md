...
## Features
* Added `BaseRecordingExtractorInterface.remove_channels`, which drops channels from the recording an interface already holds and returns the interface so the call can be chained. Interfaces that take a channel selection at construction (EDF's `channels_to_skip`, the analog interfaces' `channels_to_include`) are unchanged; this is the generic equivalent for when the channels to drop are only known after the interface exists, which is how the heterogeneous-offset error is answered. [PR #1890](https://github.com/catalystneuro/neuroconv/pull/1890)
...
## Improvements
* The two heterogeneous-offset errors now diagnose rather than rank. Whether the right answer is `data_representation="physical_units"` or dropping channels depends on why the offsets differ, which the writer cannot know but the user can read off the map of which channels carry which offset, so the interface-level error now prints that map as the tools-level one already did, and both name the two fixes as a fork and link to a new how-to guide that works through the distinction. The one answer both used to leave open, splitting the electrodes into one series per offset group, is deliberately not offered: those groups are an artifact of how the file was exported, not a property of the recording. The `TimeSeries` warning for heterogeneous units now says that the physical values will not be recoverable from the file, which was the actual consequence of the `unit="n.a."` fallback it announces. [PR #1890](https://github.com/catalystneuro/neuroconv/pull/1890)

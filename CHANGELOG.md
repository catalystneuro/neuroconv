...
* Deprecated `MedPCInterface`, to be removed on or after August 2027. `MedPCArrayEventsInterface` reads the same files and is the replacement; it writes their events as native `pynwb.event.EventsTable` objects into `nwbfile.events` rather than as `ndx-events` `Events` objects and `IntervalSeries` inside a `BehavioralEpochs` in the behavior processing module, so a conversion that moves over writes a differently shaped file on purpose. The rewrite is mechanical. Which arrays hold events is declared on the interface instead of in the metadata: `event_configuration` is keyed by the MedPC variable itself (`"A"`, not `"left_nose_poke_times"`), so the mapping that had to be given twice, once to `__init__` as `metadata_medpc_name_to_info_dict` and again as `metadata["MedPC"]["medpc_name_to_info_dict"]`, is given once. The session is picked out by `session_header` alone, the header fields MED-PC IV writes above every session, rather than by `session_conditions` plus `start_variable`: the header is fixed by MED-PC IV rather than by the MSN program, so a session always begins at its `Start Date` line and naming that line was never a choice. An old `metadata["MedPC"]["IntervalSeries"]` entry becomes a `duration` on the onset array's entry (`"G": {"name": "port_entries", "duration": "E"}`), written as one durative event type carrying each event's duration rather than as the `+1`/`-1` transition points of an `IntervalSeries`, which also retires the `ValueError` a second interval entry used to raise by constructing `BehavioralEpochs` twice. An old `metadata["MedPC"]["Events"]` entry's `description` becomes `metadata["Events"]["medpc"]["event_types"][<variable>]["event_description"]`, and passing `metadata["MedPC"]` to the new interface raises rather than being silently ignored. The single-line variables `get_metadata` used to copy into `metadata["MedPC"]` are not read at all, since that put data in the metadata dictionary; read them with `medpc_helpers.get_medpc_variables` and write them where they belong. Two written results also change: an array holding no events becomes a zero-row table rather than being skipped, and a durative type whose durations array is shorter than its onsets, which is what a session cut short mid-event leaves behind, records the missing offsets as `NaN` rather than dropping those onsets. A file whose events are all packed into one array is read by the new `MedPCCodedEventsInterface` instead, a layout `MedPCInterface` could not read at all. MedPC is the last interface writing `ndx-events`, so the dependency leaves NeuroConv when this interface is removed. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
...
## Bug Fixes
* A MedPC session is now selected by comparing each header field's value rather than the whole line, so a condition written the natural way matches a header MED-PC space-padded. MED-PC pads a single-digit hour (`Start Time:  9:47:07`) and the exact-line comparison made 272 of the 936 files of one published corpus unreadable, reporting the session as absent. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
...
## Features
* Added `MedPCArrayEventsInterface`, which writes the events of a MedPC output file as native `pynwb.event.EventsTable` objects into `nwbfile.events`. It replaces `MedPCInterface`, now deprecated, and reads the layout that one read: each lettered array holds the onset times of one event type, declared through `event_configuration` keyed by the MedPC variable itself (`"A"`, not `"left_nose_poke_times"`). An entry naming a `duration` is durative and takes its per-event durations from a second array, replacing the `IntervalSeries` the old interface wrote and retiring the `ValueError` a second interval entry used to raise. New with this interface, an entry naming a `payload` carries a per-event value from another array as a column of the same table (`"S": {"name": "cs_presentations", "payload": {"K": "cs_type"}}`), which is how a program that records the type of each trial beside its time is written; the column is written with the raw codes the program wrote and they are relabelled and explained in the metadata through `column_categories`. The session to read is picked out by `session_header`, the fields of the header MED-PC IV writes above every session, and `get_metadata` reports five of those lines, which `MedPCInterface` parsed into `metadata["MedPC"]` where no writer read them: `Start Date` and `Start Time` as `NWBFile/session_start_time`, `Subject` as `Subject/subject_id`, `MSN` as `NWBFile/protocol`, since the program that ran the session is the protocol and is what gives every array its meaning, and `Experiment` as `NWBFile/experiment_description`, the last two only where the line carries something. `Box` and `Group` are not reported, as neither has a field in the NWB schema. The date is taken as month/day/year, which is what every file seen so far uses, and a day-first one is only recognized where its first field is above twelve; the header states no timezone, so the datetime is naive and pynwb attaches the local one. Alignment is `alignment.shift_times` for a rigid offset, plus `set_aligned_timestamps` to substitute times recovered from another device per event type. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* Added `MedPCCodedEventsInterface`, for the files whose event type is carried in the data rather than by the array's name, which nothing in NeuroConv could read before. Which shape a file is in is decided by the MSN program rather than by MedPC, whose `DISKVARS`, `DISKFORMAT` and `DISKCOLUMNS` govern printing alone, which is why it is a separate interface rather than a mode: the two share only how a session is selected and how its header is read. The code is recovered either from the time value it was packed into, through `code_scale` and `code_position` (`"fraction"` for the classic `TIME.EVENTCODE` written by `Set A(Y) = BTIME-U + code/1000`, `"leading"` for the programs that add a large constant instead so the code sits above the time, as in `^PeckLeft=10000`), or from a companion array of one code per event named by `event_type_variable`. The names of the codes go in `event_code_names`, a flat mapping from code to name (`{"001": "lick"}`), kept distinct from the array interface's `event_configuration` because it is a legend rather than a declaration of what to read: every code found becomes an event type whether or not it is named, one left out takes its digits as its name (`code_029`), and a code named but never fired is written as an empty table. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* `MedPCCodedEventsInterface`'s `time_unit` also accepts one unit per event type (`{"1": "seconds", "3": "decaseconds"}`), for a program that timed two event types differently and stored them in the same array. Nothing in the file records that it did; the sign of it is that the pooled times run backwards while each type on its own climbs. One published deposit of 414 files is written that way and could not be read at all before. A mapping has to name every type the array holds, and both the ordering and session-length checks still apply to the result, so a wrong assignment is refused the same way a wrong single unit is. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* Both MedPC events interfaces now state what a stored value is worth, since MedPC records none of it: `time_unit` for the unit the program divided by before writing (seconds, deciseconds, centiseconds, milliseconds, or `clock_ticks` with the rate given by `clock_ticks_per_second`, which is fixed when MED-PC is installed and appears in no file), and `relative_mode` for a program that stored the interval since the previous event rather than the elapsed time, which is what Med Associates' own shipped example procedures do and which they call relative or incremental mode. Getting any of these wrong decodes without error, so the onsets read from a file are now checked to be non-decreasing and a series that runs backwards raises naming the causes rather than writing a plausible-looking file with wrong times. The error names Relative Mode first, since that is the likeliest cause, and asks for an issue where the program says the file is already being read as written, since that is a layout NeuroConv does not support yet. A second bound checks the decoded times against the session's own length, taken from the header's `Start Time` and `End Time`, since no event can happen after the session ended. Ordering alone cannot catch an over-scaled file, because accumulating intervals always yields a rising series and so satisfies it whether or not the program used Relative Mode; the length is what tells the two apart. `-987.987`, MedPC's documented array terminator, is also dropped along with everything after it instead of being read as an event at a negative time. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

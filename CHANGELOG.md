...
* Deprecated `BrukerTiffSinglePlaneImagingInterface`, `BrukerTiffMultiPlaneImagingInterface`, and `BrukerTiffSinglePlaneConverter`. Use the unified `BrukerTiffImagingInterface` instead. Will be removed on or after November 2026. `BrukerTiffMultiPlaneConverter` remains undeprecated for now: its `plane_separation_type="disjoint"` mode (one ``TwoPhotonSeries`` per z-plane) has no replacement on the unified interface yet, pending a `plane_index` selector on `BrukerTiffImagingExtractor` upstream. [PR #1732](https://github.com/catalystneuro/neuroconv/pull/1732)

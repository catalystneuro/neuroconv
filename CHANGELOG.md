...
* The `stimulus_type` column on the intracellular-recordings table is now optional. NWB requires a stimulus type on every `SequentialRecordings` entry, so `_build_icephys_hierarchical_tables` supplies `"not described"` there when the source states none, but an interface whose format carries no protocol (`BrukerVoltageRecordingInterface`) no longer writes a recordings column holding nothing but that placeholder on every row. `MockIcephysInterface` accepts `stimulus_type=None` to write files the same way. `AxonIntracellularInterface` is unchanged. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

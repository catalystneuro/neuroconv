...
## Features
* Added `set_aligned_segment_starting_times_and_rate` method to `ExternalVideoInterface` for constructing timestamps from per-segment starting times and a constant frame rate, without scanning video files frame-by-frame. Also auto-computes `starting_frames` from video metadata when not explicitly provided. [PR #1713](https://github.com/catalystneuro/neuroconv/pull/1713)
...
* Added `metadata_key` support to imaging interfaces (`BaseImagingExtractorInterface`, `TiffImagingInterface`, `Hdf5ImagingInterface`, `SbxImagingInterface`, `MicroManagerTiffImagingInterface`) and `use_new_metadata_format` parameter to `get_metadata()` for dict-based metadata pipeline. [PR #1694](https://github.com/catalystneuro/neuroconv/pull/1694), [PR #1702](https://github.com/catalystneuro/neuroconv/pull/1702)

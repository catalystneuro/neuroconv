...
* `set_probe` on the recording interfaces now takes `contact_id_to_channel_id`, which states which channel recorded each contact and applies it to the probe for you. A probe from a catalogue describes a part rather than a wiring, so it arrives with no channel assignment and could not be attached until the caller called probeinterface's `set_device_channel_indices` themselves. That call takes channel *indices* positional to the probe's own contact order, with `-1` for a contact nothing recorded, which is three conventions to hold at once and none of them is what a wiring table says; a list off by a permutation has the right length, raises nothing, and attributes every channel to the wrong contact. The mapping is by id on both sides, a contact left out of it is one nothing recorded, and a contact or channel that does not exist, or two contacts claiming one channel, is refused. The caller's probe is not modified.

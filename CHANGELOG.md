...
* Removed `neuroconv.tools.signal_processing.discretize_trace`, which never appeared in a tagged release, so there is no deprecation path. The signal-to-events transformation it performed is now split where the data type changes, into conditioning (signal to discrete-valued signal, via `bits`, `thresholds` or `binarize`), detection (discrete-valued signal to event frames, taking no threshold), and a frames-to-seconds step that reads durations from the timestamps rather than estimating them from a median sampling period, so an irregular clock no longer distorts them. The replacements are private while the vocabulary is still settling; `get_rising_frames_from_ttl` and `get_falling_frames_from_ttl` are unaffected and remain public. [PR #1825](https://github.com/catalystneuro/neuroconv/pull/1825)
* Renamed the signal-encoded events configuration parameter `event_specs` to `detection_configuration` and its per-entry reading key `detect` to `detection`. A signal's value is now always a list of detection specs, each spec holding `signal_conditioning`, `detection` and an optional `event_name`, and a half-filled spec raises rather than falling back to a default. Affects `DoricEventsInterface` and `DoricCSVEventsInterface`. None of the old names appeared in a tagged release, so there is no deprecation path. [PR #1825](https://github.com/catalystneuro/neuroconv/pull/1825)
...
## Features
* Added `MockSignalEncodedEventsInterface` to `neuroconv.tools.testing`, a data-free mock of the signal-encoded events pattern that generates a synthetic packed word (and optional analog traces) from a waveform description and derives events from it through the shipped code path. [PR #1825](https://github.com/catalystneuro/neuroconv/pull/1825)

...
## Features
* Added `IntanDigitalInterface` for converting Intan digital TTL lines into discrete events. Lines are selected and edge-detected via an `event_specs` argument keyed by the format's bit positions (backend-independent), writing native `pynwb.event.EventsTable` objects. Single-bit lines are supported; coded multi-bit words are deferred. `IntanConverter` now routes the digital input/output words to this interface automatically. [PR #1812](https://github.com/catalystneuro/neuroconv/pull/1812)

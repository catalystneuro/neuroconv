...
## Features
* Added `metadata_key` (default `"lightning_pose"`) and a `use_new_metadata_format` parameter to `LightningPoseDataInterface.get_metadata()`/`get_metadata_schema()` and to `LightningPoseConverter.get_metadata()` for the dict-based metadata pipeline: top-level `metadata["Devices"]` and top-level `metadata["Pose"]` holding `Skeletons` and `PoseEstimations` registries linked via `device_metadata_key`/`skeleton_metadata_key`, the shape `DeepLabCutInterface` and `MockPoseEstimationInterface` already emit. `add_to_nwbfile` writes either shape, auto-detected from the metadata, so default behavior is unchanged and both shapes produce the same file. The `reference_frame` and `confidence_definition` conversion options of `LightningPoseDataInterface.add_to_nwbfile` are deprecated and will be removed on or after August 2027: no other pose interface exposes them as conversion options, and the dict-based shape carries both per series under `metadata["Pose"]["PoseEstimations"][metadata_key]["PoseEstimationSeries"]`. Passing them still works and applies the value to every series. [PR #1927](https://github.com/catalystneuro/neuroconv/pull/1927)

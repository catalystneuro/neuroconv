...
* `get_metadata()` now returns the dict-based format on every interface that offers both, where it returned the old list-based format before. NeuroConv has written the dict-based shape for itself since [PR #1853](https://github.com/catalystneuro/neuroconv/pull/1853), so this aligns what a caller reads with what the library writes. `use_new_metadata_format=False` still returns the old shape and stays for one release cycle as the escape hatch. Metadata passed into the library is unaffected, as the old shape is converted at every entry point, so a script that states list-based metadata and hands it to `run_conversion` keeps working. What changes is reading the defaults back and editing them: `metadata = interface.get_metadata()` followed by an edit written against the old shape now edits a dictionary of the new one, and since [PR #1873](https://github.com/catalystneuro/neuroconv/pull/1873) an edit landing in a block whose name exists in both shapes is rejected at validation instead of being silently ignored at write time, so those scripts fail loudly. [PR #1934](https://github.com/catalystneuro/neuroconv/pull/1934)

...
## Bug Fixes
* Fixed `MiniscopeImagingInterface` losing the device configuration and crashing on real values. `Miniscope` is now a registered device type built by the shared registry from a single config-to-device mapping, so `gain`, `led0`, `frameRate`, `framesPerFile`, `compression` and `ROI` reach the device instead of only its name, values are coerced to the dtype the ndx-miniscope schema declares (`gain: 3.5` and `frameRate: 50` are real values that raised `TypeError`), and settings the schema has no field for (`ewl`, and the sensor offsets of the `ROI`) are named in the device description rather than dropped. The hardware design the DAQ reports as `deviceType` is now written as the `DeviceModel` that pynwb 4 asks for, in place of the `model_name` it deprecates, so several Miniscopes of one design share a single model object. The device is keyed in `metadata["Devices"]` by the Miniscope it describes rather than by the interface's `metadata_key`, since one Miniscope recorded over several sessions is one device but one series per session. Only the dict-based metadata branch changes. [PR #1846](https://github.com/catalystneuro/neuroconv/pull/1846)
* Fixed `metadata["DeviceModels"]` being dropped on the way to the device registry when writing an imaging plane, so a `Devices` entry naming its model by `device_model_metadata_key` raised `ValueError` reporting the registry the caller had just populated as empty. A `DeviceModel` entry may now also omit the NWB-required `manufacturer`, which an acquisition file rarely records; it is filled where the model is built. [PR #1846](https://github.com/catalystneuro/neuroconv/pull/1846)

...
## Features
* Added `BaseMNEContinuousDataInterface`, a base for interfaces that read continuous data through MNE-Python, covering the EEG/MEG format family (EEGLAB, BrainVision, EGI, FIF) that neither SpikeInterface nor neo reads. A subclass builds an `mne.io.BaseRaw` in `_read_raw()` and the base partitions its channels by MNE channel type: the electrode kinds (`eeg`, `seeg`, `ecog`, `dbs`) become one `ElectricalSeries` backed by an electrodes table carrying `channel_name`, `group` and `location`, and every other kind becomes a `TimeSeries` of its own, one per channel type, carrying that type's unit. Nothing a `Raw` holds is dropped: `eog`, `ecg` and `emg` are electrode voltages but not of neural tissue, `stim` lines are not measurements at all, and `mag`/`grad` are in teslas and teslas per meter, so all of them are written as recorded rather than forced into an `ElectricalSeries` or refused. Routing is by channel type and not by unit, because MNE labels trigger lines volts just as it labels EEG channels volts. Data streams through the new `MNERawDataChunkIterator`, so a `Raw` opened with `preload=False` is written without being held in memory, and a preloaded one (which some formats force when the samples sit inline in the container) goes through the same path from memory. MNE applies the calibration on read and always returns float64, so this writes the `physical_units` representation with `conversion=1.0`. This first version writes no electrode coordinates and is not temporally alignable. `mne` is an optional extra, and the base itself imports it nowhere. [PR #1898](https://github.com/catalystneuro/neuroconv/pull/1898)

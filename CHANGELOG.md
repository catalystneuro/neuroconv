...
## Features
* `KiloSortSortingInterface` now writes the templates in `templates.npy` as the `waveform_mean` column of the units table. Each template is unwhitened with `whitening_mat_inv.npy` and then zeroed again outside the channels Kilosort fit it on, because unwhitening with the dense inverse fills every channel with the inverse-whitening of a zero, which states the noise covariance rather than the cell. The channel axis spans every channel Kilosort sorted, in `channel_map.npy` order, and means the same thing for every unit. Kilosort records no scaling of its own and the schema fixes `waveform_mean` to volts, so the interface must be constructed with `gain_to_uV`; without it the waveforms are skipped with a warning. No electrodes table and no `electrodes` column are written, because a phy folder states which channels a unit came from only as indices into the recording Kilosort was run on, and connecting those to electrodes takes an assertion this interface cannot make or check; the column description records the channel order and where the geometry lives so that link can be made later. [PR #1871](https://github.com/catalystneuro/neuroconv/pull/1871)

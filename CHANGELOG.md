...
## Improvements
* Added `docs/user_guide/bids.rst`, which points at `nwb2bids` for producing a BIDS directory layout. NeuroConv writes NWB files, and BIDS organization is a separate step that runs over finished ones, so the page says to convert first and reorganize afterwards rather than expecting a BIDS output mode on the interfaces. It also notes that `nwb2bids` targets BEP032, the microelectrode electrophysiology extension, which is still under formal review and has not been merged into the BIDS specification, so the layout it produces may still change. [PR #1906](https://github.com/catalystneuro/neuroconv/pull/1906)

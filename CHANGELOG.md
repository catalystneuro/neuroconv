...
## Features
* Added `ExternalImageInterface`, which writes a collection of images into an `Images` container as paths pointing at the source files instead of embedding their pixels, so a session carrying histology or behavioral snapshots that already sit beside it does not get a second copy of them inside the NWB file. NWB allows only PNG, JPEG and GIF by reference and declares `resolution` on the embedded image type alone, so the interface rejects any other format and any per-image resolution, naming the file it stopped on, and it reads the format from the file rather than from its suffix. No color mode conversion happens, since no pixels are written, so an `LA` image stays `LA` where `ImageInterface` converts it to RGBA. `MockExternalImageInterface` joins the testing tools alongside it, taking the format and the color mode as arguments so a conversion can be composed over paths that do not exist. `ImageInterface` is unchanged and still embeds. [PR #1987](https://github.com/catalystneuro/neuroconv/pull/1987)

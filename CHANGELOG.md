...
## Features
* Added `CSVFiberPhotometryInterface` for converting raw fiber photometry recordings stored as per-stream CSV files (the CSV acquisition format read by GuPPy), parsed into the `ndx-fiber-photometry` format. Install with `pip install "neuroconv[csv_fp]"`. [PR #1746](https://github.com/catalystneuro/neuroconv/pull/1746)

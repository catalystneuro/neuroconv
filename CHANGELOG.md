...
* `DeepLabCutInterface` gained a `sampling_frequency` argument and now refuses to write when neither it nor `set_aligned_timestamps` has supplied the timing. A DeepLabCut output file's rows are video frames, and the interface used to write the row index straight into the pose series as if it were seconds, so every conversion that stated no timestamps produced a file whose pose is timed at 1 Hz, one frame per second, silently and with no indication in the file that the number was invented. Nothing in the source can repair it: the `.h5`/`.csv` carries no times and the project config's `video_sets` records only the crop and the path, so the rate has to come from the caller. Pass `sampling_frequency` for a constant frame rate, which is the usual case and makes the times `frame_index / sampling_frequency`, or call `set_aligned_timestamps` when the frames have times of their own. Files already written this way carry wrong timing rather than missing timing, so they need re-converting. [PR #1937](https://github.com/catalystneuro/neuroconv/pull/1937)

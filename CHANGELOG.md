...
* Deprecated `MedPCInterface`, to be removed in v0.12.0, in favor of `MedPCArrayEventsInterface`, which reads the same files and writes their events as native `pynwb.event.EventsTable` objects into `nwbfile.events` rather than as `ndx-events` objects and `IntervalSeries` into the behavior processing module. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
...
## Features
* Added `MedPCArrayEventsInterface` and `MedPCPackedEventsInterface`, which write the events of a MedPC output file as native `pynwb.event.EventsTable` objects into `nwbfile.events`, differing in where an event type's identity comes from: one lettered array per type, or a code packed into the decimals of the time value, whose width needs no stating because `DISKFORMAT` fixes it. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* Both MedPC events interfaces now state what a stored value is worth, since no MedPC file records it: `time_unit`, a named unit or a number of seconds per stored value, and `relative_mode` for a program that stored the interval since the previous event rather than the elapsed time. Every wrong choice decodes without error, so the onsets read are checked for running backwards and against the session length the header states. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

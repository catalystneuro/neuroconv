...
## Features
* Added `InscopixGpioInterface` (analog/monitor GPIO channels as `TimeSeries`, one `metadata["TimeSeries"]` entry each so a channel's `unit` and `conversion` are edited where every other analog interface's are) and `InscopixGpioEventsInterface` (digital and coded GPIO channels derived into `EventsTable`) for converting Inscopix `.gpio` files. [PR #1790](https://github.com/catalystneuro/neuroconv/pull/1790)

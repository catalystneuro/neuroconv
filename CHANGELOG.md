...
## Features
* `GuppyInterface` is now public, which is how a session GuPPy processed out of an existing NWB file is converted: hand the file to `add_to_nwbfile` and GuPPy's outputs are added to it, with the two GuPPy registries linked into the tables already there — each recording site to the `FiberPhotometryTable` rows its fibers occupy, each event to its occurrence rows. Writing to a new path exports the source with the outputs added. Events GuPPy read from outside the file, such as its custom event CSVs, are written as an `EventsTable` of GuPPy's own analyzed onsets for the registry to reference instead, with a warning naming the stores that did not resolve; run standalone, against a file holding no events at all, that table is what the registry links to. [PR #1880](https://github.com/catalystneuro/neuroconv/pull/1880)

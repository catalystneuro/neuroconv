...
* Added `PyPhotometryEventsInterface`, which converts the digital lines of a pyPhotometry `.ppd` recording. Each line rides in the low bit of the words of the analog input it shares a slot with, so it is read as a sampled `0`/`1` signal on that input's own clock and written as one `EventsTable` per line. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

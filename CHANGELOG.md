...
* `DoricEventsInterface` now reads each digital line via a per-line `event_specs` (`detect` one of `rising`/`falling`/`high_period`/`low_period`) and writes durative events with a duration. The default reading is now `high_period` (onset at the rising edge, duration to the falling edge), where it previously emitted rising-edge onsets only, matching `DoricCSVEventsInterface`. [PR #1821](https://github.com/catalystneuro/neuroconv/pull/1821)
* `DoricEventsInterface` now also reads the legacy "EPConsole" `.doric` HDF5 layout (``Traces/<console>/<stream>/<stream>`` with `DI--O-*` digital lines), auto-detected alongside the modern `DataAcquisition` layout. [PR #XXXX](https://github.com/catalystneuro/neuroconv/pull/XXXX)

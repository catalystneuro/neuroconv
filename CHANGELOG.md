...
## Improvements
* Added a temporal alignment user guide (`docs/user_guide/temporal_alignment.rst`) built around a gross-versus-fine model and a proposed `interface.alignment` namespace: gross alignment (repositioning a whole interface) via `shift_times`, fine alignment (re-timing against a reference clock) via `set_times` / `remap_times`, and per-object addressing via `alignment[key]`, with figures generated from a committed script. The API it documents is proposed and not yet implemented. [PR #1793](https://github.com/catalystneuro/neuroconv/pull/1793)

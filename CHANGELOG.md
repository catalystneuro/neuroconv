...
## Bug Fixes
* A MedPC session is now selected by comparing each header field's value rather than the whole line. MED-PC space-pads a single-digit hour (`Start Time:  9:47:07`), which the exact-line comparison never matched, so a condition written the natural way reported the session as absent. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
...
## Features
* Added `MedPCArrayEventsInterface` and `MedPCCodedEventsInterface`, which write the events of a MedPC output file as native `pynwb.event.EventsTable` objects into `nwbfile.events`. They differ in where an event type's identity comes from: one lettered array per type, or a code carried in the data, either packed into the time value or held in a companion array. A code packed into the decimals needs nothing stated, since `DISKFORMAT` fixes how many digits print and those digits are the code. Programs that pack the code into the leading digits instead are not read: the convention is documented but no file written that way was found among about 7,000 surveyed. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)
* Both MedPC events interfaces now state what a stored value is worth, since no MedPC file records it: `time_unit`, a named unit or a number of seconds per stored value, optionally one per event type, and `relative_mode` for a program that stored the interval since the previous event rather than the elapsed time. Every wrong choice decodes without error, so the onsets read are checked for running backwards and against the session length the header states. [PR #1944](https://github.com/catalystneuro/neuroconv/pull/1944)

...
## Improvements
* `DeepLabCutInterface` reads the skeleton's edges from the project config, which states them as pairs of bodypart names, instead of only from the `_meta.pickle` beside the output file. DeepLabCut does not always write that pickle, and when it is absent the skeleton was written with nodes and no connections. The pickle is still read when the config names no skeleton. [PR #ZZZZ](https://github.com/catalystneuro/neuroconv/pull/ZZZZ)

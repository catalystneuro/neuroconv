...
* `set_aligned_timestamps`, `set_aligned_starting_time` and `align_by_interpolation` are deprecated on the fiber photometry interfaces and will be removed on or after August 2027. Each now has a successor under `interface.alignment` (`set_times`, `shift_times` and `remap_times`) and routes into it behind a `FutureWarning`, so the behaviour is unchanged and there is no longer a second alignment mechanism to reconcile with the first. [PR #1922](https://github.com/catalystneuro/neuroconv/pull/1922)

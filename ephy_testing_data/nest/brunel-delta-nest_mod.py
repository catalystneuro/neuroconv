...
p_rate_ex = 240000.0  # external firing rate of a poisson generator
p_rate_in = 18800000.0  # external firing rate of a poisson generator
...
nest.SetStatus(espikes, [{"label": "brunel-py-ex", "withtime": True, "withgid": True, "to_file": True}])

nest.SetStatus(ispikes, [{"label": "brunel-py-in", "withtime": True, "withgid": True, "to_file": True}])
...
"""
recdict = {"to_file": True, "withtime": True, "withgid": True, "time_in_steps": False, "start": tstart}
...
for i, sd in enumerate(spike_detectors_gidtime):
    ...
    nest.SetStatus(lst, [{"to_file": True, "withtime": True}])

    if i == 0:
        nest.SetStatus(lst, [{"label": "0time", "withgid": False, "time_in_steps": False}])
    elif i == 1:
        nest.SetStatus(lst, [{"label": "0gid-1time", "withgid": True, "time_in_steps": False}])
    elif i == 2:
        nest.SetStatus(lst, [{"label": "0time_in_steps", "withgid": False, "time_in_steps": True}])
    elif i == 3:
        nest.SetStatus(lst, [{"label": "0gid-1time_in_steps", "withgid": True, "time_in_steps": True}])

"""
Additional multimeters
"""

nest.SetStatus([multimeters_cond[0]], [{"record_from": ["V_m"], "label": "0gid-1time-2Vm"}])

nest.SetStatus([multimeters_cond[1]], [{"record_from": ["V_m", "g_ex", "g_in"], "label": "0gid-1time-2Vm-3gex-4gin"}])
nest.SetStatus([multimeters_cond[2]], [{"record_from": ["g_ex", "V_m"], "label": "0gid-1time-2gex-3Vm"}])
nest.SetStatus([multimeters_cond[3]], [{"record_from": ["g_ex"], "label": "0gid-1time-2gex"}])
nest.SetStatus(
    [multimeters_cond[4]], [{"record_from": ["V_m"], "label": "0gid-1time_in_steps-2Vm", "time_in_steps": True}]
)

nest.SetStatus(
    [multimeters_curr[0]],
    [{"record_from": ["V_m", "input_currents_ex", "input_currents_in"], "label": "0gid-1time-2Vm-3Iex-4Iin"}],
)


nest.SetStatus(
    [multimeter_1n[0]], [{"record_from": ["V_m"], "label": "N1-0gid-1time-2Vm", "withgid": True, "withtime": True}]
)
nest.SetStatus(
    [multimeter_1n[1]], [{"record_from": ["V_m"], "label": "N1-0time-1Vm", "withgid": False, "withtime": True}]
)
nest.SetStatus([multimeter_1n[2]], [{"record_from": ["V_m"], "label": "N1-0Vm", "withgid": False, "withtime": False}])

...
class BaseSortingExtractorInterface(BaseExtractorInterface):
    ...
    def add_to_nwbfile(
        ...
        cast_int_to_float : bool, default: False
            If True, will cast all integer values to float. This is useful for cases where you need to concatenate
            multiple sorting extractors that may have different properties into a single Units table, since you can
            NaN-fill missing values.
        """
        ...
        if cast_int_to_float:
            first_unit_id = sorting_extractor.get_unit_ids()[0]
            for prop_key in sorting_extractor.get_property_keys():
                prop_dtype = sorting_extractor.get_property(prop_key, [first_unit_id]).dtype
                if np.issubdtype(prop_dtype, np.integer):
                    sorting_extractor.set_property(prop_key, sorting_extractor.get_property(prop_key).astype("float"))

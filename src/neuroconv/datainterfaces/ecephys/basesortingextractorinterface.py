...
class BaseSortingExtractorInterface(BaseExtractorInterface):
    ...
    def add_channel_metadata_to_nwb(self, nwbfile: NWBFile, metadata: Optional[DeepDict] = None):
        ...
        if hasattr(self, "generate_recording_with_channel_metadata"):
            ...
            add_devices_to_nwb(nwbfile=nwbfile, metadata=metadata)
            add_electrode_groups_to_nwb(recording=recording, nwbfile=nwbfile, metadata=metadata)
    ...
    def add_to_nwbfile(
        ...
        """
        from ...tools.spikeinterface import add_sorting_to_nwb

            ...
            for roi_id in roi_ids:
                ...
                try:
                    ...
                    if len(indices[0]) > 0:
                        ...
                        if interface_self.verbose:
                            print(
                                f"get_roi_locations: Found center of mass for ROI {roi_idx}: ({y_center}, {x_center})"
                            )
                    else:
                        if interface_self.verbose:
                        ...
                        try:
                            ...
                            if hasattr(metrics, "x") and hasattr(metrics, "y"):
                                ...
                                if interface_self.verbose:
                                    print(
                                        f"get_roi_locations: Using metrics coordinates for ROI {roi_idx}: ({metrics.y}, {metrics.x})"
                                    )
                            else:
                                ...
                                if interface_self.verbose:
                                    print(
                                        f"get_roi_locations: No coordinates in metrics, using image center for ROI {roi_idx}: ({image_height // 2}, {image_width // 2})"
                                    )
                            ...
                            if interface_self.verbose:
                                print(
                                    f"get_roi_locations: Error getting metrics for ROI {roi_idx}, using image center: ({image_height // 2}, {image_width // 2})"
                                )
                    ...
                    try:
                        ...
                        if interface_self.verbose:
                            print(
                                f"get_roi_locations: Using dimensions from first mask as fallback for ROI {roi_idx}: ({image_height // 2}, {image_width // 2})"
                            )
                    except Exception as e:
                        ...
                        if interface_self.verbose:
                            print(
                                f"get_roi_locations: Could not get any mask dimensions, using (0, 0) for ROI {roi_idx}"
                            )
                            print(f"get_roi_locations: Error details: {type(e).__name__}: {str(e)}")

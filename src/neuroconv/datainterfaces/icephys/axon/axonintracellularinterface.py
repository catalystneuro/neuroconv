...
class AxonIntracellularInterface(BaseDataInterface):
    ...
    records each sweep through the NWB intracellular recordings table via ``(start_index, count)``
    ranges. Multiple electrodes (dual patch) are handled by combining several interface instances in a
    converter.
    """
    ...
    def __init__(
        ...
        *,
        ...
        metadata_key : str, optional
            Identity of this interface's response ``PatchClampSeries`` in the metadata registry. Defaults to the
            file stem plus the response channel (a plain identifier), so combining several interfaces in a
            converter yields distinct keys.
        ...
        # Metadata-registry keys, resolved once here and used by both get_metadata and add_to_nwbfile: the device
        # is shared per file (the stem), the electrode is per response channel, and the series key is the user's
        # `metadata_key` if given, else the electrode key.
        self._device_metadata_key = self._file_path.stem
        self._electrode_metadata_key = f"{self._device_metadata_key}_{response_channel_name}"
        self._series_metadata_key = metadata_key or self._electrode_metadata_key

        # Derived once from the parsed header; read here and by a combining converter. `sequence` is a plain
        # grouping handle (the file stem), not an NWB object name, so it is not CamelCased.
        self._sequence_identifier = self._file_path.stem
    ...
    def get_metadata(self) -> DeepDict:
        ...
        # The registry keys were resolved at construction (device per file, electrode per response channel, series
        # key = the user's metadata_key or the electrode key). Each series entry stores its electrode_metadata_key
        # as an editable link: repointing two series at one electrode key merges them onto a single electrode
        # (e.g. the same cell across files); distinct is the default. `to_camel_case` is applied only here, the one
        # place a key is composed into an NWB object `name`.
    ...
    def add_to_nwbfile(
        ...
        # Offset by _starting_time_shift so future time alignment works
    ...
    def _add_intracellular_table_to_nwb(
        ...
        """Write one intracellular-recordings row per sweep and tag each with the run-level grouping columns.

        For Axon, we know that files have at most one full run. We therefore add the following information:

        - ``sequence``: the run's identity, which we take to be the file stem. The whole file is one run, so every
          sweep shares it.
        - ``stimulus_type``: what kind of run it was (gap-free, the protocol file's name, or "not described"),
          which rides up to that run's sequential recording.
        - ``repetition`` and ``condition``: only when the user gave them (when combining several files in a
          converter), so the runs group into repetitions and experimental conditions; a lone file leaves them out.

        We deliberately add this at the intracellular-recordings table level, so the information is available, but
        we do not build the upper tables (SimultaneousRecordings, SequentialRecordings, and above) here. Building
        those would require asserting that no simultaneous recording spans this run, i.e. that no other electrode
        shares these sweeps, and a single interface can't know that (a converter might still combine it with
        another). That is only settled once something sees all the channels and files, so we defer constructing
        the hierarchy tables to the converter and keep the interface to flat rows plus columns.

        Each row also points at this electrode's continuous response series (and, when present, its stimulus
        series) by the sweep's ``(start_index, count)`` range.
        """
        columns_to_add = {
            "sequence": self._sequence_identifier,
            "stimulus_type": self._extract_stimulus_type(),
        }
        if self._repetition is not None:
            columns_to_add["repetition"] = self._repetition
        if self._condition is not None:
            columns_to_add["condition"] = self._condition
        column_descriptions = {
            "sequence": "Run identity grouping rows into a sequential recording (shared per source file).",
            "stimulus_type": "Stimulus type of the run, carried up to its sequential recording.",
        ...
        # Add any grouping columns that are not in the table yet
        ...
        # One row per sweep, addressing the continuous series by its (start_index, count) range and carrying the
        # grouping columns (which now all exist on the table).
